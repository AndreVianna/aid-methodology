#!/usr/bin/env python3
# pt1h_r7_check.py -- R7 escaping check for PT-1-H parity test (task-056).
#
# Verifies that raw responses from both runtimes (for /api/home or /r/<id>/api/model):
...
#
# The pt1h-repo-a fixture manifest has literal U+2028/U+2029 in aid_version, so the
# escaped forms must appear in /r/<id>/api/model output (proving R7 escape is applied).
# For /api/home, the same aid_version field propagates into the repos[] entry.
#
# Usage: python3 pt1h_r7_check.py PYTHON_RAW_JSON NODE_RAW_JSON
# Exit 0 = all checks pass; exit 1 = one or more checks fail.
import sys

if len(sys.argv) < 3:
    sys.stderr.write("usage: pt1h_r7_check.py PY_JSON NODE_JSON\n")
    sys.exit(1)
...
# These are expected only in the per-repo /api/model response (which serializes ToolInfo.aid_version).
# Caller controls which file pairs to pass -- call with the /r/<id>/api/model pair.
