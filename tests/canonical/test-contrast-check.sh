...
# The approved KB summary is committed at .aid/knowledge/kb.html (format 2: it
# lives beside its KB source; the dashboard serves it at /r/<id>/kb.html).
# Follow it here so CC09 keeps gating the real shipped artifact instead of
# asserting a path that no longer exists.
SUMMARY="${REPO_ROOT}/.aid/knowledge/kb.html"
