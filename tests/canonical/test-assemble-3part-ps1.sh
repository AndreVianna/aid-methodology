...
# The script now takes -Part1 + -Part2 (two parts, no -Mermaid arg).
...
# run -Part1 a -Part2 c -Output d  -> captures OUT, RC
...
run -Part1 "$P1" -Part2 "$P2" -Output "$OUTFILE"
...
# Byte-exact concatenation in order Part1 + Part2 (matches the .sh oracle)
REF="$TMP/ref.html"; cat "$P1" "$P2" > "$REF"
assert_eq "$(cmp -s "$OUTFILE" "$REF" && echo same || echo diff)" "same" \
    "APS04 output is byte-exact Part1+Part2"
assert_file_contains "$OUTFILE" "</body>" "APS05 Part2 content present in output"
