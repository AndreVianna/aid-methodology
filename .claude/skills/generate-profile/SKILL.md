...
1. Every AID skill has a corresponding `canonical/skills/aid-{name}/` directory with
   a `SKILL.md`. The full taxonomy is **111 skill directories**: the **17 curated**
   ...
   alias of `aid-query-kb` + **64 verb-first shortcut skills** + **30 hand-authored `repurpose` skills**
   ...
   the shortcut names, check that every catalog row (excluding `repurpose: true`
   rows) plus every classic skill, `aid-triage`, and `aid-ask` has a rendered
   `canonical/skills/<name>/SKILL.md` directory:

   ```bash
   ls canonical/skills/ | wc -l   # expect 111 (= 17 curated + 94 catalog rows)
   ```
...
- [ ] `canonical/` completeness verified: 111 skills (17 curated incl. aid-triage, + 94 catalog rows = 64 verb-first + 30 repurpose
