...
1. For each task in this delivery, create the nested task folder and seed both files:
   - `.aid/works/{work}/deliveries/delivery-NNN/tasks/task-NNN/DETAIL.md` -- the 6-section task
     definition, seeded from `.claude/aid/templates/task-detail-template.md` (the former flat
   ...
   - `.aid/works/{work}/deliveries/delivery-NNN/tasks/task-NNN/STATE.md` -- seeded from
     `.claude/aid/templates/task-state-template.md`, replacing the frontmatter
...
```bash
bash .claude/aid/scripts/review/check-gaps.sh --ledger .aid/.temp/review-pending/detail.md   # exit 1 = an open criteria gap; do NOT grade
