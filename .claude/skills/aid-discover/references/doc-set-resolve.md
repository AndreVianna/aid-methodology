...
deterministically from `.claude/templates/knowledge-base/*.md` paired to the ownership map
...
synth_default_seed() {
  local tmpl_dir="${REPO:-$(pwd)}/.claude/templates/knowledge-base"
...
raw="$(bash "$REPO/.claude/scripts/config/read-setting.sh" \
        --path discovery.doc_set 2>/dev/null || true)"
