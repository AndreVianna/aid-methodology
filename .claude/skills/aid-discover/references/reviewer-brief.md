...
the `discovery-reviewer` sub-agent. Follows `.claude/templates/reviewer-dispatch.md`.
...
DELIVERABLES:
  ...
  - Grade: computed per .claude/scripts/grade.sh from the ledger; minimum is
    resolved via `bash .claude/scripts/config/read-setting.sh --skill discover --key minimum_grade --default A`
