...
passed to the `aid-reviewer` sub-agent. Follows `.claude/templates/reviewer-dispatch.md`.
...
DELIVERABLES:
  ...
  - Grade: per .claude/scripts/grade.sh; minimum resolved via
    `bash .claude/scripts/config/read-setting.sh --skill interview --key minimum_grade --default A`
