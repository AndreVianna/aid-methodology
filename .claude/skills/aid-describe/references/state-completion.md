...
```bash
bash .claude/aid/scripts/kb/lint-modality.sh --file .aid/works/{work}/REQUIREMENTS.md
```
...
- **[1] Approved:**
  ...
  - Emit pipeline pause signal (silent state-write — no output, no gate):
    ...
    bash .claude/aid/scripts/execute/writeback-state.sh --pipeline --field "Pause Reason" --value "Requirements approved — run /aid-define {work} to decompose into features"
