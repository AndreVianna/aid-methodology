...
Run `.claude/aid/scripts/summarize/grade-summary.sh .aid/knowledge/kb.html`. It orchestrates the AUTO_POOL (machine-verifiable) checks only:
...
```bash
bash .claude/aid/scripts/review/check-gaps.sh --ledger .aid/.temp/review-pending/summarize.md   # exit 1 = an open criteria gap; do NOT grade
