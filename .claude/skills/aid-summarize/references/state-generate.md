...
Use `.claude/templates/knowledge-summary/html-skeleton.html` as the seed when scaffolding
...
- Inlined CSS from `.claude/templates/knowledge-summary/component-css.css` (in `skeleton-head.html`)

Author each section file under `sections/` from KB content:
- Section structure follows `.claude/templates/knowledge-summary/section-templates/{profile}.md`
- Per-section content drawn from KB (cite source via relative `./xxx.md` link)
- 6–8 Mermaid diagrams using syntax patterns from `.claude/templates/knowledge-summary/mermaid-examples.md`

`post-mermaid.html` inlines `.claude/templates/knowledge-summary/lightbox.js` and
`.claude/templates/knowledge-summary/mermaid-init.js`. It is also the file that closes
...
Run `.claude/scripts/summarize/assemble.sh` to concatenate the multi-source layout into the
final single-file `kb.html`. Create `.aid/dashboard/` if absent before writing:

```bash
mkdir -p .aid/dashboard
bash .claude/scripts/summarize/assemble.sh --output .aid/dashboard/kb.html
...
**Critical pitfalls — see `.claude/templates/knowledge-summary/mermaid-examples.md` for full list:**
