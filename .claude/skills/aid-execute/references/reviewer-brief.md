...
to the `aid-reviewer` sub-agent. Follows `.claude/templates/reviewer-dispatch.md`.
...
RUBRIC: .claude/templates/grading-rubric.md (universal severity → grade table)
  - Grade is COMPUTED by .claude/scripts/grade.sh, not judged
...
DELIVERABLES:
  ...
  - Grade: per .claude/scripts/grade.sh; minimum resolved via
    `bash .claude/scripts/config/read-setting.sh --skill execute --key minimum_grade --default A`
