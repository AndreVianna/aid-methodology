...
```bash
cd .claude/aid/scripts/summarize
...
Steps:
1. `npm ci` in `.claude/aid/scripts/summarize/` — reproducible install from lock file.
...
3. `node .claude/aid/scripts/summarize/validate-visuals.mjs .aid/dashboard/kb.html` —
...
The `package.json` + `package-lock.json` in `.claude/aid/scripts/summarize/` ship as part of
...
- The generator (`run_generator.py`) explicitly excludes any `node_modules/` directory at any
  ...
  manifest.
- CI runs `npm ci` in `.claude/aid/scripts/summarize/` before the visual-fidelity gate; the
