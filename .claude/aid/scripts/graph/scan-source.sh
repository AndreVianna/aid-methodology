...
while IFS=$'\t' read -r p gitex; do
    ...
    case "$p" in
        .claude/skills/*/*)
            _rest="${p#.claude/skills/}"
            node=".claude/skills/${_rest%%/*}/"
            case "${_rest#*/}" in
                SKILL.md) printf '%s\tskill\n' "$node" >> "$W/convdir.tsv" ;;
            esac
            ;;
        .claude/agents/*/*)
            _rest="${p#.claude/agents/}"
            node=".claude/agents/${_rest%%/*}/"
            case "${_rest#*/}" in
...
SHORTCUT_CATALOG=".claude/aid/templates/shortcut-catalog.yml"
...
GENERATED_FILES=".claude/aid/templates/generated-files.txt"
...
if [ -s "$W/scanfiles.txt" ]; then
    tr '\n' '\0' < "$W/scanfiles.txt" | xargs -0 awk \
        ...
        function kindof(target, matched, line, from,   fb) {
            ...
            if (from ~ /^canonical\/skills\// && target == ".claude/aid/templates/shortcut-engine.md")
                return "convention"
        ...
        }
        {
            ...
            while (match(rest, /\{\{include:[A-Za-z0-9._-]+\}\}/)) {
                ...
                tgt = ".claude/aid/templates/" nm ".md"
