...
1. **Type** — read the file's path and frontmatter and resolve its **one** document type from
   ...
   `.claude/skills/`, `.claude/agents/`, `.claude/aid/templates/` and `.aid/knowledge/`.
...
`.claude/aid/templates/grading-rubric.md § Issue Severities`; classify against that document
