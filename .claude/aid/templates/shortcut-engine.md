...
- **One run, one work -- gated at the front door.** INTAKE consults the shared Work
  Initiation Gate (`.claude/aid/templates/work-initiation-gate.md`) *before* it
  ...
  (`.claude/aid/templates/state-machine-chaining.md` CHAIN), pausing only for the
...
cost lever here per `.claude/aid/templates/agent-dispatch-tiering.md`: dispatch
...
(`.claude/aid/templates/work-initiation-gate.md`) before allocating anything. Run its
enumeration helper:
...
`.claude/aid/templates/worktree-lifecycle.md § Step 2` (host-native `EnterWorktree` tool
...
2. **GRADE.**
   ```bash
   bash .claude/aid/scripts/review/check-gaps.sh --ledger <ledger-path>   # exit 1 = an open criteria gap; do NOT grade
...
3. **FIX.** Dispatch `aid-architect` (Large; same Dispatch Protocol; the
   ...
   **Circuit breaker.** If the pass's grade has not improved across 3
   ...
   bash .claude/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Artifact" --value ".aid/works/{work}/IMPEDIMENT-gate-{pass}.md"
   bash .claude/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
   ```
