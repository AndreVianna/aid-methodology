...
counterpart to `.claude/aid/templates/work-initiation-gate.md` (Concern B, work-starting) and
consumes -- without re-implementing -- the fixed contract in
`.claude/aid/scripts/works/worktree-lifecycle.sh` and
`.claude/aid/templates/worktree-lifecycle.md` (Concern C).
...
> `bash .claude/aid/scripts/works/enumerate-works.sh` (it parses `git worktree list --porcelain`
...
```bash
LOC=$(bash .claude/aid/scripts/works/worktree-lifecycle.sh locate "$work_id"); rc=$?
...
`.claude/aid/templates/worktree-lifecycle.md § Step 2` (this document never re-describes that
