...
(`.claude/aid/templates/shortcut-engine.md`), `aid-describe`, and the
...
2. **Create and enter the worktree, BEFORE authoring anything:**
   ...
   the resolved path per `.claude/aid/templates/worktree-lifecycle.md § Step
...
2. **Locate:**
   ...
   ```bash
   bash .claude/aid/scripts/works/worktree-lifecycle.sh locate <work-id>
   ```
...
4. **Enter** (rungs 1-3 -- `registered | recreated | created`), unless
   ...
   `.claude/aid/templates/worktree-lifecycle.md § Step 2`, invoke the
