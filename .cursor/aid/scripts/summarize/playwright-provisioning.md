...
`.cursor/aid/scripts/summarize/package.json` (this directory) declares **Playwright 1.61.1**
...
```bash
cd .cursor/aid/scripts/summarize
...
The `package.json` + `package-lock.json` in `.cursor/aid/scripts/summarize/` ship as part of
...
- The generator (`run_generator.py`) explicitly excludes any `node_modules/` directory at any
  ...
  manifest.
- CI runs `npm ci` in `.cursor/aid/scripts/summarize/` before the visual-fidelity gate; the
