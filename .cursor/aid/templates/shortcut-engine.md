...
The single shared engine every thin shortcut doorway (`.cursor/skills/aid-<name>/
...
A thin doorway's body (see `.cursor/skills/aid-<name>/SKILL.md`, generated by
...
- **One run, one work -- gated at the front door.** INTAKE consults the shared Work
  Initiation Gate (`.cursor/aid/templates/work-initiation-gate.md`) *before* it
  ...
  (`.cursor/aid/templates/state-machine-chaining.md` CHAIN), pausing only for the
...
checklist at `.cursor/aid/templates/dispatch-protocol-checklist.md` (ETA lookup,
...
`.cursor/aid/templates/rough-time-hints.md`. On completion, append the dispatch row
...
state's `**Advance:**` per `.cursor/aid/templates/state-machine-chaining.md` -- every
...
cost lever here per `.cursor/aid/templates/agent-dispatch-tiering.md`: dispatch
...
`.cursor/aid/templates/shortcut-scaffolding/{family}.md`, where `{family}` is the
...
hand-authored, single-shot `.cursor/skills/aid-query-kb/SKILL.md`, not a generated
...
Read `.cursor/aid/templates/shortcut-catalog.yml`. Find the row whose `name` equals
...
(`.cursor/aid/templates/work-initiation-gate.md`) before allocating anything. Run its
enumeration helper:
...
frontmatter seed -- see `.cursor/skills/aid-describe/references/state-first-run.md §
...
- `.cursor/aid/templates/shortcut-scaffolding/{family}.md`, if it exists (Family
...
bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field "Active Skill" --value {name}
bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
...
`.cursor/aid/templates/delivery-blueprint-template.md`, placed at the work root
...
1. **REVIEW.** Dispatch `aid-reviewer` (Large; follow the Dispatch Protocol
   ...
   `.cursor/skills/<skill>/references/`). On cycle 1 the ledger does not
...
2. **GRADE.**
   ```bash
   bash .cursor/aid/scripts/review/check-gaps.sh --ledger <ledger-path>   # exit 1 = an open criteria gap; do NOT grade
   bash .cursor/aid/scripts/grade.sh --explain <ledger-path>
   ```
...
3. **FIX.** Dispatch `aid-architect` (Large; same Dispatch Protocol; the
   ...
   **Circuit breaker.** If the pass's grade has not improved across 3
   ...
   `.cursor/skills/aid-execute/references/state-delivery-gate.md § Step
   ...
   bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Reason" --value "GATE {pass} pass circuit breaker -- grade not improving after 3 cycles"
   bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Artifact" --value ".aid/works/{work}/IMPEDIMENT-gate-{pass}.md"
   bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
   ```
...
bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field "Pause Reason" --value "GATE cleared; awaiting user approval before /aid-execute"
bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
