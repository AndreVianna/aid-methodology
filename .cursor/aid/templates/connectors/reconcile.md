...
>   `.cursor/skills/aid-discover/references/state-elicit.md` Step E2, once per ELICIT cycle,
...
>   `.cursor/skills/aid-set-connector/SKILL.md` (upsert) and
>   `.cursor/skills/aid-unset-connector/SKILL.md` (remove) — work-004-connector-consumption's
...
done < <(bash .cursor/aid/scripts/connectors/connector-registry.sh list --root .aid/connectors)
```
...
```bash
bash .cursor/aid/scripts/connectors/connector-registry.sh read <stem> <field> --root .aid/connectors
```
...
- **REMOVE** (`stem ∈ P \ D`) — for each stem classified REMOVE in Step R2, **purge the secret,
  ...
  ```bash
  bash .cursor/aid/scripts/connectors/connector-secret.sh purge <stem> --root .aid/connectors
...
2. **Secret capture is `api`-only (Q10).** For a **tool-managed (`mcp`)** connector or an
   **aid-managed, self-authenticating (`ssh`/`cli`)** connector, **no secret is captured** — no
   ...
   ```bash
   bash .cursor/aid/scripts/connectors/connector-secret.sh write <stem> --root .aid/connectors
   ```
