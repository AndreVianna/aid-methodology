...
> copy `.cursor/aid/templates/design-folder-readme.md` into it as `README.md`. Never
...
(`.cursor/aid/templates/…` → `<install_root>/aid/templates/…`) — there is no per-tool
...
Modeled on `.cursor/skills/aid-design/SKILL.md`. Binds class 1's 21 skills, class 2's 14
...
(`.cursor/aid/templates/work-initiation-gate.md`) by running
`bash .cursor/aid/scripts/works/enumerate-works.sh`; on new work, run
`bash .cursor/aid/scripts/works/worktree-lifecycle.sh create <work-id> <name>`, stop on a
...
(`.cursor/aid/templates/agent-dispatch-tiering.md`); verifier tier is always at least the
producer's tier.
...
3. `bash .cursor/aid/scripts/grade.sh --explain <ledger>`; not clean → loop back to the
...
`.cursor/aid/templates/` (`work-state-template.md`'s `{phase}`, `{skill}`;
