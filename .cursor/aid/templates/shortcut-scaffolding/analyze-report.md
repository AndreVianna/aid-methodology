...
| Task | Type | Notes |
|---|---|---|
| `task-001` | RESEARCH | assess `{target}` against the captured criteria/rubric; at least 2 considerations (e.g. strengths vs. risks, or 2 competing readings of an ambiguous criterion); ends with findings + a recommendation. Emits the reviewer-ledger per the global review-output schema (`.cursor/aid/templates/reviewer-ledger-schema.md` -- CLAUDE.md § Review output format), NOT prose findings, so the task's own output is directly consumable by any downstream FIX loop. |
...
- `.cursor/aid/templates/shortcut-engine.md § Family Scaffolding Consult` -- how
  this file is looked up and what happens when it is absent
- `.cursor/aid/templates/shortcut-scaffolding/document.md § Ownership boundary` --
  where a status/progress report routes instead
- `.cursor/aid/templates/shortcut-scaffolding/test-experiment.md § aid-experiment`
  -- where a controlled experiment/A-B test design routes instead
- `.cursor/aid/templates/shortcut-scaffolding/change-refactor.md § aid-remove /
  § aid-deprecate / § aid-migrate` -- where acting on a review/research
  recommendation routes instead
- `.cursor/aid/templates/reviewer-ledger-schema.md` -- the ledger schema
...
- `.cursor/skills/aid-execute/references/task-type-rules.md ## RESEARCH /
