...
allowed-tools: Read, Glob, Grep, Terminal, Task
---
...
5. **Validate the settings file, then resolve the minimum grade.**
   ...
   ```bash
   bash .cursor/aid/scripts/config/lint-settings.sh --file .aid/settings.yml --quiet
   bash .cursor/aid/scripts/config/read-setting.sh --skill <caller> --key minimum_grade
   ```
...
```bash
bash .cursor/aid/scripts/review/check-gaps.sh --ledger <durable>   # 0 clean, 1 open criteria gap
```
...
```bash
bash .cursor/aid/scripts/grade.sh --explain <durable>
```
...
- `.cursor/aid/templates/reviewer-brief-template.md` — brief and manifest
- `.cursor/aid/templates/review-rubrics/INDEX.md` — rule-set routing
- `.cursor/aid/templates/criteria-gap-protocol.md` — the gate and the register
- `.cursor/aid/templates/reviewer-ledger-schema.md` — the ledger and reconciliation
