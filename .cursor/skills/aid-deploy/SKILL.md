...
- **No `work-NNN`, a free-form description instead** -- the Lite-path direct-entry
  ...
  (`.cursor/aid/templates/shortcut-engine.md`, `INTAKE -> ... -> APPROVAL-HALT`),
...
- **No `work-NNN` argument, but a free-form description was given instead** → the
  ...
  directly to `.cursor/aid/templates/shortcut-engine.md § State: INTAKE` — Steps 1–6
...
2. Resolve work directory: if the `work-NNN` argument was given, use that work
   ...
   run `bash .cursor/aid/scripts/works/enumerate-works.sh` (main tree + every git
...
- [ ] All task grades meet minimum (from `bash .cursor/aid/scripts/config/read-setting.sh --skill deploy --key minimum_grade --default A`)
