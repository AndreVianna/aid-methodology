...
description: >
  ...
  its full behavior is defined by .cursor/skills/aid-test/SKILL.md.
...
(`.cursor/skills/aid-test/SKILL.md`) -- not an alias: its own catalog row
(`alias_of: null`, `{verb: test, artifact: performance}`), `repurpose: true`
(hand-authored). It carries **no logic of its own.**

Execute `.cursor/skills/aid-test/SKILL.md` exactly as written, with the verification
