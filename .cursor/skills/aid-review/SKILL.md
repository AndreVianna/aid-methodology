...
(`.cursor/aid/templates/work-initiation-gate.md`): run
...
the `<work-id>` resolved above). Copy `.cursor/aid/templates/work-state-template.yml` to
...
2. **Dispatch `aid-reviewer` once, in a clean context** (model+effort from INTAKE Step
   ...
   `.cursor/aid/templates/reviewer-dispatch.md` One-off reviews:
...
3. **Gate, then grade the review:** `bash .cursor/aid/scripts/review/check-gaps.sh --ledger <verify-ledger>` (exit 1 = an open criteria gap; do not grade), then `bash .cursor/aid/scripts/grade.sh --explain <verify-ledger>`.
