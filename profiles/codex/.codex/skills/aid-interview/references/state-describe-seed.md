...
The seed-authoring step of `aid-interview` (the `aid-describe` step per D3, executed today by
`aid-interview`). Entered after the seasoned-analyst engine has gathered all requirements intent
...
(`.codex/skills/aid-discover/references/state-review.md` with `greenfield: true`).
...
- `.codex/skills/aid-discover/references/state-review.md` -- the review subsystem invoked at
...
- `.codex/skills/aid-discover/references/reviewer-brief.md` -- reviewer brief template; the
  `{{GREENFIELD_BLOCK}}` substitution ("`greenfield: true` case") must be populated.
- `.codex/skills/aid-discover/references/document-expectations.md` -- `## Greenfield Mode`
  block applies when `greenfield: true` (evidence substitution + as-built red flags relaxed +
  dimension floors retained).
- `.codex/aid/templates/kb-authoring/frontmatter-schema.md` -- `source: forward-authored` is
...
When DESCRIBE-SEED is re-entered on a subsequent `/aid-interview` call (after the user answered a
...
**Invoke `.codex/skills/aid-discover/references/state-review.md` with `greenfield: true`.**

**Reviewer brief (`.codex/skills/aid-discover/references/reviewer-brief.md`):**
