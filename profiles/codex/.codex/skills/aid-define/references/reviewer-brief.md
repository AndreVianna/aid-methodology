# /aid-define — Reviewer Dispatch Brief Template

Loaded by `/aid-define` CROSS-REFERENCE state (State 6). Renders the brief
passed to the `aid-reviewer` sub-agent. Follows `.codex/aid/templates/reviewer-dispatch.md`.
...
DELIVERABLES:
  ...
  - Grade: per .codex/aid/scripts/grade.sh; minimum resolved via
    `bash .codex/aid/scripts/config/read-setting.sh --skill interview --key minimum_grade --default A`
