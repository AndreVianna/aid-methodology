...
```bash
bash .codex/aid/scripts/housekeep/housekeep-state.sh \
...
```bash
bash .codex/aid/scripts/housekeep/branch-commit.sh \
...
bash .codex/aid/scripts/kb/kb-freshness-check.sh \
...
Every extraction subagent in `.codex/skills/aid-discover/references/agent-prompts.md`
...
Read each dispatched agent's full prompt from `.codex/skills/aid-discover/references/
...
(`.codex/skills/aid-discover/SKILL.md S Targeted Discovery (Re-entry)`),
...
the ETA band from `.codex/aid/templates/rough-time-hints.md` for the
...
- `/aid-discover`'s targeted re-entry flips the synthesized entry's
  ...
  `Pending` (`bash .codex/aid/scripts/summarize/writeback-state.sh --set kb_grade Pending`
