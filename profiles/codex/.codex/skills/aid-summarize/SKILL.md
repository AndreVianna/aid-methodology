...
Run `.codex/aid/scripts/summarize/summarize-preflight.sh` before any state. It verifies:
...
| Argument | Effect |
|----------|--------|
| `--grade X` | Override the minimum acceptable grade. Format: `[A-F][-+]?`. Without this, runs `bash .codex/aid/scripts/config/read-setting.sh --skill summary --key minimum_grade --default A` (resolves per-skill override → global `review.minimum_grade` → default `A`). When passed, persist to `.aid/settings.yml` `summary.minimum_grade` via `/aid-config`. |
| `--theme palette=X` | Override color palette (e.g., `--theme palette=brand-acme`). Default uses the canonical palette in `.codex/aid/templates/knowledge-summary/design-tokens.md`. |
...
See `.codex/aid/templates/knowledge-summary/grading-rubric.md` for the per-check definitions and pass criteria. It defines no grade and carries no ladder -- the letter comes from `grade.sh` over the ledger, per `.codex/aid/templates/grading-rubric.md`, AID's single grading rubric.

---

## References

- `.codex/aid/templates/knowledge-summary/prompt.md` — agent guidance for the GENERATE step (long-form)
- `.codex/aid/templates/knowledge-summary/design-tokens.md` — color palette, typography, spacing
- `.codex/aid/templates/knowledge-summary/component-css.css` — full reusable CSS (inlined)
- `.codex/aid/templates/knowledge-summary/lightbox.js` — full reusable JS (theme, lightbox, scrollspy, a11y)
- `.codex/aid/scripts/summarize/validate-visuals.mjs` — §7 visual-fidelity gate (Playwright-based): T1 readable text, T2 minimal overlap, T3 correct layout for every authored visual (inline `<svg>`, `.diagram-box`, infographic container). **Not invoked by `emit-summary-findings.sh`** — run it by hand; see `references/state-validate.md`
- `.codex/aid/templates/knowledge-summary/section-templates/` — `kb-category`-keyed rendering hints (retired as project-type profile selectors)
- `.codex/aid/templates/knowledge-summary/accessibility-checklist.md` — WCAG AA targets, focus trap pattern
- `.codex/aid/templates/knowledge-summary/grading-rubric.md` — the per-check definitions and pass criteria the `SUMMARY-*` rules cite. Defines no grade; `grade.sh` does that from the ledger
- `.codex/aid/templates/knowledge-summary/html-skeleton.html` — doctype, head, semantic landmarks, noscript (with `{{NOSCRIPT_DOC_LIST}}` placeholder for derived doc list)
- `.codex/aid/scripts/summarize/emit-summary-findings.sh` — orchestrates the machine checks and writes a ledger row per failure, citing the rule it breaks. It computes no grade; `grade.sh` does, from the ledger
- `.codex/aid/scripts/summarize/manual-checklist.sh` — records the human-judgment answers (`--input PATH` headless mode; `--interactive` for raw-terminal use). It records; it does not score
- `.codex/aid/scripts/summarize/spot-check-facts.sh` — extracts HTML claims, grep-matches against source KB, writes `.aid/.temp/summarize/spot-check-facts.txt` (aids the user's K2 judgment)
