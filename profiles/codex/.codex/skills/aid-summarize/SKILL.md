...
- `.codex/aid/templates/knowledge-summary/mermaid-init.js` — Mermaid theme variables for both modes (kept for D-011; retired in D-012)
- `.codex/aid/templates/knowledge-summary/mermaid-examples.md` — diagram syntax patterns + pitfalls (kept for D-011; retired in D-012)
...
- `.codex/aid/scripts/summarize/grade-summary.sh` — orchestrates AUTO_POOL checks, reads `.manual-checklist.json` for MANUAL_POOL, prints Machine + Human + Overall grades
...
- `.codex/aid/scripts/summarize/spot-check-facts.sh` — extracts HTML claims, grep-matches against source KB, writes `.spot-check-facts.txt` (aids the user's K2 judgment)
