...
5. Node.js >= 20 is available (required for `validate-visuals.mjs` visual-fidelity validation; the summarize validators declare `"engines": { "node": ">=20" }` in `.codex/aid/scripts/summarize/package.json` alongside Playwright 1.61.1 — Node 18/19 is insufficient; the Mermaid network-fetch check was removed in D-012 / Change 7).
