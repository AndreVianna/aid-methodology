...
On approval:
```bash
bash .codex/aid/scripts/summarize/writeback-state.sh --set summary_approved yes --set last_summary "$(date -u +%Y-%m-%d)"
```
then transition to WRITEBACK.

On rejection:
```bash
bash .codex/aid/scripts/summarize/writeback-state.sh --set summary_approved no
```
