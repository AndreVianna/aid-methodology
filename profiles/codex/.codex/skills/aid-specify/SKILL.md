...
description: >
  ...
  result to that feature's SPEC.md. One feature at a time.
...
3. **Single work** → normalize its `work_id` to the bare `work-NNN` branch name and
   `locate`+enter it (`.codex/aid/templates/downstream-worktree-entry.md` — the
   ...
   check the work STATE.yml's Features State view (row for this feature) and show status.
   Exit.
...
`.codex/aid/templates/downstream-worktree-entry.md` to normalize `<work-id>` to its bare
...
| `--reset` | Clear `## Technical Specification` from SPEC.md and delete STATE.yml. |
