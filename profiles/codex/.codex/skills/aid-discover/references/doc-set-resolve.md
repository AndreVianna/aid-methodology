...
deterministically from `.codex/aid/templates/knowledge-base/*.md` paired to the ownership map
...
Concern ids follow the model in `.codex/aid/templates/kb-authoring/concern-model.md`.
...
synth_default_seed() {
  local tmpl_dir="${REPO:-$(pwd)}/.codex/aid/templates/knowledge-base"
...
raw="$(bash "$REPO/.codex/aid/scripts/config/read-setting.sh" \
        --path discovery.doc_set 2>/dev/null || true)"
...
`.codex/aid/templates/kb-authoring/concern-model.md`) adds an adaptivity layer above
