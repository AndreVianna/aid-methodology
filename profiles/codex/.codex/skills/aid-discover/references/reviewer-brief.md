...
the `aid-reviewer` sub-agent. Follows `.codex/aid/templates/reviewer-dispatch.md`.
...
DECLARED REVIEW CRITERIA (resolve; do not invent):
  ...
  Resolution is defined in .codex/aid/templates/kb-authoring/review-rubric.md
...
DELIVERABLES:
  ...
  - Findings format: per .codex/aid/templates/kb-authoring/principles.md P3 temp-ledger
  - Ledger location: `.aid/.temp/review-pending/discovery.md`
  - Severity scale: per the routed rubric (CRITICAL / HIGH / MEDIUM / LOW / MINOR)
  - Grade: computed per .codex/aid/scripts/grade.sh from the ledger; minimum is
    resolved via `bash .codex/aid/scripts/config/read-setting.sh --skill discover --key minimum_grade --default A`
