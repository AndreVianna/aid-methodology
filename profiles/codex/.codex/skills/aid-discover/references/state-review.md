...
```bash
bash .codex/aid/scripts/kb/closure-check.sh \
...
```bash
bash .codex/aid/scripts/kb/kb-dual-intent-probes.sh essence \
...
```bash
bash .codex/aid/scripts/kb/kb-dual-intent-probes.sh work \
  --doc-set .aid/generated/doc-set.tsv \
  --kb-dir .aid/knowledge \
...
```bash
bash .codex/aid/scripts/kb/kb-actback-task.sh check \
  --doc-set .aid/generated/doc-set.tsv \
  --kb-dir .aid/knowledge \
...
**M2's authoring-standard checks are rule rows now**, in
[`review-rubrics/kb.md`](../../../aid/templates/review-rubrics/kb.md) as `KB-01`–`KB-09`. They used to
live here, inside this dispatch prose, which meant no other caller could apply them and no ledger row
could cite them — a criterion is not dispatch machinery.

The mandate resolves them through the rule set like any other reviewer; their scope note (hand-authored
`primary`/`extension` docs only) is stated in the rule set, so it cannot drift from the rules it bounds.
...
```bash
bash .codex/aid/scripts/summarize/writeback-state.sh --set kb_grade "{grade}" --set last_kb_review "$(date -u +%Y-%m-%d)"
```
