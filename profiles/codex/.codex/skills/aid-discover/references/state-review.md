...
```bash
bash .codex/aid/scripts/grade.sh --explain .aid/.temp/review-pending/discovery.md
```
...
If `--grade` provided, update `.aid/settings.yml` `discover.minimum_grade` (via `/aid-config` or direct YAML edit). Resolve current minimum via `bash .codex/aid/scripts/config/read-setting.sh --skill discover --key minimum_grade --default A`.
