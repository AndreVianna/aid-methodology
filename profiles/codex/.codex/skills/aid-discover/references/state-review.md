...
`.codex/aid/templates/reviewer-dispatch.md` § Dispatch by path: the `Agent` prompt carries the
...
| M2-NNN | [HIGH]   | Pending | schemas.md    | — | [M2] [AUTHORING-LAYOUT] `## Contents` index missing on a doc with more than 3 sections | grep -c "^## " .aid/knowledge/schemas.md = 7, no "## Contents" |
