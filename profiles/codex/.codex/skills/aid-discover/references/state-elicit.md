...
> principle (`.codex/aid/templates/kb-authoring/principles.md` P7 — "Exception (connector
...
signal per `.codex/aid/templates/state-machine-chaining.md` §4. Do **not** write the
...
- **Preset (`<preset-id>`):** read the matching row of
  `.codex/aid/templates/connectors/preset-catalog.md` (LLM-read, as Step 0d reads the domain
...
done < <(bash .codex/aid/scripts/connectors/connector-registry.sh list --root .aid/connectors)
```
...
```bash
bash .codex/aid/scripts/connectors/connector-registry.sh read <stem> <field> --root .aid/connectors
```
...
- **REMOVE** (`stem ∈ P \ D`) — for each stem classified REMOVE in Step R2, **purge the secret,
  ...
  ```bash
  bash .codex/aid/scripts/connectors/connector-secret.sh purge <stem> --root .aid/connectors
...
2. **Secret capture is aid-managed-only (Q10).** For a **tool-managed (`mcp`)** connector, **no
   ...
   **never invoked** (there is no `secret_reference` to fill). For an **aid-managed
   ...
   ```bash
   bash .codex/aid/scripts/connectors/connector-secret.sh write <stem> --root .aid/connectors
   ```
...
```bash
bash .codex/aid/scripts/connectors/build-connectors-index.sh \
