...
4. **Consult the Work Initiation Gate, then allocate the work folder + STATE.** First run
   ...
   `.codex/aid/templates/work-state-template.yml` to
...
3. **Gate, then grade the response:** `bash .codex/aid/scripts/review/check-gaps.sh --ledger <ledger>` (exit 1 = an open criteria gap; do not grade), then `bash .codex/aid/scripts/grade.sh --explain <ledger>`. Not
