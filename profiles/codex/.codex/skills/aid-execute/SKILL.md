...
3. Read second arg (or first arg when shorthand): the `task-NNN` identifier; also resolve `delivery-NNN` from the task identifier or from the Source field
   ...
   **before** step 4 below reads anything under `.aid/works/{work}/…`, follow
   `.codex/aid/templates/downstream-worktree-entry.md` to normalize the work id to its bare
...
Run `bash .codex/aid/scripts/config/read-setting.sh --skill execute --key minimum_grade --default A` to resolve the minimum grade for this skill (`.aid/settings.yml` is the source).
...
- Task mutable state (State, Review, Elapsed, Notes):
  ...
  - **The requirements SLICE this task traces to — not the whole document:**
    ...
    ```
    bash .codex/aid/scripts/execute/slice-requirements.sh .aid/works/{work} task-NNN
    ```
...
`.codex/aid/templates/dispatch-protocol-checklist.md`; read it before any subagent
...
- Grade >= minimum grade (from `bash .codex/aid/scripts/config/read-setting.sh --skill execute --key minimum_grade --default A`)
