...
```bash
bash .codex/aid/scripts/execute/writeback-state.sh --delivery-id NNN --lifecycle Gated
```
...
- **Schema reference:** "Output per `.codex/aid/templates/reviewer-ledger-schema.md`.
...
```bash
bash .codex/aid/scripts/review/check-gaps.sh --ledger .aid/.temp/review-pending/execute-delivery-NNN.md   # exit 1 = an open criteria gap; do NOT grade
bash .codex/aid/scripts/grade.sh --explain .aid/.temp/review-pending/execute-delivery-NNN.md
```
...
bash .codex/aid/scripts/execute/writeback-state.sh --pipeline --field Lifecycle --value "Paused-Awaiting-Input"
bash .codex/aid/scripts/execute/writeback-state.sh --pipeline --field "Pause Reason" --value "Delivery gate blocked on non-CODE issues -- upstream fix required (SPEC/TASK/KB)"
...
```bash
bash .codex/aid/scripts/execute/writeback-state.sh --delivery-id NNN --gate-field Tier --gate-value "{Small|Medium|Large}"
bash .codex/aid/scripts/execute/writeback-state.sh --delivery-id NNN --gate-field Grade --gate-value "{grade}"
bash .codex/aid/scripts/execute/writeback-state.sh --delivery-id NNN --gate-field Timestamp --gate-value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
bash .codex/aid/scripts/execute/writeback-state.sh --delivery-id NNN --block "BLOCK"
```
...
```bash
bash .codex/aid/scripts/execute/writeback-state.sh --delivery-id NNN --lifecycle Done
```
