...
5. **Validate the settings file, then resolve the minimum grade.**
   ...
   ```bash
   bash .codex/aid/scripts/config/lint-settings.sh --file .aid/settings.yml --quiet
   bash .codex/aid/scripts/config/read-setting.sh --skill <caller> --key minimum_grade
   ```
...
```bash
bash .codex/aid/scripts/review/check-gaps.sh --ledger <durable>   # 0 clean, 1 open criteria gap
```
...
```bash
bash .codex/aid/scripts/grade.sh --explain <durable>
```
...
- `.codex/aid/templates/reviewer-brief-template.md` — brief and manifest
- `.codex/aid/templates/review-rubrics/INDEX.md` — rule-set routing
- `.codex/aid/templates/criteria-gap-protocol.md` — the gate and the register
- `.codex/aid/templates/reviewer-ledger-schema.md` — the ledger and reconciliation
