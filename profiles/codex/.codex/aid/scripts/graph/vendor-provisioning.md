...
> whoever next touches `.codex/aid/templates/knowledge-graph/vendor/`.
...
the graph templates under `.codex/aid/templates/graph/`; nothing else depends on them.
...
"/.codex/aid/scripts/graph"`) something to diff against the npm registry, so a version bump
...
6. **Run the full generator** (`run_generator.py`) so every profile picks up the new bytes, and
   ...
   `.codex/aid/templates/knowledge-graph/` already carries.
