...
//   (`.codex/aid/scripts/summarize/assemble.sh`) already validates for
...
function main() {
	...
	const templatesDir = path.join(repoRoot, '.codex/aid/templates/knowledge-graph');
	const summaryTemplatesDir = path.join(repoRoot, '.codex/aid/templates/knowledge-summary');
	const predicatePath = path.join(repoRoot, '.codex/aid/scripts/graph/coverage-predicate.mjs');
	const scaleCeilingPath = path.join(repoRoot, '.codex/aid/templates/graph/scale-ceiling.yml');
	...
	// vendors .codex/aid/templates/knowledge-graph/vendor/, present after, no
