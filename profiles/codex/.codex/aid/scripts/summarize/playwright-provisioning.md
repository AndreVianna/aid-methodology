...
`.codex/aid/scripts/summarize/package.json` (this directory) declares **Playwright 1.61.1**
...
```bash
cd .codex/aid/scripts/summarize
...
Steps:
1. `npm ci` in `.codex/aid/scripts/summarize/` — reproducible install from lock file.
...
3. `node .codex/aid/scripts/summarize/validate-visuals.mjs .aid/dashboard/kb.html` —
...
The `package.json` + `package-lock.json` in `.codex/aid/scripts/summarize/` ship as part of
...
- The generator (`run_generator.py`) explicitly excludes any `node_modules/` directory at any
  ...
  manifest.
- CI runs `npm ci` in `.codex/aid/scripts/summarize/` before the visual-fidelity gate; the
