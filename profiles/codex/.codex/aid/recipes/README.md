...
A recipe is a single Markdown file directly under `.codex/aid/recipes/`. It has two
...
flat form defined in `.codex/aid/templates/delivery-plans/task-template.md`
...
After the agent infers `workType`, it scans `.codex/aid/recipes/` and reads each
...
1. Copy `.codex/aid/templates/recipe-template.md` to `.codex/aid/recipes/{name}.md`
...
7. After adding the file, re-run the work-002 generator to render the updated
   `.codex/aid/recipes/` into all three install trees (claude-code, codex, cursor).
...
- **No recipe versioning.** Updating a recipe file updates everyone's behavior. If
  ...
  old one age out.
- **Flat layout.** Place recipes directly under `.codex/aid/recipes/` — no
...
emerge. Project-local recipes live alongside the seed recipes in `.codex/aid/recipes/`
