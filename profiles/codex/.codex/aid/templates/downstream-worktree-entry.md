...
> `bash .codex/aid/scripts/works/enumerate-works.sh` (it parses `git worktree list --porcelain`
...
```bash
LOC=$(bash .codex/aid/scripts/works/worktree-lifecycle.sh locate "$work_id"); rc=$?
...
`.codex/aid/templates/worktree-lifecycle.md § Step 2` (this document never re-describes that
