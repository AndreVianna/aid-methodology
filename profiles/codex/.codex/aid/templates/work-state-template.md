...
started: "{YYYY-MM-DD}"
minimum_grade: "{resolved at runtime by `bash .codex/aid/scripts/config/read-setting.sh --skill {phase} --key minimum_grade --default A`; source is `.aid/settings.yml`}"
...
consumption contract: `.codex/aid/templates/connectors/consumption-protocol.md`. Coordinate
...
<!-- AUTHORED -- single-delivery FLATTENED works only (see ## Delivery Lifecycle note above).
     ...
     `.aid/works/{work}/delivery-001-issues.md`; see `.codex/aid/templates/delivery-issues.md`) and runs
