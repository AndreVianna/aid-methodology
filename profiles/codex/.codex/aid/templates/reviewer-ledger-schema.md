...
| 4 | `Doc` | yes | Affected file path (relative to repo root). Examples: `foo.md`, `.codex/aid/scripts/bar.sh`, `tests/canonical/baz.sh`. For doc-wide issues with no specific file, use `—`. |
...
```markdown
| 3 | [HIGH] | Pending | .codex/skills/aid-plan/SKILL.md | 42 | SK-01 — dispatch table names a non-existent agent | ls .codex/agents/ |
```
...
`.codex/aid/templates/kb-authoring/review-rubric.md § Resolving review criteria`; this schema
does not restate it.
...
```markdown
| 7 | [LOW] | Pending | .codex/aid/templates/foo.md | 12 | G-01 — inline count not measured at authoring time | resolved LOW via file-level override of G-01 (MINOR global); why: "this count gates a downstream parse" |
```
...
`.codex/aid/templates/grading-rubric.md`** (`§ Issue Severities` and `§ Grade Calculation`).
