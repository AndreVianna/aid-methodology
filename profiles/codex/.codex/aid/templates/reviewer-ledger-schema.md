...
| 4 | `Rule` | yes | The ID of the rule the finding violates, from the artifact's rule set in [`.codex/aid/templates/review-rubrics/INDEX.md`](review-rubrics/INDEX.md). Format `<CLASS>-<NN>` (e.g. `CODE-03`, `NAR-04`). See **Rule values** below. |
| 5 | `Doc` | yes | Affected file path (relative to repo root). Examples: `foo.md`, `.codex/aid/scripts/bar.sh`, `tests/canonical/baz.sh`. For doc-wide issues with no specific file, use `--` (the same sentinel as `Rule` and `Line`; **not** the em dash `—` this schema's older examples used). |
...
is defined once**, at [`.codex/aid/templates/grading-rubric.md#severity-scale`](.codex/aid/templates/grading-rubric.md#severity-scale) --
...
`.codex/aid/scripts/grade.sh` reads the ledger as a markdown table and counts findings by Severity, filtered to Status ∈ {`Pending`, `Recurred`}.
...
**Rows are written by `.codex/aid/scripts/review/writeback-ledger.sh`, one call per row.** Do not
re-emit the table.
...
- `.codex/aid/scripts/review/writeback-ledger.sh` — the sole writer of ledger rows
- `.codex/aid/scripts/grade.sh` — the grader that parses this ledger
- `.codex/aid/templates/review-rubrics/INDEX.md` — the rule sets the `Rule` column cites
- `.codex/agents/aid-reviewer/AGENT.md` — sub-agent output contract (references this schema)
- `.codex/aid/templates/reviewer-dispatch.md` — universal reviewer dispatch brief (references this schema)
