...
The single shared engine every thin shortcut doorway (`.codex/skills/aid-<name>/
...
A thin doorway's body (see `.codex/skills/aid-<name>/SKILL.md`, generated by
...
checklist at `.codex/aid/templates/dispatch-protocol-checklist.md` (ETA lookup,
...
`.codex/aid/templates/rough-time-hints.md`. On completion, emit the closing `✓ ...
...
state's `**Advance:**` per `.codex/aid/templates/state-machine-chaining.md` -- every
...
cost lever here per `.codex/aid/templates/agent-dispatch-tiering.md`: dispatch
...
single-shot `.codex/skills/aid-ask/SKILL.md`, not a generated thin doorway over
INTAKE-DETAIL).
...
Read `.codex/aid/templates/shortcut-catalog.yml`. Find the row whose `name` equals
...
`.codex/aid/templates/worktree-lifecycle.md § Step 2` (host-native `EnterWorktree` tool
...
Copy `.codex/aid/templates/work-state-template.yml` to
...
frontmatter seed -- see `.codex/skills/aid-describe/references/state-first-run.md §
...
- `.codex/aid/templates/shortcut-scaffolding/{family}.md`, if it exists (Family
...
bash .codex/aid/scripts/execute/writeback-state.sh --pipeline --field "Active Skill" --value {name}
bash .codex/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
...
`.codex/skills/aid-detail/references/first-run.md § Step 0`.)
...
`.codex/aid/scripts/execute/derive-waves.sh --from-tasks <work-dir>`, which the
...
1. **REVIEW.** Dispatch `aid-reviewer` (Large; follow the Dispatch Protocol
   ...
   `.codex/skills/<skill>/references/`). On cycle 1 the ledger does not
...
bash .codex/aid/scripts/execute/writeback-state.sh --pipeline --field "Pause Reason" --value "GATE cleared; awaiting user approval before /aid-execute"
bash .codex/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
