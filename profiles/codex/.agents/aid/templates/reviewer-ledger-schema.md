...
| 4 | `Doc` | yes | Affected file path (relative to repo root). Examples: `foo.md`, `.agents/aid/scripts/bar.sh`, `tests/canonical/baz.sh`. For doc-wide issues with no specific file, use `—`. |
...
`.agents/aid/scripts/grade.sh` reads the ledger as a markdown table and counts findings by Severity, filtered to Status ∈ {`Pending`, `Recurred`}.
...
- `.agents/aid/scripts/grade.sh` — the grader that parses this ledger
- `.agents/agents/aid-reviewer/AGENT.md` — sub-agent output contract (references this schema)
- `.agents/aid/templates/reviewer-dispatch.md` — universal reviewer dispatch brief (references this schema)
