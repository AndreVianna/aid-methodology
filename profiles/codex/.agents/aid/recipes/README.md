...
A recipe is a single Markdown file directly under `.agents/aid/recipes/`. It has two
...
flat form defined in `.agents/aid/templates/delivery-plans/task-template.md`
...
After the agent infers `workType`, it scans `.agents/aid/recipes/` and reads each
...
1. Copy `.agents/aid/templates/recipe-template.md` to `.agents/aid/recipes/{name}.md`
...
7. After adding the file, re-run the work-002 generator to render the updated
   `.agents/aid/recipes/` into all three install trees (claude-code, codex, cursor).
...
- **No recipe versioning.** Updating a recipe file updates everyone's behavior. If
  ...
  old one age out.
- **Flat layout.** Place recipes directly under `.agents/aid/recipes/` — no
...
emerge. Project-local recipes live alongside the seed recipes in `.agents/aid/recipes/`
