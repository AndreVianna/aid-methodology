...
`.agents/templates/state-machine-chaining.md`).
...
`.agents/templates/state-machine-chaining.md`). Re-run `/aid-discover` after confirming the
...
(i.e., not synthesized from `.agents/templates/knowledge-base/*.md` by `synth_default_seed`).
