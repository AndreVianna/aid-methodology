...
```bash
raw="$(bash .agents/aid/scripts/config/read-setting.sh \
...
```bash
bash .agents/aid/scripts/kb/build-project-index.sh \
  --root . \
...
`.agents/aid/templates/state-machine-chaining.md`).
...
`.agents/aid/templates/state-machine-chaining.md`). Re-run `/aid-discover` after confirming the
...
(i.e., not synthesized from `.agents/aid/templates/knowledge-base/*.md` by `synth_default_seed`).
...
6. **Preserve** `**Project Type:**`, `**User Approved:**`, `## External Documentation` (the `**Minimum Grade:**` field is now in `.aid/settings.yml` — read it via `bash .agents/aid/scripts/config/read-setting.sh --skill discover --key minimum_grade --default A`)
