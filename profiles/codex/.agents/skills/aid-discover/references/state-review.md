...
3. Include in the prompt:
   - **Ledger lifecycle:** "Read the existing `.aid/.temp/review-pending/discovery.md`
     ...
     no narrative."
   - **Schema reference:** "Output per `.agents/aid/templates/reviewer-ledger-schema.md`."
...
```bash
bash .agents/aid/scripts/grade.sh --explain .aid/.temp/review-pending/discovery.md
```
...
If `--grade` provided, update `.aid/settings.yml` `discover.minimum_grade` (via `/aid-config` or direct YAML edit). Resolve current minimum via `bash .agents/aid/scripts/config/read-setting.sh --skill discover --key minimum_grade --default A`.
