...
If `--grade` provided, update `.aid/settings.yml` `discover.minimum_grade` (via `/aid-config` or direct YAML edit). Add first Review History entry under `## Review History`. Resolve current minimum via `bash .agents/scripts/config/read-setting.sh --skill discover --key minimum_grade --default A`.
