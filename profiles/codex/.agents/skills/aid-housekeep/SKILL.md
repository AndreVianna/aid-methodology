...
| `--grade X` | Pass-through to the SUMMARY-DELTA delegation to `/aid-summarize`. Format: `[A-F][-+]?` (e.g., `A`, `A-`, `B+`). Without this, resolved via `bash .agents/scripts/config/read-setting.sh --skill summary --key minimum_grade --default A`. |
...
Run `bash .agents/scripts/housekeep/housekeep-state.sh --state <STATE_FILE> --resume`
