...
```bash
KB_STAGE=$(bash .agents/aid/scripts/housekeep/housekeep-state.sh \
...
`.agents/aid/scripts/config/read-setting.sh --skill summary --key minimum_grade
...
approved. Do NOT edit anything in `.agents/skills/aid-summarize/` — the skill
runs unmodified.
...
(`.agents/skills/aid-summarize/SKILL.md:60`).
