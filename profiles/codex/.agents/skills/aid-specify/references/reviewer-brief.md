...
`aid-reviewer` sub-agent. Follows `.agents/aid/templates/reviewer-dispatch.md`.
...
DELIVERABLES:
  ...
  - Grade: per .agents/aid/scripts/grade.sh; minimum resolved via
    `bash .agents/aid/scripts/config/read-setting.sh --skill specify --key minimum_grade --default A`
