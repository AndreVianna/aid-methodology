...
- **Ledger lifecycle:** "Append new findings as rows with Status: Pending to
  ...
  first if it exists. Output per `.agents/aid/templates/reviewer-ledger-schema.md` —
  ONE table, no narrative. After writing the ledger, run:
  `bash .agents/aid/scripts/grade.sh .aid/.temp/review-pending/interview-<work>-cross-ref.md`
...
```bash
bash .agents/aid/scripts/grade.sh --explain .aid/.temp/review-pending/interview-<work>-cross-ref.md
```
