...
per `.agents/templates/reviewer-ledger-schema.md` (ONE markdown table, no narrative).
...
```bash
bash .agents/scripts/grade.sh --explain .aid/.temp/review-pending/interview-<work>-lite.md
```
...
`.agents/templates/reviewer-ledger-schema.md`. The table is the entire file content
...
**Minimum grade:** read from `bash .agents/scripts/config/read-setting.sh --skill interview --key minimum_grade --default A` if it
