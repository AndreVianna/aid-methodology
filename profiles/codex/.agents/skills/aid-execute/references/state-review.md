...
The brief carries the universal rubric pointer (`.agents/aid/templates/grading-rubric.md`)
...
3. If the critical persists after the fix → **STOP.** Raise an IMPEDIMENT
   ...
   bash .agents/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Reason" --value "Critical finding persists after fix attempt — architecture-conflict impediment raised"
