...
1. **Read `MaxConcurrent`** from `.aid/knowledge/STATE.md` top-of-file metadata:
   `bash .agents/aid/scripts/config/read-setting.sh --path execution.max_parallel_tasks --default 5` (default `5` if absent).
...
```bash
bash .agents/aid/scripts/execute/writeback-state.sh \
...
2. Update state:
   ...
   ```bash
   bash .agents/aid/scripts/execute/writeback-state.sh --delivery-id DDD --lifecycle Blocked
   ```
   ...
   bash .agents/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Reason" --value "Task failed with unresolved impediment -- task-{NNN}"
   bash .agents/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Artifact" --value ".aid/{work}/IMPEDIMENT-task-{NNN}.md"
...
4. **Compute the failure-block-radius (transitive-descendant BFS):**
   ...
   Run `.agents/aid/scripts/execute/compute-block-radius.sh` with the
...
bash .agents/aid/scripts/execute/writeback-state.sh --pipeline --field Phase --value Execute
bash .agents/aid/scripts/execute/writeback-state.sh --pipeline --field "Active Skill" --value aid-execute
bash .agents/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
