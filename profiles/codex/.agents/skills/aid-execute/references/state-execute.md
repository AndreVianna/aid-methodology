...
1. **Read `MaxConcurrent`** from `.aid/knowledge/STATE.md` top-of-file metadata:
   `bash .agents/scripts/config/read-setting.sh --path execution.max_parallel_tasks --default 5` (default `5` if absent).
...
4. **Compute the failure-block-radius (transitive-descendant BFS):**
   ...
   Run `.agents/scripts/execute/compute-block-radius.sh` with the
...
bash .agents/scripts/execute/writeback-state.sh --pipeline --field Phase --value Execute
bash .agents/scripts/execute/writeback-state.sh --pipeline --field "Active Skill" --value aid-execute
bash .agents/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
