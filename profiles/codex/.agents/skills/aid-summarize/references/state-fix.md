...
- **D1 (diagram parse)** — locate the failing `<pre.mermaid>` block, identify the syntax error from the validator output, apply the fix per `.agents/aid/templates/knowledge-summary/mermaid-examples.md` "Common failure patterns" table.
