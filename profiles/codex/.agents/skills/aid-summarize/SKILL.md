...
Run `.agents/scripts/summarize/summarize-preflight.sh` before any state. It verifies:
...
- `.agents/templates/knowledge-summary/prompt.md` — agent guidance for the GENERATE step (long-form)
- `.agents/templates/knowledge-summary/design-tokens.md` — color palette, typography, spacing
- `.agents/templates/knowledge-summary/component-css.css` — full reusable CSS (inlined)
- `.agents/templates/knowledge-summary/lightbox.js` — full reusable JS (theme, lightbox, scrollspy, a11y)
- `.agents/templates/knowledge-summary/mermaid-init.js` — Mermaid theme variables for both modes
- `.agents/templates/knowledge-summary/mermaid-examples.md` — one valid example per diagram type + pitfalls table
- `.agents/templates/knowledge-summary/section-templates/{profile}.md` — section structure per project type
