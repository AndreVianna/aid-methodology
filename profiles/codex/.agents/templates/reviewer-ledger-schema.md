...
| 4 | `Doc` | yes | Affected file path (relative to repo root). Examples: `foo.md`, `.agents/scripts/bar.sh`, `tests/canonical/baz.sh`. For doc-wide issues with no specific file, use `—`. |
...
`.agents/scripts/grade.sh` reads the ledger as a markdown table and counts findings by Severity, filtered to Status ∈ {`Pending`, `Recurred`}.
...
- `.agents/scripts/grade.sh` — the grader that parses this ledger
- `.agents/agents/reviewer/AGENT.md` and `.agents/agents/discovery-reviewer/AGENT.md` — sub-agent output contracts (reference this schema)
- `.agents/templates/reviewer-dispatch.md` — universal reviewer dispatch brief (references this schema)
