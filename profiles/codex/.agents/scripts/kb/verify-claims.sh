  ...
  while IFS= read -r raw_line; do
    ...
    while IFS= read -r cite; do
      ...
      for prefix in "" ".aid/knowledge/" \
                    "canonical/" ".agents/scripts/" \
                    ".agents/scripts/kb/" ".agents/scripts/execute/" \
                    ".agents/scripts/summarize/" ".agents/scripts/interview/" \
                    ".agents/skills/" ".agents/agents/" ".agents/recipes/" \
                    ".agents/templates/" ".agents/templates/knowledge-base/" \
                    ".agents/templates/knowledge-summary/" \
                    ".agents/templates/kb-authoring/" ".agents/templates/reports/" \
                    ".agents/templates/specs/" ".agents/templates/delivery-plans/" \
                    ".agents/templates/feedback-artifacts/" \
...
# ---------------------------------------------------------------------------
# FRONTMATTER + KB AUTHORING LINT (.agents/templates/kb-authoring/)
# ---------------------------------------------------------------------------
# Per .agents/templates/kb-authoring/principles.md P4, the lint enforces
...
#   - Registered generated files (per .agents/templates/generated-files.txt) exist
...
REGISTRY="$ROOT/.agents/templates/generated-files.txt"
if [[ -f "$REGISTRY" ]]; then
