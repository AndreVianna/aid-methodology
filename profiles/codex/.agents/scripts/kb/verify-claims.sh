...
# resolves to .agents/scripts/kb/preflight.sh (specific) before falling
# through to canonical/ (general).
# ---------------------------------------------------------------------------
PREFIXES=("" ".aid/knowledge/")

# .agents/scripts/*/ — each subdir as a prefix
if [[ -d "$ROOT/canonical/scripts" ]]; then
  PREFIXES+=(".agents/scripts/")
  while IFS= read -r d; do
    PREFIXES+=(".agents/scripts/$(basename "$d")/")
  done < <(find "$ROOT/canonical/scripts" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort)
fi

# .agents/templates/*/ — each subdir as a prefix + .agents/templates/ itself
if [[ -d "$ROOT/canonical/templates" ]]; then
  PREFIXES+=(".agents/templates/")
  while IFS= read -r d; do
    PREFIXES+=(".agents/templates/$(basename "$d")/")
