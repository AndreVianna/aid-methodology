...
| 4 | `Doc` | yes | Affected file path (relative to repo root). Examples: `foo.md`, `.agent/scripts/bar.sh`, `tests/canonical/baz.sh`. For doc-wide issues with no specific file, use `—`. |
...
`.agent/scripts/grade.sh` reads the ledger as a markdown table and counts findings by Severity, filtered to Status ∈ {`Pending`, `Recurred`}.
...
- `.agent/scripts/grade.sh` — the grader that parses this ledger
- `.agent/agents/reviewer/AGENT.md` and `.agent/agents/discovery-reviewer/AGENT.md` — sub-agent output contracts (reference this schema)
- `.agent/templates/reviewer-dispatch.md` — universal reviewer dispatch brief (references this schema)
