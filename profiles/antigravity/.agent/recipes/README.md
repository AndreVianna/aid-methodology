...
A recipe is a single Markdown file directly under `.agent/recipes/`. It has two
...
1. Copy `.agent/templates/recipe-template.md` to `.agent/recipes/{name}.md`
...
7. After adding the file, re-run the work-002 generator to render the updated
   `.agent/recipes/` into all three install trees (claude-code, codex, cursor).
...
- **No recipe versioning.** Updating a recipe file updates everyone's behavior. If
  ...
  old one age out.
- **Flat layout.** Place recipes directly under `.agent/recipes/` — no
...
emerge. Project-local recipes live alongside the seed recipes in `.agent/recipes/`
