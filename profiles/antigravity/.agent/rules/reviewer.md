---
trigger: always_on
description: Adversarial quality evaluator. Produces a structured issue list with severity and source tags. Does NOT fix anything; does NOT compute the grade. Grading is computed deterministically by `.agent/scripts/grade.sh` from the issue list.
---
...
Your output is a single markdown file at `.aid/.temp/review-pending/<scope>.md` containing **exactly one markdown table** per the schema at `.agent/templates/reviewer-ledger-schema.md`.
