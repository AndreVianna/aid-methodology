...
//   (`.agent/aid/scripts/summarize/assemble.sh`) already validates for
...
function main() {
	...
	const templatesDir = path.join(repoRoot, '.agent/aid/templates/knowledge-graph');
	const summaryTemplatesDir = path.join(repoRoot, '.agent/aid/templates/knowledge-summary');
	const predicatePath = path.join(repoRoot, '.agent/aid/scripts/graph/coverage-predicate.mjs');
	const scaleCeilingPath = path.join(repoRoot, '.agent/aid/templates/graph/scale-ceiling.yml');
	...
	// vendors .agent/aid/templates/knowledge-graph/vendor/, present after, no
