...
while IFS=$'\t' read -r p gitex; do
    ...
    case "$p" in
        .agent/skills/*/*)
            _rest="${p#.agent/skills/}"
            node=".agent/skills/${_rest%%/*}/"
            case "${_rest#*/}" in
                SKILL.md) printf '%s\tskill\n' "$node" >> "$W/convdir.tsv" ;;
            esac
            ;;
        .agent/agents/*/*)
            _rest="${p#.agent/agents/}"
            node=".agent/agents/${_rest%%/*}/"
            case "${_rest#*/}" in
...
SHORTCUT_CATALOG=".agent/aid/templates/shortcut-catalog.yml"
...
GENERATED_FILES=".agent/aid/templates/generated-files.txt"
...
if [ -s "$W/scanfiles.txt" ]; then
    tr '\n' '\0' < "$W/scanfiles.txt" | xargs -0 awk \
        ...
        function kindof(target, matched, line, from,   fb) {
            ...
            if (from ~ /^canonical\/skills\// && target == ".agent/aid/templates/shortcut-engine.md")
                return "convention"
