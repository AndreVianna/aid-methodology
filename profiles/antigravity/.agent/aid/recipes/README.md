...
A recipe is a single Markdown file directly under `.agent/aid/recipes/`. It has two
...
flat form defined in `.agent/aid/templates/delivery-plans/task-template.md`
...
After the agent infers `workType`, it scans `.agent/aid/recipes/` and reads each
...
1. Copy `.agent/aid/templates/recipe-template.md` to `.agent/aid/recipes/{name}.md`
...
7. After adding the file, re-run the work-002 generator to render the updated
   `.agent/aid/recipes/` into all three install trees (claude-code, codex, cursor).
...
- **No recipe versioning.** Updating a recipe file updates everyone's behavior. If
  ...
  old one age out.
- **Flat layout.** Place recipes directly under `.agent/aid/recipes/` — no
...
emerge. Project-local recipes live alongside the seed recipes in `.agent/aid/recipes/`
