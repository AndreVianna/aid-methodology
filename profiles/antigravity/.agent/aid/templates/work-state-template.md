...
started: "{YYYY-MM-DD}"
minimum_grade: "{resolved at runtime by `bash .agent/aid/scripts/config/read-setting.sh --skill {phase} --key minimum_grade --default A`; source is `.aid/settings.yml`}"
...
<!-- AUTHORED -- single-delivery FLATTENED works only (see ## Delivery Lifecycle note above).
     ...
     `.aid/works/{work}/delivery-001-issues.md`; see `.agent/aid/templates/delivery-issues.md`) and runs
