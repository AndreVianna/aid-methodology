...
counterpart to `.agent/aid/templates/work-initiation-gate.md` (Concern B, work-starting) and
consumes -- without re-implementing -- the fixed contract in
`.agent/aid/scripts/works/worktree-lifecycle.sh` and
`.agent/aid/templates/worktree-lifecycle.md` (Concern C).
...
> `bash .agent/aid/scripts/works/enumerate-works.sh` (it parses `git worktree list --porcelain`
...
```bash
LOC=$(bash .agent/aid/scripts/works/worktree-lifecycle.sh locate "$work_id"); rc=$?
...
`.agent/aid/templates/worktree-lifecycle.md § Step 2` (this document never re-describes that
