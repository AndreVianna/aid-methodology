...
(`.agent/aid/templates/shortcut-engine.md`), `aid-describe`, and the
...
```bash
bash .agent/aid/scripts/works/enumerate-works.sh
```
...
2. **Create and enter the worktree, BEFORE authoring anything:**
   ...
   the resolved path per `.agent/aid/templates/worktree-lifecycle.md § Step
...
2. **Locate:**
   ...
   ```bash
   bash .agent/aid/scripts/works/worktree-lifecycle.sh locate <work-id>
   ```
...
4. **Enter** (rungs 1-3 -- `registered | recreated | created`), unless
   ...
   `.agent/aid/templates/worktree-lifecycle.md § Step 2`, invoke the
