...
The single shared engine every thin shortcut doorway (`.agent/skills/aid-<name>/
...
A thin doorway's body (see `.agent/skills/aid-<name>/SKILL.md`, generated by
...
- **One run, one work -- gated at the front door.** INTAKE consults the shared Work
  Initiation Gate (`.agent/aid/templates/work-initiation-gate.md`) *before* it
  ...
  (`.agent/aid/templates/state-machine-chaining.md` CHAIN), pausing only for the
...
checklist at `.agent/aid/templates/dispatch-protocol-checklist.md` (ETA lookup,
...
`.agent/aid/templates/rough-time-hints.md`. On completion, append the dispatch row
...
state's `**Advance:**` per `.agent/aid/templates/state-machine-chaining.md` -- every
...
Read `.agent/aid/templates/shortcut-catalog.yml`. Find the row whose `name` equals
...
(`.agent/aid/templates/work-initiation-gate.md`) before allocating anything. Run its
enumeration helper:
...
`.agent/aid/templates/worktree-lifecycle.md § Step 2` (host-native `EnterWorktree` tool
...
frontmatter seed -- see `.agent/skills/aid-describe/references/state-first-run.md §
...
- `.agent/aid/templates/shortcut-scaffolding/{family}.md`, if it exists (Family
...
bash .agent/aid/scripts/execute/writeback-state.sh --pipeline --field "Active Skill" --value {name}
bash .agent/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
...
`.agent/skills/aid-detail/references/first-run.md § Step 0`.)
...
seeded from `.agent/aid/templates/specs/spec-template.md`:
...
`.agent/aid/templates/task-detail-template.md`:
...
1. **REVIEW.** Dispatch `aid-reviewer` (Large; follow the Dispatch Protocol
   ...
   `.agent/skills/<skill>/references/`). On cycle 1 the ledger does not
...
2. **GRADE.**
   ```bash
   bash .agent/aid/scripts/review/check-gaps.sh --ledger <ledger-path>   # exit 1 = an open criteria gap; do NOT grade
   bash .agent/aid/scripts/grade.sh --explain <ledger-path>
   ```
...
3. **FIX.** Dispatch `aid-architect` (Large; same Dispatch Protocol; the
   ...
   **Circuit breaker.** If the pass's grade has not improved across 3
   ...
   `.agent/skills/aid-execute/references/state-delivery-gate.md § Step
   ...
   bash .agent/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Reason" --value "GATE {pass} pass circuit breaker -- grade not improving after 3 cycles"
   bash .agent/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Artifact" --value ".aid/works/{work}/IMPEDIMENT-gate-{pass}.md"
   bash .agent/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
   ```
...
bash .agent/aid/scripts/execute/writeback-state.sh --pipeline --field "Pause Reason" --value "GATE cleared; awaiting user approval before /aid-execute"
bash .agent/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
