...
> copy `.agent/aid/templates/design-folder-readme.md` into it as `README.md`. Never
...
(`.agent/aid/templates/…` → `<install_root>/aid/templates/…`) — there is no per-tool
...
Modeled on `.agent/skills/aid-design/SKILL.md`. Binds class 1's 21 skills, class 2's 14
...
(`.agent/aid/templates/work-initiation-gate.md`) by running
`bash .agent/aid/scripts/works/enumerate-works.sh`; on new work, run
`bash .agent/aid/scripts/works/worktree-lifecycle.sh create <work-id> <name>`, stop on a
...
(`.agent/aid/templates/agent-dispatch-tiering.md`); verifier tier is always at least the
producer's tier.
...
3. `bash .agent/aid/scripts/grade.sh --explain <ledger>`; not clean → loop back to the
...
`.agent/aid/templates/` (`work-state-template.md`'s `{phase}`, `{skill}`;
