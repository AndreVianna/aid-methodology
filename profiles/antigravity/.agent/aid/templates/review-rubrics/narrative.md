...
| `NAR-02` | Every citation uses a durable anchor, not a bare line number | `authoring-conventions.md § Citation Rule (Durable Anchors)` | MUST | mechanical | `bash .agent/aid/scripts/kb/kb-citation-lint.sh --root <dir>` | `Step 2` — one volatile cite is confined → `[MEDIUM]`; widespread → escaped → `[HIGH]` |
| `NAR-03` | Every cited path and anchor resolves | `authoring-conventions.md § Citation Rule (Durable Anchors)` | MUST | mechanical | `bash .agent/aid/scripts/kb/kb-citation-lint.sh --root <dir> --profile resolvable --depth 4` | `Step 2` — one dead cite is confined → `[MEDIUM]`; widespread → escaped → `[HIGH]` |
...
| `NAR-09` | Frontmatter is complete and valid | `authoring-conventions.md § Frontmatter Rules` | MUST | mechanical | `bash .agent/aid/scripts/kb/lint-frontmatter.sh --root <dir>` | `[HIGH]` |
