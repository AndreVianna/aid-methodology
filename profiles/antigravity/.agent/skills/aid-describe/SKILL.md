...
2. **Locate and enter:**
   `bash .agent/aid/scripts/works/worktree-lifecycle.sh locate <work_id>` -- resolves via
...
(`.agent/aid/templates/work-initiation-gate.md`) before doing anything else. Run its
...
2. **Create and enter the worktree, before authoring anything** (gate `§ 3a` step 2):
   `bash .agent/aid/scripts/works/worktree-lifecycle.sh create work-001 {name}` --
...
- **New work:** ask for a name; **resolve `<work-id>`** as `work-{NNN+1}`, where `NNN` is
  ...
  `bash .agent/aid/scripts/works/worktree-lifecycle.sh create <work-id> {name}` -- STOP
