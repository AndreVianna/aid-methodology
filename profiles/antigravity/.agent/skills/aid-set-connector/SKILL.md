...
description: >
  ...
  question-set, prefilled from .agent/aid/templates/connectors/preset-catalog.md when <tool>
...
step (`.agent/skills/aid-discover/references/state-elicit.md` Step E2): use it to add a
...
Read `.agent/aid/templates/connectors/preset-catalog.md` (LLM-read from disk, same as ELICIT's
...
if [ -f ".aid/connectors/${STEM}.md" ]; then
  CLASS=UPDATE
  OLD_TYPE=$(bash .agent/aid/scripts/connectors/connector-registry.sh read "$STEM" connection_type --root .aid/connectors)
  OLD_AUTH=$(bash .agent/aid/scripts/connectors/connector-registry.sh read "$STEM" auth_method --root .aid/connectors)
else
  CLASS=ADD
  OLD_TYPE=""
  OLD_AUTH=""
...
Per [`.agent/aid/templates/connectors/reconcile.md`](../../aid/templates/connectors/reconcile.md)
...
```bash
bash .agent/aid/scripts/connectors/build-connectors-index.sh \
...
(`.agent/aid/templates/kb-authoring/principles.md` P7 "Exception (connector sub-phase)"):
