...
`.agent/aid/templates/state-machine-chaining.md`.
...
**not** use `.agent/aid/scripts/works/worktree-lifecycle.sh` or
`.agent/aid/templates/work-initiation-gate.md` -- both hard-validate a
...
of `.agent/aid/templates/worktree-lifecycle.md § Step 2` only.
...
WT_ROOT="${MAIN_ROOT}/.agent/worktrees"
mkdir -p "$WT_ROOT"
...
> `bash .agent/aid/scripts/config/read-setting.sh --skill update-kb --key minimum_grade --default A`
...
> `.agent/aid/templates/state-machine-chaining.md`. Mechanical states
