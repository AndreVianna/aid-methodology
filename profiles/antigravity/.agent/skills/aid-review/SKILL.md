...
(`.agent/aid/templates/work-initiation-gate.md`): run
...
`bash .agent/aid/scripts/works/worktree-lifecycle.sh create <work-id> <name>` --
...
the `<work-id>` resolved above). Copy `.agent/aid/templates/work-state-template.yml` to
...
2. **Dispatch `aid-reviewer` once, in a clean context** (model+effort from INTAKE Step
   ...
   `.agent/aid/templates/reviewer-dispatch.md` One-off reviews:
...
3. **Gate, then grade the review:** `bash .agent/aid/scripts/review/check-gaps.sh --ledger <verify-ledger>` (exit 1 = an open criteria gap; do not grade), then `bash .agent/aid/scripts/grade.sh --explain <verify-ledger>`.
