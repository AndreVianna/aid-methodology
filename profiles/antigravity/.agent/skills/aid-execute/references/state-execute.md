...
`.agent/aid/templates/connectors/consumption-protocol.md` (`task -> its owning (SPEC-traced)
...
2. **MANDATORY (State-Write Protocol above) -- update state before doing
   ...
   ```bash
   bash .agent/aid/scripts/execute/writeback-state.sh --delivery-id DDD --lifecycle Blocked
   ```
