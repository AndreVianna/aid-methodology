...
```bash
bash .agent/aid/scripts/execute/writeback-state.sh --delivery-id NNN --lifecycle Gated
```
...
```bash
bash .agent/aid/scripts/review/check-gaps.sh --ledger .aid/.temp/review-pending/execute-delivery-NNN.md   # exit 1 = an open criteria gap; do NOT grade
bash .agent/aid/scripts/grade.sh --explain .aid/.temp/review-pending/execute-delivery-NNN.md
```
...
```bash
bash .agent/aid/scripts/execute/writeback-state.sh --delivery-id NNN --lifecycle Blocked
bash .agent/aid/scripts/execute/writeback-state.sh --pipeline --field Lifecycle --value "Paused-Awaiting-Input"
bash .agent/aid/scripts/execute/writeback-state.sh --pipeline --field "Pause Reason" --value "Delivery gate blocked on non-CODE issues -- upstream fix required (SPEC/TASK/KB)"
...
bash .agent/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Reason" --value "Delivery gate circuit breaker triggered -- grade not improving after 3 cycles"
bash .agent/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Artifact" --value ".aid/works/{work}/IMPEDIMENT-delivery-{NNN}.md"
bash .agent/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
...
```bash
bash .agent/aid/scripts/execute/writeback-state.sh --delivery-id NNN --gate-field Tier --gate-value "{Small|Medium|Large}"
bash .agent/aid/scripts/execute/writeback-state.sh --delivery-id NNN --gate-field Grade --gate-value "{grade}"
bash .agent/aid/scripts/execute/writeback-state.sh --delivery-id NNN --gate-field Timestamp --gate-value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
bash .agent/aid/scripts/execute/writeback-state.sh --delivery-id NNN --block "BLOCK"
```
...
```bash
bash .agent/aid/scripts/execute/writeback-state.sh --delivery-id NNN --lifecycle Done
```
