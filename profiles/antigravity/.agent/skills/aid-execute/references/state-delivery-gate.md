...
- **Schema reference:** "Output per `.agent/templates/reviewer-ledger-schema.md`.
...
```bash
bash .agent/scripts/grade.sh --explain .aid/.temp/review-pending/execute-delivery-NNN.md
```
...
bash .agent/scripts/execute/writeback-state.sh --pipeline --field "Pause Reason" --value "Delivery gate blocked on non-CODE issues — upstream fix required (SPEC/TASK/KB)"
...
bash .agent/scripts/execute/writeback-state.sh --pipeline --field "Block Reason" --value "Delivery gate circuit breaker triggered — grade not improving after 3 cycles"
bash .agent/scripts/execute/writeback-state.sh --pipeline --field "Block Artifact" --value ".aid/{work}/IMPEDIMENT-delivery-{NNN}.md"
bash .agent/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
