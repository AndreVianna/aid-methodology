...
Run `bash .agent/aid/scripts/config/read-setting.sh --skill execute --key minimum_grade --default A` to resolve the minimum grade for this skill (`.aid/settings.yml` is the source).
...
- Task mutable state (State, Review, Elapsed, Notes):
  ...
  - **The requirements SLICE this task traces to — not the whole document:**
    ...
    ```
    bash .agent/aid/scripts/execute/slice-requirements.sh .aid/works/{work} task-NNN
    ```
...
different root (`.aid/.worktrees/task-NNN/` vs. the work-level `.agent/worktrees/work-NNN-{name}`),
different branch, different purpose; the two compose without conflict
(`.agent/aid/templates/downstream-worktree-entry.md § Composition with aid-execute's per-task
worktrees`).
...
```bash
bash .agent/aid/scripts/execute/writeback-state.sh \
...
- Grade >= minimum grade (from `bash .agent/aid/scripts/config/read-setting.sh --skill execute --key minimum_grade --default A`)
