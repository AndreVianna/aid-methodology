...
deterministically from `.agent/aid/templates/knowledge-base/*.md` paired to the ownership map
...
Concern ids follow the model in `.agent/aid/templates/kb-authoring/concern-model.md`.
...
synth_default_seed() {
  local tmpl_dir="${REPO:-$(pwd)}/.agent/aid/templates/knowledge-base"
...
raw="$(bash "$REPO/.agent/aid/scripts/config/read-setting.sh" \
        --path discovery.doc_set 2>/dev/null || true)"
...
`.agent/aid/templates/kb-authoring/concern-model.md`) adds an adaptivity layer above
