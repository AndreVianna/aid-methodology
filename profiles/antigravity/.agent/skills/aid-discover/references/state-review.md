...
`.agent/aid/scripts/review/writeback-ledger.sh --set-status` to carry a row forward and
...
```bash
bash .agent/aid/scripts/review/check-gaps.sh --ledger .aid/.temp/review-pending/{{SCOPE}}.md   # exit 1 = an open criteria gap; do NOT grade
