...
bash .agent/scripts/execute/writeback-state.sh --pipeline --field Phase --value Plan
bash .agent/scripts/execute/writeback-state.sh --pipeline --field "Active Skill" --value aid-plan
...
```bash
bash .agent/scripts/grade.sh --explain .aid/.temp/review-pending/plan.md
```

| Condition | Action |
|-----------|--------|
| Grade ≥ minimum (from `bash .agent/scripts/config/read-setting.sh --skill plan --key minimum_grade --default A`) | Move to next deliverable. |
