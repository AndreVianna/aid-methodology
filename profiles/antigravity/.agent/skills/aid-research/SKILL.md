...
4. **Consult the Work Initiation Gate, then allocate the work folder + STATE.** First run
   ...
   `.agent/aid/templates/work-state-template.yml` to
...
3. **Gate, then grade the response:** `bash .agent/aid/scripts/review/check-gaps.sh --ledger <ledger>` (exit 1 = an open criteria gap; do not grade), then `bash .agent/aid/scripts/grade.sh --explain <ledger>`. Not
