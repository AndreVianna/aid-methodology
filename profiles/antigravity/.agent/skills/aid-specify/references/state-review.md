...
```bash
bash .agent/aid/scripts/grade.sh --explain .aid/.temp/review-pending/specify-<feature>.md
```

Compare to minimum grade from `bash .agent/aid/scripts/config/read-setting.sh --skill specify --key minimum_grade --default A`.
...
- **Schema reference:** "Output per `.agent/aid/templates/reviewer-ledger-schema.md`. The
  ledger is the entire file — ONE markdown table, no headers, no narrative."

```bash
bash .agent/aid/scripts/grade.sh --explain .aid/.temp/review-pending/blueprint-delivery-NNN.md
```
