...
1. For each task in this delivery, create the nested task folder and seed both files:
   ...
   - `.aid/{work}/deliveries/delivery-NNN/tasks/task-NNN/STATE.md` -- seeded from
     `.agent/aid/templates/task-state-template.md` with `State: Pending`, empty
...
| Condition | Action |
|-----------|--------|
| Grade ≥ minimum (from `bash .agent/aid/scripts/config/read-setting.sh --skill detail --key minimum_grade --default A`) | Move to next deliverable. |
