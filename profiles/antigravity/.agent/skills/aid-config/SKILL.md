...
- Copy `.agent/aid/templates/settings.yml` → `.aid/settings.yml` verbatim.
...
Run:
```bash
bash .agent/aid/scripts/config/read-setting.sh --path <key> --default '(unset)'
```
...
The canonical resolution helper is `.agent/aid/scripts/config/read-setting.sh`. Consumer skills invoke:
...
Canonical schema: `.agent/aid/templates/settings.yml`. Top-level sections + per-skill overrides as described in the tables above.
