...
Every extraction subagent in `.agent/skills/aid-discover/references/agent-prompts.md`
...
Read each dispatched agent's full prompt from `.agent/skills/aid-discover/references/
...
- `/aid-discover`'s targeted re-entry flips the synthesized entry's
  ...
  `Pending` (`bash .agent/aid/scripts/summarize/writeback-state.sh --set kb_grade Pending`
