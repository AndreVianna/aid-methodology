...
CANDIDATES=$(bash .agent/scripts/housekeep/cleanup-classify.sh \
    --root "$REPO_ROOT" \
...
SLUG=$(bash .agent/scripts/housekeep/housekeep-state.sh --state "$STATE_FILE" --read --field "Branch" | sed 's#^aid/housekeep-##')
[ -z "$SLUG" ] && SLUG="$(date +%Y%m%d)"
bash .agent/scripts/housekeep/branch-commit.sh --ensure-branch --slug "$SLUG"
```
