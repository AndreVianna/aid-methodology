...
Scan `.agent/aid/recipes/` relative to the AID installation root (the same directory that
contains `.agent/skills/`). For each `.md` file, read the `applies-to` and `summary:`
...
`.agent/aid/recipes/` relative to the AID installation root, i.e. the same directory
that contains `.agent/skills/`). For each `.md` file, read the `applies-to` field
