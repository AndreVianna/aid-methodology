...
2. Resolve work directory: if the `work-NNN` argument was given, use that work
   ...
   run `bash .agent/aid/scripts/works/enumerate-works.sh` (main tree + every git
...
- [ ] All task grades meet minimum (from `bash .agent/aid/scripts/config/read-setting.sh --skill deploy --key minimum_grade --default A`)
