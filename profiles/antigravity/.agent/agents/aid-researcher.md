...
- Consult `.aid/connectors/INDEX.md` and, for a relevant `connection_type: mcp` connector, use the host tool's MCP to gather additional evidence (`.agent/aid/templates/connectors/consumption-protocol.md`) — optional, read-only enrichment alongside KB/codebase/web sources; aid-managed (`api`/`ssh`/`url`/`cli`) consumption is out of scope
