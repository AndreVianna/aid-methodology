...
tools: Read, Glob, Grep, Write, Edit, shell
...
- Before implementing, consult `.aid/connectors/INDEX.md`; for a relevant `connection_type: mcp` connector, pull extra context (e.g. a linked ticket's latest description/comments) through the host tool's MCP, following the same connector-resolution ladder and MCP-first read recipe `/aid-read-ticket` embodies (`.github/aid/templates/connectors/ticket-resolution.md` § Connector-Resolution Ladder); a dispatched agent cannot issue that host command itself, so it reads the shared recipe. This is optional, read-only enrichment and never replaces the TASK file's own Scope / Acceptance Criteria.
