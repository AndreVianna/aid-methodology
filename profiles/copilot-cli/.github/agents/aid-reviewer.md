...
`.github/aid/templates/kb-authoring/review-rubric.md § Resolving review criteria`; the short form:
...
full contract lives in `.github/aid/templates/kb-authoring/frontmatter-schema.md § oracle:`.
...
```
| 3 | [HIGH] | Pending | .github/skills/aid-plan/SKILL.md | 88 | SK-01 — the dispatch table names `aid-planner`, which does not exist, so a dispatch at this step resolves to nothing at run time | `ls -d .github/agents/aid-planner` → no such directory; `severity: declared` |
```
...
**Classify against `.github/aid/templates/grading-rubric.md § Issue Severities` — the single
