...
> whoever next touches `.github/aid/templates/knowledge-graph/vendor/`.
...
the graph templates under `.github/aid/templates/graph/`; nothing else depends on them.
...
"/.github/aid/scripts/graph"`) something to diff against the npm registry, so a version bump
...
2. **Re-run the integrity grep** against the new bytes, before replacing anything:
   ...
   for f in <the changed files>; do
     grep -c "canonical/" "$f"; grep -c "AGENTS.md" "$f"
...
6. **Run the full generator** (`run_generator.py`) so every profile picks up the new bytes, and
   ...
   `.github/aid/templates/knowledge-graph/` already carries.
