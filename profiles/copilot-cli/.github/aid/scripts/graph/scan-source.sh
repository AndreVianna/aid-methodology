...
while IFS=$'\t' read -r p gitex; do
    ...
    case "$p" in
        .github/skills/*/*)
            _rest="${p#.github/skills/}"
            node=".github/skills/${_rest%%/*}/"
            case "${_rest#*/}" in
                SKILL.md) printf '%s\tskill\n' "$node" >> "$W/convdir.tsv" ;;
            esac
            ;;
        .github/agents/*/*)
            _rest="${p#.github/agents/}"
            node=".github/agents/${_rest%%/*}/"
            case "${_rest#*/}" in
...
SHORTCUT_CATALOG=".github/aid/templates/shortcut-catalog.yml"
...
GENERATED_FILES=".github/aid/templates/generated-files.txt"
...
if [ -s "$W/scanfiles.txt" ]; then
    tr '\n' '\0' < "$W/scanfiles.txt" | xargs -0 awk \
        ...
        function kindof(target, matched, line, from,   fb) {
            ...
            if (from ~ /^canonical\/skills\// && target == ".github/aid/templates/shortcut-engine.md")
                return "convention"
        ...
        }
        {
            ...
            while (match(rest, /\{\{include:[A-Za-z0-9._-]+\}\}/)) {
                ...
                tgt = ".github/aid/templates/" nm ".md"
