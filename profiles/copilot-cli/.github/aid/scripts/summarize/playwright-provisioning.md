...
`.github/aid/scripts/summarize/package.json` (this directory) declares **Playwright 1.61.1**
...
```bash
cd .github/aid/scripts/summarize
...
Steps:
1. `npm ci` in `.github/aid/scripts/summarize/` — reproducible install from lock file.
...
3. `node .github/aid/scripts/summarize/validate-visuals.mjs .aid/dashboard/kb.html` —
...
The `package.json` + `package-lock.json` in `.github/aid/scripts/summarize/` ship as part of
...
- The generator (`run_generator.py`) explicitly excludes any `node_modules/` directory at any
  ...
  manifest.
- CI runs `npm ci` in `.github/aid/scripts/summarize/` before the visual-fidelity gate; the
