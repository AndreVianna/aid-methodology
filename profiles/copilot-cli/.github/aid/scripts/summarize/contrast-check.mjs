...
//
// Exit codes:
//   0  All checks pass (or all resolvable pairs pass).
//   1  One or more checks failed.
//   2  Invocation error (missing file, unknown flag).
//
// Corrected dark-theme extraction: each named selector's CSS block is read as
// the FIRST occurrence, in document order, that declares at least one custom
// property; a block with none (e.g. component-css.css's
// `html[data-theme="dark"] { color-scheme: dark; }`) is skipped in favour of
// the next matching block. Without this, the dark theme's reported ratios were
// the light theme's, re-checked.
//
// Theme-divergence check (unconditional): when a block matching the dark
// selector is present anywhere in the source, the dark map must differ from
// the light map on at least one token both declare — otherwise the dark
// extraction harvested nothing (or the wrong block) and the "dark" verdict is
// meaningless. Where no such block is present at all, the check reports [N/A]
// rather than failing.
...
function checkTheme(name, vars, pairList) {
	console.log(`\n[${name} theme]`);
	let pass = 0, fail = 0;
	const failures = [];
...
const lightResult = checkTheme('light', light, pairs);
const darkResult = checkTheme('dark', dark, pairs);

// --- Theme divergence (unconditional; emitted directly after the dark theme
// block's own lines, per grade-summary.sh:363's `sed -n '/\[dark theme\]/,$p'`
// — no other default-path line moves) ---
...
console.log('');
const totalFail = lightResult.fail + darkResult.fail + (divergenceFailed ? 1 : 0);
if (totalFail === 0) {
