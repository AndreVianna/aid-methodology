...
>   `.github/skills/aid-discover/references/state-elicit.md` Step E2, once per ELICIT cycle,
...
>   `.github/skills/aid-set-connector/SKILL.md` (upsert) and
>   `.github/skills/aid-unset-connector/SKILL.md` (remove) — work-004-connector-consumption's
...
done < <(bash .github/aid/scripts/connectors/connector-registry.sh list --root .aid/connectors)
```
...
```bash
bash .github/aid/scripts/connectors/connector-registry.sh read <stem> <field> --root .aid/connectors
```
...
2. **Secret capture is `api`-only (Q10).** For a **tool-managed (`mcp`)** connector or an
   **aid-managed, self-authenticating (`ssh`/`cli`)** connector, **no secret is captured** — no
   ...
   ```bash
   bash .github/aid/scripts/connectors/connector-secret.sh write <stem> --root .aid/connectors
   ```
