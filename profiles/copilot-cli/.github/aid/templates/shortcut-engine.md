...
A thin doorway's body (see `.github/skills/aid-<name>/SKILL.md`, generated by
...
- **One run, one work -- gated at the front door.** INTAKE consults the shared Work
  Initiation Gate (`.github/aid/templates/work-initiation-gate.md`) *before* it
  ...
  (`.github/aid/templates/state-machine-chaining.md` CHAIN), pausing only for the
...
state's `**Advance:**` per `.github/aid/templates/state-machine-chaining.md` -- every
...
cost lever here per `.github/aid/templates/agent-dispatch-tiering.md`: dispatch
...
`.github/aid/templates/shortcut-scaffolding/{family}.md`, where `{family}` is the
...
hand-authored, single-shot `.github/skills/aid-query-kb/SKILL.md`, not a generated
...
Read `.github/aid/templates/shortcut-catalog.yml`. Find the row whose `name` equals
...
(`.github/aid/templates/work-initiation-gate.md`) before allocating anything. Run its
enumeration helper:
...
`.github/aid/templates/worktree-lifecycle.md § Step 2` (host-native `EnterWorktree` tool
...
- `.github/aid/templates/shortcut-scaffolding/{family}.md`, if it exists (Family
...
bash .github/aid/scripts/execute/writeback-state.sh --pipeline --field "Active Skill" --value {name}
bash .github/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
...
bash .github/aid/scripts/execute/writeback-state.sh --pipeline --field "Pause Reason" --value "GATE cleared; awaiting user approval before /aid-execute"
bash .github/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
