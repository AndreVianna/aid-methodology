...
counterpart to `.github/aid/templates/work-initiation-gate.md` (Concern B, work-starting) and
consumes -- without re-implementing -- the fixed contract in
`.github/aid/scripts/works/worktree-lifecycle.sh` and
`.github/aid/templates/worktree-lifecycle.md` (Concern C).
...
> `bash .github/aid/scripts/works/enumerate-works.sh` (it parses `git worktree list --porcelain`
...
```bash
LOC=$(bash .github/aid/scripts/works/worktree-lifecycle.sh locate "$work_id"); rc=$?
...
`.github/aid/templates/worktree-lifecycle.md § Step 2` (this document never re-describes that
