...
| `DEF-06` | Every cited path, anchor and identifier resolves | `authoring-conventions.md § Citation Rule (Durable Anchors)` | MUST | mechanical | `bash .github/aid/scripts/kb/kb-citation-lint.sh --root <artifact dir> --profile resolvable --depth 4` | `Step 2` — one bad cite is confined → `[MEDIUM]`; widespread → escaped → `[HIGH]` |
