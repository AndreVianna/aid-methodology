...
> copy `.github/aid/templates/design-folder-readme.md` into it as `README.md`. Never
...
(`.github/aid/templates/…` → `<install_root>/aid/templates/…`) — there is no per-tool
...
Modeled on `.github/skills/aid-design/SKILL.md`. Binds class 1's 21 skills, class 2's 14
...
(`.github/aid/templates/work-initiation-gate.md`) by running
`bash .github/aid/scripts/works/enumerate-works.sh`; on new work, run
`bash .github/aid/scripts/works/worktree-lifecycle.sh create <work-id> <name>`, stop on a
...
(`.github/aid/templates/agent-dispatch-tiering.md`); verifier tier is always at least the
producer's tier.
...
3. `bash .github/aid/scripts/grade.sh --explain <ledger>`; not clean → loop back to the
...
`.github/aid/templates/` (`work-state-template.md`'s `{phase}`, `{skill}`;
