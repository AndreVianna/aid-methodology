...
| 4 | `Doc` | yes | Affected file path (relative to repo root). Examples: `foo.md`, `.github/scripts/bar.sh`, `tests/canonical/baz.sh`. For doc-wide issues with no specific file, use `—`. |
...
`.github/scripts/grade.sh` reads the ledger as a markdown table and counts findings by Severity, filtered to Status ∈ {`Pending`, `Recurred`}.
...
- `.github/scripts/grade.sh` — the grader that parses this ledger
- `.github/agents/reviewer/AGENT.md` and `.github/agents/discovery-reviewer/AGENT.md` — sub-agent output contracts (reference this schema)
- `.github/templates/reviewer-dispatch.md` — universal reviewer dispatch brief (references this schema)
