...
> **Minimum Grade:** {resolved at runtime by `bash .github/scripts/config/read-setting.sh --skill {phase} --key minimum_grade --default A`; source is `.aid/settings.yml`}
> **Started:** {YYYY-MM-DD}
...
> One block per delivery from PLAN.md (or the single work-root SPEC.md delivery on the lite path), written by the delivery-gate closing step of `aid-execute`. Distinct from per-task quick-check findings — the gate aggregates those deferred [HIGH] rows (via `delivery-NNN-issues.md`) and runs a full grade.sh pass. Instances of the deferred-[HIGH] log live at `.aid/work-NNN/delivery-NNN-issues.md`; see `.github/templates/delivery-issues.md` for the template.
...
> Appended by every dispatcher on subagent completion (L1+L2+L3 traceability; always-on, never optional). One row per dispatch. Created on first use ("create section if missing"). Source: `.github/skills/aid-discover/SKILL.md ## Dispatch Protocol`.
