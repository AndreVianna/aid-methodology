...
4. **Consult the Work Initiation Gate, then allocate the work folder + STATE.** First run
   the gate (`.github/aid/templates/work-initiation-gate.md`):
...
3. **Gate, then grade the response:** `bash .github/aid/scripts/review/check-gaps.sh --ledger <ledger>` (exit 1 = an open criteria gap; do not grade), then `bash .github/aid/scripts/grade.sh --explain <ledger>`. Not
