...
deterministically from `.github/templates/knowledge-base/*.md` paired to the ownership map
...
synth_default_seed() {
  local tmpl_dir="${REPO:-$(pwd)}/.github/templates/knowledge-base"
...
raw="$(bash "$REPO/.github/scripts/config/read-setting.sh" \
        --path discovery.doc_set 2>/dev/null || true)"
