...
Run `bash .github/aid/scripts/kb/discover-preflight.sh .aid/knowledge/` to verify:
...
- **Path & citation hygiene** — bare citations missing skill prefix (`SKILL.md` → `.github/skills/<skill>/SKILL.md`); references to files that no longer exist.
...
6. Reset the `kb_grade` frontmatter scalar in STATE.md to `Pending` (`bash .github/aid/scripts/summarize/writeback-state.sh --set kb_grade Pending`) so next run re-reviews
7. Report completion
