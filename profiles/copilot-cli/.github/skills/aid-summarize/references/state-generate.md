...
else: MIN_GRADE = bash .github/scripts/config/read-setting.sh --skill summary --key minimum_grade --default A
```
...
Run `.github/scripts/summarize/fetch-mermaid.sh`. It:
...
- **Document purpose** — first read the YAML frontmatter `intent:` field (per
  `.github/templates/kb-authoring/frontmatter-schema.md`). The `intent:`
...
Use `.github/templates/knowledge-summary/html-skeleton.html` as the seed when scaffolding
...
- Inlined CSS from `.github/templates/knowledge-summary/component-css.css` (in `skeleton-head.html`)

Author each section file under `sections/` from KB content:
- Section structure follows `.github/templates/knowledge-summary/section-templates/{profile}.md`
- Per-section content drawn from KB (cite source via relative `./xxx.md` link)
- 6–8 Mermaid diagrams using syntax patterns from `.github/templates/knowledge-summary/mermaid-examples.md`

`post-mermaid.html` inlines `.github/templates/knowledge-summary/lightbox.js` and
`.github/templates/knowledge-summary/mermaid-init.js`. It is also the file that closes
...
Run `.github/scripts/summarize/assemble.sh` to concatenate the multi-source layout into the
final single-file `kb.html`. Create `.aid/dashboard/` if absent before writing:

```bash
mkdir -p .aid/dashboard
bash .github/scripts/summarize/assemble.sh --output .aid/dashboard/kb.html
...
**Critical pitfalls — see `.github/templates/knowledge-summary/mermaid-examples.md` for full list:**
