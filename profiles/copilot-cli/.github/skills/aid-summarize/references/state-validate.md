...
▶ validation suite starting (~1 min total — **2** validators actually run, `validate-html-output.sh` and `contrast-check.mjs`, at ~30 s each per `.github/aid/templates/rough-time-hints.md`; the coverage and retired-runtime checks are inline)
...
bash .github/aid/scripts/summarize/emit-summary-findings.sh .aid/knowledge/kb.html \
...
> `grep -c validate-visuals .github/aid/scripts/summarize/emit-summary-findings.sh`.
