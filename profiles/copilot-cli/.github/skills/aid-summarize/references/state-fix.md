...
- **D1 (diagram parse — only when Mermaid blocks present)** — locate the failing `<pre class="mermaid">` block, identify the syntax error from the validator output, and fix the Mermaid syntax. Consult `.github/aid/templates/knowledge-summary/mermaid-examples.md` for common failure patterns (kept for D-011; retired in D-012).
