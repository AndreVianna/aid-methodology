...
Compare to minimum grade from `bash .github/aid/scripts/config/read-setting.sh --skill specify --key minimum_grade --default A`.
...
- **Schema reference:** "Output per `.github/aid/templates/reviewer-ledger-schema.md`. The
  ledger is the entire file — ONE markdown table, no headers, no narrative."

```bash
bash .github/aid/scripts/grade.sh --explain .aid/.temp/review-pending/blueprint-delivery-NNN.md
```
