...
CANDIDATES=$(bash .github/scripts/housekeep/cleanup-classify.sh \
    --root "$REPO_ROOT")
CLASSIFY_EXIT=$?
```
...
SLUG=$(bash .github/scripts/housekeep/housekeep-state.sh --state "$STATE_FILE" --read --field "Branch" | sed 's#^aid/housekeep-##')
[ -z "$SLUG" ] && SLUG="$(date +%Y%m%d)"
bash .github/scripts/housekeep/branch-commit.sh --ensure-branch --slug "$SLUG"
```
