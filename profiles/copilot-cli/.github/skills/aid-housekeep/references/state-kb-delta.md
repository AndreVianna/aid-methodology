...
bash .github/aid/scripts/kb/kb-freshness-check.sh \
...
Every extraction subagent in `.github/skills/aid-discover/references/agent-prompts.md`
...
Read each dispatched agent's full prompt from `.github/skills/aid-discover/references/
...
(`.github/skills/aid-discover/SKILL.md S Targeted Discovery (Re-entry)`),
...
the ETA band from `.github/aid/templates/rough-time-hints.md` for the
...
- `/aid-discover`'s targeted re-entry flips the synthesized entry's
  ...
  `Pending` (`bash .github/aid/scripts/summarize/writeback-state.sh --set kb_grade Pending`
