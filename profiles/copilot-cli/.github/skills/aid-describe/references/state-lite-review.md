...
per `.github/aid/templates/reviewer-ledger-schema.md` (ONE markdown table, no narrative).
...
```bash
bash .github/aid/scripts/grade.sh --explain .aid/.temp/review-pending/interview-<work>-lite.md
```
...
`.github/aid/templates/reviewer-ledger-schema.md`. The table is the entire file content
...
**Minimum grade:** read from `bash .github/aid/scripts/config/read-setting.sh --skill interview --key minimum_grade --default A` if it
