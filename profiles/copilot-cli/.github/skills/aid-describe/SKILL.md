...
2. **Locate and enter:**
   `bash .github/aid/scripts/works/worktree-lifecycle.sh locate <work_id>` -- resolves via
...
2. **Create and enter the worktree, before authoring anything** (gate `§ 3a` step 2):
   `bash .github/aid/scripts/works/worktree-lifecycle.sh create work-001 {name}` --
...
- **New work:** ask for a name; **resolve `<work-id>`** as `work-{NNN+1}`, where `NNN` is
  ...
  `bash .github/aid/scripts/works/worktree-lifecycle.sh create <work-id> {name}` -- STOP
