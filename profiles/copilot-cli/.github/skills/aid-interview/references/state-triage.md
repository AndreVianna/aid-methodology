...
Scan `.github/aid/recipes/` relative to the AID installation root (the same directory that
contains `.github/skills/`). For each `.md` file, read the `applies-to` and `summary:`
...
`.github/aid/recipes/` relative to the AID installation root, i.e. the same directory
that contains `.github/skills/`). For each `.md` file, read the `applies-to` field
