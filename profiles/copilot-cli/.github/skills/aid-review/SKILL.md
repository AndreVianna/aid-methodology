...
(`.github/aid/templates/work-initiation-gate.md`): run
...
`bash .github/aid/scripts/works/worktree-lifecycle.sh create <work-id> <name>` --
...
the `<work-id>` resolved above). Copy `.github/aid/templates/work-state-template.yml` to
...
2. **Dispatch `aid-reviewer` once, in a clean context** (model+effort from INTAKE Step
   ...
   `.github/aid/templates/reviewer-dispatch.md` One-off reviews:
...
3. **Gate, then grade the review:** `bash .github/aid/scripts/review/check-gaps.sh --ledger <verify-ledger>` (exit 1 = an open criteria gap; do not grade), then `bash .github/aid/scripts/grade.sh --explain <verify-ledger>`.
