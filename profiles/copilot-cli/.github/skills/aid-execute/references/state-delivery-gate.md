...
- **Schema reference:** "Output per `.github/aid/templates/reviewer-ledger-schema.md`.
...
```bash
bash .github/aid/scripts/execute/writeback-state.sh --delivery-id NNN --lifecycle Blocked
bash .github/aid/scripts/execute/writeback-state.sh --pipeline --field Lifecycle --value "Paused-Awaiting-Input"
bash .github/aid/scripts/execute/writeback-state.sh --pipeline --field "Pause Reason" --value "Delivery gate blocked on non-CODE issues -- upstream fix required (SPEC/TASK/KB)"
...
bash .github/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Reason" --value "Delivery gate circuit breaker triggered -- grade not improving after 3 cycles"
bash .github/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Artifact" --value ".aid/works/{work}/IMPEDIMENT-delivery-{NNN}.md"
bash .github/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
...
bash .github/aid/scripts/execute/writeback-state.sh --delivery-id NNN --gate-field Grade --gate-value "{grade}"
bash .github/aid/scripts/execute/writeback-state.sh --delivery-id NNN --gate-field Timestamp --gate-value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
bash .github/aid/scripts/execute/writeback-state.sh --delivery-id NNN --block "BLOCK"
```
...
```bash
bash .github/aid/scripts/execute/writeback-state.sh --delivery-id NNN --lifecycle Done
```
