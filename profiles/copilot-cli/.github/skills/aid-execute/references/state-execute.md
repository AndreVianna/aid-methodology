...
1. **Read `MaxConcurrent`** from `.aid/knowledge/STATE.md` top-of-file metadata:
   `bash .github/scripts/config/read-setting.sh --path execution.max_parallel_tasks --default 5` (default `5` if absent).
...
2. Update status:
   ...
   bash .github/scripts/execute/writeback-state.sh --pipeline --field "Block Reason" --value "Task failed with unresolved impediment — task-{NNN}"
   bash .github/scripts/execute/writeback-state.sh --pipeline --field "Block Artifact" --value ".aid/{work}/IMPEDIMENT-task-{NNN}.md"
...
4. **Compute the failure-block-radius (transitive-descendant BFS):**
   ...
   Run `.github/scripts/execute/compute-block-radius.sh` with the
...
bash .github/scripts/execute/writeback-state.sh --pipeline --field Phase --value Execute
bash .github/scripts/execute/writeback-state.sh --pipeline --field "Active Skill" --value aid-execute
bash .github/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
