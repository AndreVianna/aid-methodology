...
A recipe is a single Markdown file directly under `.github/recipes/`. It has two
...
flat form defined in `.github/templates/delivery-plans/task-template.md`
...
After the agent infers `workType`, it scans `.github/recipes/` and reads each
...
1. Copy `.github/templates/recipe-template.md` to `.github/recipes/{name}.md`
...
7. After adding the file, re-run the work-002 generator to render the updated
   `.github/recipes/` into all three install trees (claude-code, codex, cursor).
...
- **No recipe versioning.** Updating a recipe file updates everyone's behavior. If
  ...
  old one age out.
- **Flat layout.** Place recipes directly under `.github/recipes/` — no
...
emerge. Project-local recipes live alongside the seed recipes in `.github/recipes/`
