...
A recipe is a single Markdown file directly under `.claude/aid/recipes/`. It has two
...
flat form defined in `.claude/aid/templates/delivery-plans/task-template.md`
...
After the agent infers `workType`, it scans `.claude/aid/recipes/` and reads each
...
1. Copy `.claude/aid/templates/recipe-template.md` to `.claude/aid/recipes/{name}.md`
...
7. After adding the file, re-run the work-002 generator to render the updated
   `.claude/aid/recipes/` into all three install trees (claude-code, codex, cursor).
...
- **No recipe versioning.** Updating a recipe file updates everyone's behavior. If
  ...
  old one age out.
- **Flat layout.** Place recipes directly under `.claude/aid/recipes/` — no
...
emerge. Project-local recipes live alongside the seed recipes in `.claude/aid/recipes/`
