...
# kb-category into the two emitted sections Primary and Meta (a doc declaring
# kb-category: extension is folded into Primary), and is source-value-agnostic. A
...
# schema is Document/Audience/Tags/See-instead/Objective/Summary.
#
...
Per .claude/aid/templates/kb-authoring/principles.md P3, this script runs LAST
...
#
#     <kb-category>\n<aud_cell> | <tags_cell> | <see_cell> | <obj_cell> | <sum_cell>
#
...
END {
    ...
    print cat
    print aud_cell " | " tags_cell " | " see_cell " | " obj_cell " | " sum_cell
}
KBIDX_AWK_EOF
...
<!-- AUTO-GENERATED $TS by .claude/aid/scripts/kb/build-kb-index.sh -->
<!-- DO NOT EDIT - regenerate with: bash .claude/aid/scripts/kb/build-kb-index.sh --root .aid/knowledge --output .aid/knowledge/INDEX.md -->
...
EOF
    ...
    for f in "${all_docs[@]}"; do
        ...
        # extension is provenance, not routing: fold those docs into Primary so the
        # section has one alphabetical sort. Full if-block, not `[[ ... ]] && ...`:
        # under `set -e` a false trailing &&-list would abort this loop.
        if [[ "$cat_line" == "extension" ]]; then
            cat_line="primary"
        fi
    ...
    for category in primary meta; do
        emitted_header=0
        for i in "${!all_docs[@]}"; do
            if [[ "${doc_cat[$i]}" == "$category" ]]; then
                if [[ $emitted_header -eq 0 ]]; then
                    ...
                    echo ""
                    echo "| Document | Audience | Tags | See-instead | Objective | Summary |"
                    echo "|----------|----------|------|-------------|-----------|---------|"
                    emitted_header=1
                fi
                echo "${doc_row[$i]}"
    ...
    echo "---"
    echo ""
    echo "*To regenerate this index, run \`bash .claude/aid/scripts/kb/build-kb-index.sh --root .aid/knowledge --output .aid/knowledge/INDEX.md\`.*"
