...
Name of the build script (relative to `.claude/aid/scripts/` or as a
project-relative path). Listed in `.claude/aid/templates/generated-files.txt` registry.
...
- Each entry is one of:
  - A **repo-relative path** (`src/foo.ts`, `.claude/aid/scripts/kb/`)
  - A **glob** (`src/parsers/*.py`, `.claude/aid/templates/*.md`)
...
```yaml
sources:
  - .claude/aid/templates/knowledge-base/
