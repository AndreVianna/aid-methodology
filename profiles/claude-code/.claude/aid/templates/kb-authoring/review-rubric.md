...
[`.claude/aid/templates/grading-rubric.md#severity-scale`](.claude/aid/templates/grading-rubric.md#severity-scale). Modality sets the band; blast radius
and reversibility select within the MUST band. Not restated here.

Worst-issue dominates the grade (per `.claude/aid/templates/grading-rubric.md`).
...
Per `.claude/aid/templates/grading-rubric.md`, the grade is COMPUTED from the severity
...
| `[TEACHBACK]` | _the cited rule's anchor_ | A teach-back FAIL item — the KB does not support defining the cited concept from the KB alone (per-term limb), or the KB cannot support a coherent engine-narration (non-lexical limb); any open `[TEACHBACK]` row forces grade <= D |
