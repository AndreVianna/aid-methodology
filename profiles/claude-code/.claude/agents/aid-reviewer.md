...
- **Look severity up** against the violated rule's anchor — see [`.claude/aid/templates/grading-rubric.md#severity-scale`](.claude/aid/templates/grading-rubric.md#severity-scale). You do not *assign* it; the rule and the instance determine it. **No invented severity bands.**
...
Your output is a single markdown table in **the scratch ledger path you were given**, per the schema at `.claude/aid/templates/reviewer-ledger-schema.md`. You are never told the durable ledger's path — see the resume contract below.
