...
Scan `.claude/aid/recipes/` relative to the AID installation root (the same directory that
contains `.claude/skills/`). For each `.md` file, read the `applies-to` and `summary:`
...
`.claude/aid/recipes/` relative to the AID installation root, i.e. the same directory
