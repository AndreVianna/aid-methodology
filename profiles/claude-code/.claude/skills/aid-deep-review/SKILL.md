...
5. **Validate the settings file, then resolve the minimum grade.**
   ...
   ```bash
   bash .claude/aid/scripts/config/lint-settings.sh --file .aid/settings.yml --quiet
   bash .claude/aid/scripts/config/read-setting.sh --skill <caller> --key minimum_grade
   ```
