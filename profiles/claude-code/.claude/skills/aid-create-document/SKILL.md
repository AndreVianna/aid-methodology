...
3. **Gate, then grade:** `bash .claude/aid/scripts/review/check-gaps.sh --ledger <ledger>` (exit 1 = an open criteria gap; do not grade), then `bash .claude/aid/scripts/grade.sh --explain <ledger>`. Not clean -> loop
