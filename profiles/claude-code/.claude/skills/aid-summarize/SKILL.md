...
See `.claude/aid/templates/knowledge-summary/grading-rubric.md` for the per-check definitions and pass criteria. It defines no grade and carries no ladder -- the letter comes from `grade.sh` over the ledger, per `.claude/aid/templates/grading-rubric.md`, AID's single grading rubric.

---

## References

- `.claude/aid/templates/knowledge-summary/prompt.md` — agent guidance for the GENERATE step (long-form)
- `.claude/aid/templates/knowledge-summary/design-tokens.md` — color palette, typography, spacing
- `.claude/aid/templates/knowledge-summary/component-css.css` — full reusable CSS (inlined)
- `.claude/aid/templates/knowledge-summary/lightbox.js` — full reusable JS (theme, lightbox, scrollspy, a11y)
...
- `.claude/aid/templates/knowledge-summary/section-templates/` — `kb-category`-keyed rendering hints (retired as project-type profile selectors)
- `.claude/aid/templates/knowledge-summary/accessibility-checklist.md` — WCAG AA targets, focus trap pattern
- `.claude/aid/templates/knowledge-summary/grading-rubric.md` — the per-check definitions and pass criteria the `SUMMARY-*` rules cite. Defines no grade; `grade.sh` does that from the ledger
...
- `.claude/aid/scripts/summarize/emit-summary-findings.sh` — orchestrates the machine checks and writes a ledger row per failure, citing the rule it breaks. It computes no grade; `grade.sh` does, from the ledger
- `.claude/aid/scripts/summarize/manual-checklist.sh` — records the human-judgment answers (`--input PATH` headless mode; `--interactive` for raw-terminal use). It records; it does not score
