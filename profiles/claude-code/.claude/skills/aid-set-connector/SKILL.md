...
description: >
  ...
  question-set, prefilled from .claude/aid/templates/connectors/preset-catalog.md when <tool>
...
step (`.claude/skills/aid-discover/references/state-elicit.md` Step E2): use it to add a
...
Read `.claude/aid/templates/connectors/preset-catalog.md` (LLM-read from disk, same as ELICIT's
...
if [ -f ".aid/connectors/${STEM}.md" ]; then
  CLASS=UPDATE
  OLD_TYPE=$(bash .claude/aid/scripts/connectors/connector-registry.sh read "$STEM" connection_type --root .aid/connectors)
  OLD_AUTH=$(bash .claude/aid/scripts/connectors/connector-registry.sh read "$STEM" auth_method --root .aid/connectors)
else
  CLASS=ADD
  OLD_TYPE=""
  OLD_AUTH=""
...
Per [`.claude/aid/templates/connectors/reconcile.md`](../../aid/templates/connectors/reconcile.md)
...
(`.claude/aid/templates/kb-authoring/principles.md` P7 "Exception (connector sub-phase)"):
