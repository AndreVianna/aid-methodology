...
- Assign severity by lookup against the violated rule's anchor — see [`.cursor/aid/templates/grading-rubric.md#severity-scale`](.cursor/aid/templates/grading-rubric.md#severity-scale). **No invented severity bands.**
...
Your output is a single markdown file at `.aid/.temp/review-pending/<scope>.md` containing **exactly one markdown table** per the schema at `.cursor/aid/templates/reviewer-ledger-schema.md`.
