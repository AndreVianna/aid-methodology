  ...
  while IFS= read -r raw_line; do
    ...
    while IFS= read -r cite; do
      ...
      for prefix in "" ".aid/knowledge/" \
                    "canonical/" ".cursor/scripts/" \
                    ".cursor/scripts/kb/" ".cursor/scripts/execute/" \
                    ".cursor/scripts/summarize/" ".cursor/scripts/interview/" \
                    ".cursor/skills/" ".cursor/agents/" ".cursor/recipes/" \
                    ".cursor/templates/" ".cursor/templates/knowledge-base/" \
                    ".cursor/templates/knowledge-summary/" \
                    ".cursor/templates/kb-authoring/" ".cursor/templates/reports/" \
                    ".cursor/templates/specs/" ".cursor/templates/delivery-plans/" \
                    ".cursor/templates/feedback-artifacts/" \
...
# ---------------------------------------------------------------------------
# FRONTMATTER + KB AUTHORING LINT (.cursor/templates/kb-authoring/)
# ---------------------------------------------------------------------------
# Per .cursor/templates/kb-authoring/principles.md P4, the lint enforces
...
#   - Registered generated files (per .cursor/templates/generated-files.txt) exist
...
REGISTRY="$ROOT/.cursor/templates/generated-files.txt"
if [[ -f "$REGISTRY" ]]; then
