...
# resolves to .cursor/scripts/kb/preflight.sh (specific) before falling
# through to canonical/ (general).
# ---------------------------------------------------------------------------
PREFIXES=("" ".aid/knowledge/")

# .cursor/scripts/*/ — each subdir as a prefix
if [[ -d "$ROOT/canonical/scripts" ]]; then
  PREFIXES+=(".cursor/scripts/")
  while IFS= read -r d; do
    PREFIXES+=(".cursor/scripts/$(basename "$d")/")
  done < <(find "$ROOT/canonical/scripts" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort)
fi

# .cursor/templates/*/ — each subdir as a prefix + .cursor/templates/ itself
if [[ -d "$ROOT/canonical/templates" ]]; then
  PREFIXES+=(".cursor/templates/")
  while IFS= read -r d; do
    PREFIXES+=(".cursor/templates/$(basename "$d")/")
