...
while IFS=$'\t' read -r p gitex; do
    ...
    case "$p" in
        .cursor/skills/*/*)
            _rest="${p#.cursor/skills/}"
            node=".cursor/skills/${_rest%%/*}/"
            case "${_rest#*/}" in
                SKILL.md) printf '%s\tskill\n' "$node" >> "$W/convdir.tsv" ;;
            esac
            ;;
        .cursor/agents/*/*)
            _rest="${p#.cursor/agents/}"
            node=".cursor/agents/${_rest%%/*}/"
            case "${_rest#*/}" in
...
SHORTCUT_CATALOG=".cursor/aid/templates/shortcut-catalog.yml"
...
GENERATED_FILES=".cursor/aid/templates/generated-files.txt"
...
if [ -s "$W/scanfiles.txt" ]; then
    tr '\n' '\0' < "$W/scanfiles.txt" | xargs -0 awk \
        ...
        function kindof(target, matched, line, from,   fb) {
            ...
            if (from ~ /^canonical\/skills\// && target == ".cursor/aid/templates/shortcut-engine.md")
                return "convention"
        ...
        }
        {
            ...
            while (match(rest, /\{\{include:[A-Za-z0-9._-]+\}\}/)) {
                ...
                tgt = ".cursor/aid/templates/" nm ".md"
