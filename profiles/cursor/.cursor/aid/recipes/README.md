...
A recipe is a single Markdown file directly under `.cursor/aid/recipes/`. It has two
...
flat form defined in `.cursor/aid/templates/delivery-plans/task-template.md`
...
After the agent infers `workType`, it scans `.cursor/aid/recipes/` and reads each
...
1. Copy `.cursor/aid/templates/recipe-template.md` to `.cursor/aid/recipes/{name}.md`
...
7. After adding the file, re-run the work-002 generator to render the updated
   `.cursor/aid/recipes/` into all three install trees (claude-code, codex, cursor).
...
- **No recipe versioning.** Updating a recipe file updates everyone's behavior. If
  ...
  old one age out.
- **Flat layout.** Place recipes directly under `.cursor/aid/recipes/` — no
...
emerge. Project-local recipes live alongside the seed recipes in `.cursor/aid/recipes/`
