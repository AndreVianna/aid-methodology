...
- **One run, one work.** Every invocation allocates a brand-new `work-NNN`; the
  ...
  (`.cursor/aid/templates/state-machine-chaining.md` CHAIN), pausing only for
...
3. **FIX.** Dispatch `aid-architect` (Large; same Dispatch Protocol; the
   ...
   **Circuit breaker.** If the pass's grade has not improved across 3
   ...
   bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Artifact" --value ".aid/{work}/IMPEDIMENT-gate-{pass}.md"
   bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
   ```
