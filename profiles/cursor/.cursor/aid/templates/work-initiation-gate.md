...
(`.cursor/aid/templates/shortcut-engine.md`), `aid-describe`, and the
...
```bash
bash .cursor/aid/scripts/works/enumerate-works.sh
```
...
2. **Create and enter the worktree, BEFORE authoring anything:**
   ...
   the resolved path per `.cursor/aid/templates/worktree-lifecycle.md § Step
...
2. **Locate:**
   ...
   ```bash
   bash .cursor/aid/scripts/works/worktree-lifecycle.sh locate <work-id>
   ```
...
4. **Enter** (rungs 1-3 -- `registered | recreated | created`), unless
   ...
   `.cursor/aid/templates/worktree-lifecycle.md § Step 2`, invoke the
