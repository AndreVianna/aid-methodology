...
| `KB-02` | A `## Contents` index is present near the top, for a document with more than three sections | `authoring-conventions.md § KB Document Layout` | SHOULD | mechanical | `grep -c '^## ' <doc>`; if > 3, require a Contents heading | `[LOW]; escaped (>1 doc) → [MEDIUM]` |
