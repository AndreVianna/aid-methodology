...
A recipe is a single Markdown file directly under `.cursor/recipes/`. It has two
...
flat form defined in `.cursor/templates/delivery-plans/task-template.md`
...
After the agent infers `workType`, it scans `.cursor/recipes/` and reads each
...
1. Copy `.cursor/templates/recipe-template.md` to `.cursor/recipes/{name}.md`
...
7. After adding the file, re-run the work-002 generator to render the updated
   `.cursor/recipes/` into all three install trees (claude-code, codex, cursor).
...
- **No recipe versioning.** Updating a recipe file updates everyone's behavior. If
  ...
  old one age out.
- **Flat layout.** Place recipes directly under `.cursor/recipes/` — no
...
emerge. Project-local recipes live alongside the seed recipes in `.cursor/recipes/`
