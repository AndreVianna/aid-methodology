...
description: >
  ...
  question-set, prefilled from .cursor/aid/templates/connectors/preset-catalog.md when <tool>
...
step (`.cursor/skills/aid-discover/references/state-elicit.md` Step E2): use it to add a
...
Read `.cursor/aid/templates/connectors/preset-catalog.md` (LLM-read from disk, same as ELICIT's
...
if [ -f ".aid/connectors/${STEM}.md" ]; then
  CLASS=UPDATE
  OLD_TYPE=$(bash .cursor/aid/scripts/connectors/connector-registry.sh read "$STEM" connection_type --root .aid/connectors)
  OLD_AUTH=$(bash .cursor/aid/scripts/connectors/connector-registry.sh read "$STEM" auth_method --root .aid/connectors)
else
  CLASS=ADD
  OLD_TYPE=""
  OLD_AUTH=""
...
Per [`.cursor/aid/templates/connectors/reconcile.md`](../../aid/templates/connectors/reconcile.md)
...
(`.cursor/aid/templates/kb-authoring/principles.md` P7 "Exception (connector sub-phase)"):
