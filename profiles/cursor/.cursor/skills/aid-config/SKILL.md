...
```bash
bash .cursor/aid/scripts/config/lint-settings.sh --file .aid/settings.yml
```
...
```bash
bash .cursor/aid/scripts/config/read-setting.sh --key minimum_grade --default A
```
...
Canonical schema: `.cursor/aid/templates/settings.yml`. Flat top-level scalar keys
