...
- Copy `.cursor/aid/templates/settings.yml` → `.aid/settings.yml` verbatim.
...
Run:
```bash
bash .cursor/aid/scripts/config/read-setting.sh --path <key> --default '(unset)'
```
...
The canonical resolution helper is `.cursor/aid/scripts/config/read-setting.sh`. Consumer skills invoke:
...
Canonical schema: `.cursor/aid/templates/settings.yml`. Top-level sections + per-skill overrides as described in the tables above.
