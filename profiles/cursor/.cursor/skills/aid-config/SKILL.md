...
Canonical schema: `.cursor/templates/settings.yml`. Top-level sections:
...
A helper script `.cursor/scripts/config/read-setting.sh` (authored alongside
this skill) provides the canonical resolution logic. Consumer skills invoke:

```bash
bash .cursor/scripts/config/read-setting.sh --skill discover --key minimum_grade
