...
when the user types `/aid-describe escalate-from-recipe` or selects the escalate option at
...
type `/aid-describe escalate` in CONDENSED-INTAKE to trigger the full-path escalation).
...
User types the literal string `/aid-describe escalate-from-recipe` as a slot value.
...
If the user later types `/aid-describe escalate` in CONDENSED-INTAKE, the standard
