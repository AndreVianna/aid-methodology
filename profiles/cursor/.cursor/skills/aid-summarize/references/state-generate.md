...
Run `.cursor/aid/scripts/summarize/fetch-mermaid.sh`. It:
...
Use `.cursor/aid/templates/knowledge-summary/html-skeleton.html` as the seed when scaffolding
...
- Inlined CSS from `.cursor/aid/templates/knowledge-summary/component-css.css` (in `skeleton-head.html`)

Author each section file under `sections/` from KB content:
- Section structure follows `.cursor/aid/templates/knowledge-summary/section-templates/{profile}.md`
- Per-section content drawn from KB (cite source via relative `./xxx.md` link)
- 6–8 Mermaid diagrams using syntax patterns from `.cursor/aid/templates/knowledge-summary/mermaid-examples.md`

`post-mermaid.html` inlines `.cursor/aid/templates/knowledge-summary/lightbox.js` and
`.cursor/aid/templates/knowledge-summary/mermaid-init.js`. It is also the file that closes
...
Run `.cursor/aid/scripts/summarize/assemble.sh` to concatenate the multi-source layout into the
...
**Critical pitfalls — see `.cursor/aid/templates/knowledge-summary/mermaid-examples.md` for full list:**
