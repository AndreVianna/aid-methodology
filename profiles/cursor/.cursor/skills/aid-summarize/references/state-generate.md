...
else: MIN_GRADE = bash .cursor/scripts/config/read-setting.sh --skill summary --key minimum_grade --default A
```
...
Run `.cursor/scripts/summarize/fetch-mermaid.sh`. It:
...
- **Document purpose** — first read the YAML frontmatter `intent:` field (per
  `.cursor/templates/kb-authoring/frontmatter-schema.md`). The `intent:`
...
bash .cursor/scripts/summarize/assemble.sh --output .aid/dashboard/kb.html
