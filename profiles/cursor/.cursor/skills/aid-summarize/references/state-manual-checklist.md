...
Run `.cursor/aid/scripts/summarize/spot-check-facts.sh`. It extracts numeric/named claims from the HTML, greps the source KB, and writes `.aid/.temp/summarize/spot-check-facts.txt` (each line: `[OK|MISS] HTML-claim | KB-evidence`). Show the user the `MISS` lines, if any.
...
```
bash .cursor/aid/scripts/summarize/manual-checklist.sh \
...
```bash
bash .cursor/aid/scripts/review/check-gaps.sh --ledger .aid/.temp/review-pending/summarize.md
