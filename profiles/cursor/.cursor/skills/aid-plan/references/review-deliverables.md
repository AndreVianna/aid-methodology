...
Compare to minimum grade from `bash .cursor/aid/scripts/config/read-setting.sh --skill plan --key minimum_grade --default A`.

| Condition | Action |
|-----------|--------|
| Grade >= minimum | Ensure `deliveries/delivery-NNN/STATE.yml` exists for each delivery in PLAN.md; create any missing ones, seeding the top-level `delivery_state` scalar as `Pending-Spec` -- task-001/004. Delete ledger: `rm -f .aid/.temp/review-pending/plan.md`. Print summary, done. |
