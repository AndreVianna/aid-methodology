...
```bash
bash .cursor/aid/scripts/kb/build-project-index.sh \
  --root . \
...
Look up `.cursor/aid/templates/kb-authoring/domain-doc-matrix.md` for a row matching
...
2. **For a novel domain (no curated row, not decomposable into curated components):** research
   ...
   - map it to exactly one spine dimension (C0–C9, D, or meta) from
     `.cursor/aid/templates/kb-authoring/domain-doc-matrix.md` §"The dimension spine",
...
`.cursor/aid/templates/state-machine-chaining.md`). Re-run `/aid-discover` after confirming the
...
4. **Matrix lifecycle — optional candidate artifact (FR-40):**
   ...
   > Review and submit as a PR to .cursor/aid/templates/kb-authoring/domain-doc-matrix.md
...
bash .cursor/aid/scripts/kb/harvest-coined-terms.sh \
  ...
  --denylist .cursor/aid/scripts/kb/coined-term-denylist.txt \
  --top 60
```
...
```bash
bash .cursor/aid/scripts/kb/recon-classify.sh \
...
not aid retrieval (`.cursor/aid/templates/agent-dispatch-tiering.md`, Retrieval-heavy row).
...
(i.e., not synthesized from `.cursor/aid/templates/knowledge-base/*.md` by `synth_default_seed`).
...
5. Set the KB run-state frontmatter: `bash .cursor/aid/scripts/summarize/writeback-state.sh --set kb_status "In Progress" --set kb_grade Pending` (the `kb_status`/`kb_grade` scalars in `.aid/knowledge/STATE.md`'s leading YAML block — surgical frontmatter rewrite, markdown body untouched)
6. **Preserve** `## External Documentation` (unaffected by this step; the `**Minimum Grade:**` field lives in `.aid/settings.yml` — read it via `bash .cursor/aid/scripts/config/read-setting.sh --skill discover --key minimum_grade --default A`; the doc-set-level `**User Approved:**` header-blockquote line is a distinct, untouched concept — see `state-approval.md`)
