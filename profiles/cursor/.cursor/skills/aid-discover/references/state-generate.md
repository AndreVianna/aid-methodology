...
6. **Preserve** `**Project Type:**`, `**User Approved:**`, `## External Documentation` (the `**Minimum Grade:**` field is now in `.aid/settings.yml` — read it via `bash .cursor/aid/scripts/config/read-setting.sh --skill discover --key minimum_grade --default A`)
