...
deterministically from `.cursor/templates/knowledge-base/*.md` paired to the ownership map
...
synth_default_seed() {
  local tmpl_dir="${REPO:-$(pwd)}/.cursor/templates/knowledge-base"
...
raw="$(bash "$REPO/.cursor/scripts/config/read-setting.sh" \
        --path discovery.doc_set 2>/dev/null || true)"
