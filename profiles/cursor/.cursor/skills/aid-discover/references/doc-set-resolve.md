...
deterministically from `.cursor/aid/templates/knowledge-base/*.md` paired to the ownership map
...
Concern ids follow the model in `.cursor/aid/templates/kb-authoring/concern-model.md`.
...
synth_default_seed() {
  local tmpl_dir="${REPO:-$(pwd)}/.cursor/aid/templates/knowledge-base"
...
raw="$(bash "$REPO/.cursor/aid/scripts/config/read-setting.sh" \
        --path discovery.doc_set 2>/dev/null || true)"
...
`.cursor/aid/templates/kb-authoring/concern-model.md`) adds an adaptivity layer above
