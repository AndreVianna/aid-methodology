...
```bash
bash .cursor/aid/scripts/execute/writeback-state.sh --delivery-id NNN --lifecycle Gated
```
...
```bash
bash .cursor/aid/scripts/execute/writeback-state.sh --delivery-id NNN --lifecycle Blocked
bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field Lifecycle --value "Paused-Awaiting-Input"
bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field "Pause Reason" --value "Delivery gate blocked on non-CODE issues -- upstream fix required (SPEC/TASK/KB)"
...
bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Reason" --value "Delivery gate circuit breaker triggered -- grade not improving after 3 cycles"
bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field "Block Artifact" --value ".aid/works/{work}/IMPEDIMENT-delivery-{NNN}.md"
bash .cursor/aid/scripts/execute/writeback-state.sh --pipeline --field Updated --value "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
```
...
```bash
bash .cursor/aid/scripts/execute/writeback-state.sh --delivery-id NNN --lifecycle Done
```
