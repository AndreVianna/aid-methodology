...
Run `bash .cursor/scripts/config/read-setting.sh --skill execute --key minimum_grade --default A` to resolve the minimum grade for this skill (`.aid/settings.yml` is the source).
...
`.cursor/templates/dispatch-protocol-checklist.md`; read it before any subagent
...
- Grade ≥ minimum grade (from `bash .cursor/scripts/config/read-setting.sh --skill execute --key minimum_grade --default A`)
