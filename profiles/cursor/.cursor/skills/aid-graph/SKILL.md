...
Run `.cursor/aid/scripts/graph/graph-preflight.sh` before any state. It verifies
...
| `--reset` | Forces regeneration by **discarding the digest comparison** and nothing else: no artifact is deleted, and **no ledger is preserved**. Passed straight through to `.cursor/aid/scripts/graph/graph-stale-check.sh --reset`. |
| `--grade X` | Overrides the minimum acceptable grade **for this run only**. Format: `^[A-F][+-]?$`. Passed to `.cursor/aid/scripts/graph/grade-graph.sh --grade X`. **Nothing is persisted** — `.aid/settings.yml` is itself one of this run's staleness inputs, so writing a grade floor into it would force an unrelated regeneration on the next run. A durable floor is set through `/aid-config`, which owns that file. |
...
**iff `.cursor/aid/templates/knowledge-graph/graph-skeleton.html` is installed.** That
...
VALIDATE runs `.cursor/aid/scripts/graph/grade-graph.sh`, which invokes the reused leaf
