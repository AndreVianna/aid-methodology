...
| 4 | `Doc` | yes | Affected file path (relative to repo root). Examples: `foo.md`, `.cursor/scripts/bar.sh`, `tests/canonical/baz.sh`. For doc-wide issues with no specific file, use `—`. |
...
`.cursor/scripts/grade.sh` reads the ledger as a markdown table and counts findings by Severity, filtered to Status ∈ {`Pending`, `Recurred`}.
...
- `.cursor/scripts/grade.sh` — the grader that parses this ledger
- `.cursor/agents/reviewer/AGENT.md` and `.cursor/agents/discovery-reviewer/AGENT.md` — sub-agent output contracts (reference this schema)
- `.cursor/templates/reviewer-dispatch.md` — universal reviewer dispatch brief (references this schema)
