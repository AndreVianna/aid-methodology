---
name: {{recipe-name}}  # Fill in before saving as .cursor/recipes/{name}.md. These slots are placeholders, not real recipe slots.
...
<!-- Multi-task recipes add ### task-002 etc. with `**Depends on:** task-NNN` per task; see .cursor/recipes/add-crud-endpoint.md for a 3-task example. -->
...
as defined in .cursor/templates/delivery-plans/task-template.md.
...
File must live directly under .cursor/recipes/ (not in a subdirectory). Name the
