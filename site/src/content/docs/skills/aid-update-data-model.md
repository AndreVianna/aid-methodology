...
description: 'Direct-entry Lite-path shortcut (Update an existing data model/entity''s schema, with forward+rollback migration.) -- skips the aid-describe interview/triage.'
...
- **`description`** — Direct-entry Lite-path shortcut (Update an existing data model/entity's schema, with forward+rollback migration.) -- skips the aid-describe interview/triage. Binds VERB=`update` ARTIFACT=`data-model` and runs the shared shortcut engine, producing a fully-graded flattened Lite work that halts for approval. State machine: delegated to canonical/aid/templates/shortcut-engine.md (INTAKE -> CAPTURE -> SPEC -> PLAN -> DETAIL -> GATE -> APPROVAL-HALT).
...
flowchart TB
  ...
  n1(["aid-update-data-model<br/>Bind VERB=update, ARTIFACT=data-model"])
  n2{"INTAKE"}
  n3(["CONTINUATION"])
...
<a id="fragment-n1"></a>**1 · `aid-update-data-model`** — Bind VERB=update, ARTIFACT=data-model · _entry_

~~~~plaintext title="canonical/skills/aid-update-data-model/SKILL.md#L18" wrap
Bind **VERB=`update`**, **ARTIFACT=`data-model`**, then run the shared engine at `canonical/aid/templates/shortcut-engine.md`. The engine scaffolds the flattened Lite work (feature-001 structure), authors REQUIREMENTS -> SPEC -> PLAN + BLUEPRINT -> DETAIL tasks with reduced capture, runs the per-document Grading Gates (feature-004), and halts at the FR-10 approval gate. It never executes. This shortcut's `default_type`/`group` are its row in `canonical/aid/templates/shortcut-catalog.yml`.
~~~~

[Source: `canonical/skills/aid-update-data-model/SKILL.md#L18`](https://github.com/AndreVianna/aid-methodology/blob/master/canonical/skills/aid-update-data-model/SKILL.md#L18)
