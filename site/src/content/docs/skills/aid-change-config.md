---
title: 'aid-change-config'
description: 'Direct-entry Lite-path shortcut (Change an existing configuration option or feature flag.) -- skips the aid-describe interview/triage.'
generatedFrom: 'canonical/skills/aid-change-config/SKILL.md'
---

<!-- generated — do not edit; source: canonical/skills/aid-change-config/SKILL.md -->

## Frontmatter

- **`name`** — aid-change-config
- **`description`** — Direct-entry Lite-path shortcut (Change an existing configuration option or feature flag.) -- skips the aid-describe interview/triage. Binds VERB=`change` ARTIFACT=`config` and runs the shared shortcut engine, producing a fully-graded flattened Lite work that halts for approval. State machine: delegated to canonical/aid/templates/shortcut-engine.md (INTAKE -> CAPTURE -> SPEC -> PLAN -> DETAIL -> GATE -> APPROVAL-HALT).
- **`allowed-tools`** — Read, Glob, Grep, Bash, Write, Edit, Agent
- **`argument-hint`** — [description]  -- what to change; runs straight to a graded flattened Lite work

[Definition: `canonical/skills/aid-change-config/SKILL.md`](https://github.com/AndreVianna/aid-methodology/blob/master/canonical/skills/aid-change-config/SKILL.md)
...
flowchart TB
  ...
  n1(["aid-change-config<br/>Bind VERB=change, ARTIFACT=config"])
  n2{"INTAKE"}
  n3(["CONTINUATION"])
...
<a id="fragment-n1"></a>**1 · `aid-change-config`** — Bind VERB=change, ARTIFACT=config · _entry_

~~~~plaintext title="canonical/skills/aid-change-config/SKILL.md#L18" wrap
Bind **VERB=`change`**, **ARTIFACT=`config`**, then run the shared engine at `canonical/aid/templates/shortcut-engine.md`. The engine scaffolds the flattened Lite work (feature-001 structure), authors REQUIREMENTS -> SPEC -> PLAN + BLUEPRINT -> DETAIL tasks with reduced capture, runs the per-document Grading Gates (feature-004), and halts at the FR-10 approval gate. It never executes. This shortcut's `default_type`/`group`/`alias_of` are its row in `canonical/aid/templates/shortcut-catalog.yml`.
~~~~

[Source: `canonical/skills/aid-change-config/SKILL.md#L18`](https://github.com/AndreVianna/aid-methodology/blob/master/canonical/skills/aid-change-config/SKILL.md#L18)
