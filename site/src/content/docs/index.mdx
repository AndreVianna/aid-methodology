...
*Three entry points (plus `/aid-ask` for a plain question) · 111 skills — 17 curated pipeline, on-demand and
router skills (including `/aid-triage`), plus a 94-row shortcut catalog: 64 verb-first shortcut skills and 30
hand-authored `repurpose` skills (among them `/aid-ask`, a Q&A alias of `/aid-query-kb`). Name your change with
a **shortcut** and the shortcut engine collapses Describe through Detail into one autonomous run straight to
...
111 skills deliver AID: 17 curated skills — the pipeline phases above, the off-pipeline on-demand
skills that are not catalog rows (`/aid-housekeep`, `/aid-update-kb`, the connector and ticket
skills, `/aid-config`, `/aid-summarize`), and the `/aid-triage` suggest-only router — plus the
94-row shortcut catalog's skills: 64 verb-first shortcut
skills (`/aid-fix`, `/aid-create-api`, …) and 30 hand-authored `repurpose` skills.
