...
*Three entry points (plus `/aid-ask` for a plain question) · 76 skills — 18 curated pipeline/on-demand/router skills
(including `/aid-triage`), 24 hand-authored specialist skills (including `/aid-ask` for Q&A), and 34 verb-first
shortcut skills. Name your change with a **shortcut** and the
...
76 skills deliver AID: 18 curated skills — the pipeline phases above, the off-pipeline on-demand
skills that are not catalog rows (`/aid-housekeep`, `/aid-update-kb`, the connector and ticket
skills, `/aid-config`, `/aid-summarize`, `/aid-graph`), and the `/aid-triage` suggest-only router — plus the
58-row shortcut catalog's skills: 34 verb-first shortcut
skills (`/aid-fix`, `/aid-create-api`, …) and 24 hand-authored `repurpose` skills.
