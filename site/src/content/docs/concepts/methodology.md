...
*113 skill directories in total — 19 curated pipeline / on-demand / router skills plus the 94-row shortcut catalog's skills (58 canonical names + 36 aliases; 64 engine-generated verb-first shortcut doorways + 30 hand-authored `repurpose` skills). Unchanged: 10 agents, 14 KB doc types.*

**A. The 19 curated skills** — the pipeline phases plus the on-demand and router skills that are *not* in the shortcut catalog; their groups, phase numbers, and mandatory pipeline membership. (`aid-deploy`, `aid-monitor`, `aid-query-kb` and `aid-ask` are now catalog `repurpose` rows — see table D.)
...
canonical/  (single source of truth — never edit profiles/ directly)
  ├── skills/        (113 skill directories — 19 curated + 94 catalog skills)
  ├── agents/        (10 agents)
  └── aid/
