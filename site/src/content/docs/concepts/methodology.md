...
**A. The 17 curated skills** — the pipeline phases plus the on-demand and router skills that are *not* in the shortcut catalog; their groups, phase numbers, and mandatory pipeline membership. (`aid-deploy`, `aid-monitor`, `aid-query-kb` and `aid-ask` are now catalog `repurpose` rows — see table D.)
