...
<LinkCard
  title="All skills"
  description="Every AID skill, one card each — grouped by skill group and verb family."
  href="/skills/"
/>
...
then appends a `#### Technical Specification` subsection with design decisions and an
acceptance-criteria mapping.

**Artifact produced:** a `#### Technical Specification` subsection appended to that
feature's `### Feature NNN` section in `REQUIREMENTS.md` § 11.
...
A `### delivery-NNN` stanza in `PLAN.md` — the delivery definition (scope, gate criteria,
dependencies).

**Artifact produced:** `PLAN.md` — the delivery sequence and dependency graph — plus one
one `### delivery-NNN` stanza in `PLAN.md` per delivery.
