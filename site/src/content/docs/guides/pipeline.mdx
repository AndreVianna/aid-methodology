...
task gets a `task-NNN.md` file with type (IMPLEMENT / TEST / MIGRATE / CONFIGURE /
...
Not every project needs all six phases. A **lite path** — Interview → Specify → Execute
— is sufficient for small, well-understood features. The [Methodology](/concepts/methodology)
page explains when to use the lite path and when the full pipeline pays off.
