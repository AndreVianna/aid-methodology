...
flowchart LR
    ...
    I(["/aid-describe<br/>Phase 2a"])
    Def(["/aid-define<br/>Phase 2b"])
...
go directly from Configure to Describe (Phase 2a). The Deliver skills (deploy + monitor) are
optional and loop findings back into Describe.
...
The Describe skill gathers requirements conversationally. It asks you structured
questions about your goals, constraints, and scope, TRIAGE routes full or lite, and on the
full path writes `REQUIREMENTS.md`. On the lite path it emits a `SPEC.md` + tasks directly.
...
enhancement / question), and routes actionable items back into `/aid-describe` as new
...
Not every project needs all six phases. For small, well-scoped work, `/aid-describe`'s
TRIAGE routes down the **lite path** — Describe → Execute, **skipping Specify, Plan, and
