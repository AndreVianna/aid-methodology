...
| **VALIDATE** | Confirm canonical completeness — the full skill and agent set (113 skills, 10 agents) and non-empty templates. |
| **RENDER** | Run `run_generator.py`, which renders every profile from canonical via the `render.py` core, writes each emission manifest, and runs the deletion pass. |
...
<Aside type="tip">
**Always regenerate via `run_generator.py`.** It is the single entry point: it renders
every profile through the `render.py` core, updates each emission manifest, and runs the
deletion pass atomically. Calling the lower-level `render.py` directly (outside its
`--self-test`) would leave the manifests stale and fail the render-drift CI check.
</Aside>
