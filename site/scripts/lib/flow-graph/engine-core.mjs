...
function _buildEngineCore(override = {}) {
  ...
  // Applied to INTAKE (CONTINUATION) and GATE.
  ...
  //
  // GATE currently yields NOTHING, and that is correct rather than broken: work-003
  // delivery-012 moved the fix loop and the circuit breaker out of this template into
  // `/aid-deep-review`, which the GATE section now names as their owner in as many words.
  // The rule stays keyed on GATE rather than being deleted — it is generic (any bolded arm
  // whose context carries a B1 trigger), so a breaker returning to this file charts again
  // with no code change. flow-engine-core.test.mjs asserts BOTH halves: the node's absence
  // here, and the delegation sentence that explains it — so a breaker that vanished by
  // accident cannot pass as one that moved on purpose.
  //
