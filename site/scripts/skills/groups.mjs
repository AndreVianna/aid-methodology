...
export const CURATED_GROUPS = [
  ...
  },
  {
    ...
    blurb: 'Skills for executing detailed tasks and reviewing their output, each through a graded adversarial review loop.',
    // work-003 extracts review into two chainable skills. They belong here, not in Support: they are
    // chain-called from a task's review step, not invoked to set the workspace up. Omitting them is not
    // a cosmetic gap -- `assignGroups` THROWS on a skill that is in neither this table nor the shortcut
    // catalog ("unassignable skill"), which fails `gen-skills.mjs` outright and took 104 site tests
    // with it at the work-003/master merge.
    members: ['aid-execute', 'aid-light-review', 'aid-deep-review'],
  },
];
