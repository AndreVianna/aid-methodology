...
describe('AC-5 — composed node spine (doorway + 8 engine nodes)', () => {
  // Was 10 (doorway + 9). work-003 delivery-012 moved the GATE fix loop and circuit breaker
  // into /aid-deep-review, so the engine spine is 8 nodes and the composed chart is 9.
  // Non-vacuity: we must have exactly 9 nodes.
  it('chart has exactly 9 nodes (1 doorway + 8 engine)', () => {
    expect(createApiChart.nodes.length).toBe(9);
  });
  ...
  const expectedNames = [
    ...
    'DETAIL',          // n7
    'GATE',            // n8
    'APPROVAL-HALT',   // n9 -- no B1 sibling: the breaker moved to /aid-deep-review
  ];
  ...
  it('node ids are n1…n9 (offset=1 gives n2…n9 for engine, n1 for doorway)', () => {
    const ids = createApiChart.nodes
      .slice()
      .sort((a, b) => a.order - b.order)
      .map((n) => n.id);
    expect(ids).toEqual(['n1', 'n2', 'n3', 'n4', 'n5', 'n6', 'n7', 'n8', 'n9']);
  });
...
describe('AC-6 — validateChart passes', () => {
  ...
  it('chart.exits has no "Circuit breaker" -- it is charted where it now lives', () => {
    // The composed chart mirrors the engine: with no B1 under GATE there is no third exit.
    // The positive half of this claim (the GATE section delegates the breaker, and
    // /aid-deep-review still carries it) is asserted in flow-engine-core.test.mjs.
    expect(createApiChart.nodes.find((n) => n.name === 'Circuit breaker')).toBeUndefined();
    expect(createApiChart.exits).toHaveLength(2);
  });
});
...
describe('Purity — extractEngineDoorway does not mutate the engine core', () => {
  ...
  it('calling extractEngineDoorway on a different skill still yields 9 nodes', () => {
    ...
    expect(fixChart.nodes.length).toBe(9);
  });
});
