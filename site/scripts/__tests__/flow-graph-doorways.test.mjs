...
describe('engine core — the eight-node spine, in order', () => {
  ...
  it('is the seven State Machine rows with each B1 node after its parent', () => {
    // One B1 node, not two: work-003 delivery-012 moved the GATE circuit breaker into
    // /aid-deep-review along with the fix loop it guards, so only INTAKE still has one.
  ...
  it('each B1 node sits immediately after its parent, not merely somewhere later', () => {
    ...
    // GATE's B1 slot is empty, so its successor is the declared advance target.
    expect(names[names.indexOf('GATE') + 1]).toBe('APPROVAL-HALT');
  });
...
describe('AC-4 fixture 1 — aid-create-api: the loop is delegated, so the chart has none', () => {
  const chart = liveChart('aid-create-api');
  const selfEdges = chart.edges.filter((e) => e.from === e.to);

  it('no self-edge anywhere in a composed doorway chart', () => {
    // Was: exactly one, on GATE, kind loop-back, provenance shortcut-engine.md. work-003
    // delivery-012 moved the fix loop into /aid-deep-review, and the composed chart mirrors
    // the engine core it splices.
    expect(selfEdges).toHaveLength(0);
  });

  it('GATE still advances, so the absence is a delegated loop and not a dropped edge', () => {
    // Non-vacuity: a chart that failed to build, or lost GATE entirely, would also report
    // zero self-edges. This pins that GATE is present and connected.
    const gateNode = chart.nodes.find((n) => n.name === 'GATE');
    expect(gateNode).toBeTruthy();
    expect(chart.edges.filter((e) => e.from === gateNode.id)).toHaveLength(1);
  });
});
...
describe('AC-4 fixture 1 — aid-create-api: GATE (1 outgoing edge)', () => {
  const chart = liveChart('aid-create-api');
  const gateNode = chart.nodes.find((n) => n.name === 'GATE');
  const gateEdges = chart.edges.filter((e) => e.from === gateNode.id);

  it('GATE node kind is step, not decision', () => {
    // A decision node is one that branches. With the breaker gone GATE has a single
    // successor, so charting it as a decision would draw a diamond with one way out.
    expect(gateNode.kind).toBe('step');
  });

  it('GATE has exactly 1 outgoing edge (non-vacuity bound)', () => {
    // Was 3: 2 branch + 1 loop-back, when the circuit breaker and the fix loop lived here.
    expect(gateEdges).toHaveLength(1);
  });

  it('that edge goes to APPROVAL-HALT as a plain sequence with no condition', () => {
    const ahNode = chart.nodes.find((n) => n.name === 'APPROVAL-HALT');
    expect(gateEdges[0].to).toBe(ahNode.id);
    expect(gateEdges[0].kind).toBe('sequence');
    expect(gateEdges[0].condition).toBeNull();
  });

  it('no Circuit breaker node exists to branch to', () => {
    expect(chart.nodes.find((n) => n.name === 'Circuit breaker')).toBeUndefined();
  });
});

describe('AC-4 fixture 1 — aid-create-api: exits (APPROVAL-HALT, CONTINUATION)', () => {
  const chart = liveChart('aid-create-api');
  ...
  it('exits has exactly those two entries', () => {
    // Bounding the length is what stops the removed third exit from being a free deletion:
    // without it, this block would pass on an exits array that had gained an unrelated node.
    expect(chart.exits).toHaveLength(2);
  });
...
describe('AC-4 fixture 1 — aid-create-api: the spine (node order)', () => {
  ...
  it('chart has exactly 9 nodes (1 doorway + 8 engine segment)', () => {
...
describe('AC-4 fixture 1 (lighter) — aid-fix: bare-verb binding and engine spine', () => {
  ...
  it('eight engine-segment node names match the expected engine spine', () => {
    // Non-vacuity: the slice must have 8 nodes. Was 9, with 'Circuit breaker' between GATE
    // and APPROVAL-HALT, until work-003 delivery-012 moved it to /aid-deep-review.
    const ENGINE_SPINE = [
      'INTAKE', 'CONTINUATION', 'CAPTURE', 'SPEC', 'PLAN',
      'DETAIL', 'GATE', 'APPROVAL-HALT',
    ];
