...
## Per-Delivery Execution Graph

> Task-level dependency graph + parallelism for each delivery (generated by /aid-detail).
> The strategic `## Execution Graph` above is the feature-level view; this is the
> machine-readable task-level view consumed by /aid-execute and the dashboard reader.
> bash IMPLEMENT lands first in each feature lane; the ps1 parity twin depends on it.
...
| task-005 | task-001 |
| task-006 | task-002, task-005 |
| task-007 | task-001, task-002, task-005 |
| task-008 | task-003, task-007 |
| task-009 | task-005, task-006, task-007 |
