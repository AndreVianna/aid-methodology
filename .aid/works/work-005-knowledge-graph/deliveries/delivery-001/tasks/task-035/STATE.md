...
review: "--"
elapsed: "--"
notes: "Hub SHIPPED and browser-verified (3d67b8f4, 7ebe14ba, 51c4842b, a3dfd128). CLOSED since: (1) --gk-project added to GRAPH_KIND_TOKENS + PV13a/PV13b + both fixture CSS themes, so the contrast gate covers the star colour (W5-17 filed then WITHDRAWN); (2) hub test coverage landed -- GT15/GV25 assert sixteen LENS_KEYS, new GT71-GT76 cover hubEdges/HUB_ID/depth-from-hub, and GV27 was de-vacuified in passing; (3) profiles/** + dogfood trees re-rendered and resynced, render-drift green. ALSO DONE in this session, beyond the hub: package.json moved out of canonical/ to the repo root (both offenders) with dependabot repointed; the visual-fidelity CI job DELETED on the owner's decision after it was found reporting green on a path eliminated four weeks earlier; S5 guards in two graph suites made mode-blind (blob hashes, not git diff); test-multitool-isolation T21-T26 given a per-line non-invocable-mention filter in place of a file allowlist. STILL OPEN: (4) owner has a second finding to raise about the SELECTED NODE panel, deferred; (5) W5-18 (four self-invalidating `git show HEAD:` assertions) filed, not started; (6) jsdom is not a repo devDependency so the canvas suite runs ~6 of ~53 assertions on CI -- needs a decision; (7) coverage-parity baseline is pre-work-005 (6c7dd7d5) and needs a re-bootstrap, not accept-list rows (not a required check)."
ticket_ref: "--"
---
