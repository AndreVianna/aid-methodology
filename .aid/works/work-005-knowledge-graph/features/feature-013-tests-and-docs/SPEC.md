...
| `architecture.md` § *Knowledge Base Maintenance group*, `:214–:216` | One slot for `/aid-graph` in that group's on-demand list, in the shape the line already uses — a name plus a parenthesised role, alongside `aid-housekeep`, `aid-ask`, `aid-update-kb` and `aid-summarize`. D1 class 1's *member-of-a-skill-list* shape, so it is the same edit as `docs/aid-methodology.md:408`, not a new kind. **Its closing `CONFIRMED in docs/aid-methodology.md "Skill Inventory"` (`:217`) is falsified by this feature's own table-A and `:408` edits**, so the citation is re-verified against the edited sibling in the same commit rather than carried — the pathology this SPEC diagnoses for `quality-gates.md`, where a stated verification suppressed the scrutiny that would have caught it | Always |
| `pipeline-contracts.md` § the off-pipeline sentence, `:94–:96` | `/aid-graph` added to the off-pipeline-or-optional list beside `aid-summarize`, `aid-deploy`, `aid-monitor`, `aid-housekeep`, `aid-ask` and `aid-update-kb`, since FR-7 makes it on-demand and off the numbered pipeline. Same falsified-citation condition as the row above — `CONFIRMED: docs/aid-methodology.md` at `:98–:99` | Always |
