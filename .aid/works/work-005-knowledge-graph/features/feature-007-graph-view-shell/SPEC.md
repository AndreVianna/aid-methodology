...
| 2026-07-30 | **SCOPED FREEZE EXCEPTION — this A+ SPEC is reopened for exactly four seams and nothing else** (STATE.md Q26 § Freeze; owner decision "freeze exception 1", widened the same day to a fourth seam). Every one was found by a downstream feature attempting to implement a clause, and all four are one class: *this SPEC named an obligation and left unstated a rule a consumer must have.* **(1) The density exemption is withdrawn in D10's favour, resolving a self-contradiction.** D1a routed "the density control never hides a gap" to a **renderer-side exemption**, which its own contract makes unimplementable — thinning is `project()`'s (D3), the canvas draws `visibleNodes` (D4), consumer rule 1 forbids a renderer deciding membership from `LensState`, the lens is interpreted exactly once (D3), so an exemption would have to **re-add nodes `project()` removed** — while D10's density row already decided it the other way ("thinned at `2`–`5` like any node below the threshold") and D4's `visibleNodes` row says these records are "thinned … exactly like any other node". Two clauses to one, and the one is also the only implementable reading: the exemption is gone and D10 now carries the derivation, including the part that makes the loss nil — **the gap *set* is never thinned**, `coverageGaps` being the union `R ∪ G` computed once per load rather than per lens, so AC-15 binds unconditionally on the set and is drawn in full at the Coverage preset's own `density: 1`; above it a gap **mark** thins with everything else. **GV07** gains that pair of clauses. **(2) The two system preferences gain a carrier.** `prefers-reduced-motion` and `forced-colors` appear in no `LensState` and no `ViewModel` field, `project()` is pure with no DOM, and the store exposed no getter — yet D5d and § Accessibility assign detection *and exposure* here, and **NFR-4 is a requirement**, so a consumer honouring it had no sanctioned route and feature-008 read `matchMedia` itself. A `ViewModel` field is refused (a preference is not a function of the lens and `project()` could neither read nor derive it): the **shell** detects, `createStore` takes the pair, and the store publishes `getPreferences()` / `setPreferences()` / `subscribePreferences()` — one route, both renderings, `project()` untouched and still pure, the DOM read left where every other one is. **GV27** asserts it. **(3) The shell↔canvas viewport interface is named.** `CONTROL_MANIFEST` was **already satisfied** — D8 authors zoom/pan's keyboard equivalents with `requirement: NFR-6` and GV17/GV24 bind whatever it authors — so no manifest redesign was owed; the gap was the **value**, the step factor being the canvas's and a fit a function of an extent living only in canvas-private state. `mountCanvas` now returns a **viewport handle**, `{viewportFor(action)}` over the seven actions, whose result the entry's own handler writes through `setLens({zoom})`; with no handle the entries stay present, focusable and never `disabled` and write nothing, `zoom` being graph-only with nothing drawn to transform — the one state GV17 and GV24 now scope for rather than assert through. **(4) `nodeEmphasis`'s composition is stated as a total precedence, `'focus'` first** *(owner ruling)*. `:580` asserted that lens emphasis and selection "compose in `nodeEmphasis`" and never said how, while the map holds **one** class per id — so with Coverage active a selected gap node qualified twice, and under the other order **no node carried `'focus'` at all**: feature-009's reveal never fires, its `selected` badge — **its own AC-S5**'s text carrier — never renders, and **its own AC-S8**'s antecedent goes vacuous so `TV18` passes green. Now: focus, then the gap class, then `'dimmed'`, then `'normal'`, total over all five values; nothing is lost because `coverageGaps` publishes both gap sets independently and at load. **The edge axis is settled in the same clause and needs no precedence — with the derivation, not an assurance:** `edgeEmphasis` occurs at exactly two lines here, so **D6f is the only clause assigning any edge class**, and selection assigns none (D7a's dimming composes into `nodeEmphasis`), so Provenance-with-a-live-selection — reachable because D6a's Provenance patch does not clear `focus.nodeId` the way Coverage's and Overview's do — still gives each key one value from one clause. **GV28** asserts both axes. **Nothing else is touched**: no other wording, no stale count, no citation, no structure; `GV01`–`GV26` keep their numbers and concerns, `LensState` stays at fourteen fields and `ViewModel` gains none, and four inbound items are answered in place (feature-008's Open Items 1–3, feature-009's Open Item 5) with both SPECs owing a re-sweep and neither edited from here (Open Item 13). Delta **+25 lines** | Owner decision (STATE.md Q26 § Freeze) |
...
- `degree` and `degreeByKind` (a `Map<Kind, integer>`) are computed in one pass over `edges`. `degree`
  ...
  degree-based — so the two mechanisms share no counter. **No renderer exempts a gap node from thinning:**
  thinning is `project()`'s (D3), and **D10's density row** decides the case and states what does hold.
...
| `emphasis` | `'none' \| 'coverage' \| 'provenance-chain'` | The only field that drives **lens-level** dimming and highlighting. Selection dims and highlights too, through `focus.nodeId` (D7a), and the two compose in `nodeEmphasis` **by the total precedence D4 states, `'focus'` first** — so this is the only *lens* channel, not the only source of a class | both |
...
**The two system preferences live on the store, in neither record.** `prefers-reduced-motion` and
`forced-colors` are not lens state and not functions of the lens, and `project()` is pure with no DOM (D4,
rule 2), so a field on either record would make a preference a product of a projection it is not. **The
shell detects** each at load and on its `change` event (D5d) and passes the pair into `createStore`, which
publishes it (§ API Contracts): **one route both renderings read**, the DOM read staying in the shell so
the store still projects headless, and a flip re-projecting nothing — no `revision`, no `subscribe`
notification, no membership or emphasis decided (**GV27**), and no renderer querying a media feature.
...
| `nodeEmphasis` | `Map<id, 'normal' \| 'dimmed' \| 'kb-unbacked' \| 'artifact-undocumented' \| 'focus'>` | Per-node emphasis class, keyed over **`visibleNodes` and nothing else**, so no class — `'focus'` least of all — can land on a node neither rendering may draw. `'int-undocumented'` is **renamed `'artifact-undocumented'`**, because the class is `Kind = source-artifact` and the old name was the prefix-keyed proxy AC-15's re-key removed. **One class per node while two inputs assign classes, so the composition is a total precedence and `'focus'` takes it** *(owner decision 2026-07-30, on a silence feature-009's gate found)*: **(1)** the fold-resolved `focus.nodeId` (D6c clause 2) is `'focus'`, whatever else applies to it; **(2)** else, under `emphasis: 'coverage'`, its gap class — `'kb-unbacked'` and `'artifact-undocumented'` are keyed on **disjoint kinds** (D6d), so those two cannot contend and no order between them is stated; **(3)** else `'dimmed'`, where the active lens or the selection dims it (D6d, D6f, D7a); **(4)** else `'normal'`. Total over all five values, so no id is ever unclassed and none is classed twice. **Ranking `'focus'` first loses no fact**, because both are published independently: `coverageGaps` carries the two gap sets as their own field rather than through this map — its `artifactUndocumented` half computed once per load (D10) — and `focus.nodeId` is on `LensState`, so a surface wanting both draws the gap marker from `coverageGaps` and the focus treatment from here — which is what feature-009's `selected` badge and its reveal need, and the route a canvas gap badge must take for AC-15 to hold under a live selection, the opposite order having left no marked id at all. **GV28** asserts all four steps |
| `edgeEmphasis` | `Map<key, 'normal' \| 'dimmed' \| 'chain'>` | Per-edge emphasis class, keyed over the `visibleEdges` rows `edgeFold` does **not** mark `'collapsed'` — the drawn set, for the same reason `nodeEmphasis` is keyed over `visibleNodes`. **The edge axis needs no precedence, and the derivation is given rather than the assurance:** **D6f is the only clause in this SPEC assigning any edge class**, and it marks the chain rows `'chain'` and dims the rest, so every drawn row under `'provenance-chain'` is one or the other; under `'coverage'` (node classes only, D6d) and `'none'` (no lens-level dimming, D3) every drawn row is `'normal'`. Selection assigns **no** edge class — D7a's dimming composes into `nodeEmphasis`, which is what **D3's `emphasis` row** says — so Provenance **with a live selection**, which D6a's patch does not clear the way Coverage's and Overview's do, still gives each key exactly one value from exactly one clause. Two classes cannot co-occur on one key, so there is nothing to rank; the map stays total over the drawn rows by exhaustion. **GV28** asserts that exhaustion |
...
was never the sole carrier. **Detection and exposure are this feature's, over D3's store route**; the
drawing is feature-008's.
...
**The zoom/pan entries are authored above; what was missing is one value.** The step factor and a fit's extent
are canvas-private layout state that never crosses this seam, so `mountCanvas` returns a **viewport handle** —
`{viewportFor(action)}` over the seven action tokens (zoom in, zoom out, fit, one pan step in each of four
directions), each returning a `{scale, panX, panY}` — or nothing where the module is absent or not ready
(§ Feature Flow step 6). The entry's own handler writes its result through `setLens({zoom})`: one writer on
this path, the effect assertable at the shell (assertion 3, **GV17**), and the factor and the extent left
wholly to **feature-008**. **With no handle the entries stay present, focusable and never `disabled`** —
assertion 2's question and AC-21's — and write nothing, `zoom` being graph-only (D3) with nothing to move.
...
| `density` | shown at level `1`, thinned at `2`–`5` like any node below the threshold | Level 1 thins nothing, and the Coverage preset sets `density: 1`, so a zero-row node is visible in the lens that exists to surface it by two independent routes. Above level 1 the reader has explicitly asked for less. **This row is the whole rule, and D1a's routed renderer-side exemption is withdrawn against it** *(2026-07-30, owner decision)*: an exemption from thinning is a **membership** decision, `project()` makes every one of those exactly once (D3), consumer rule 1 forbids a renderer making one from `LensState`, and a canvas-side exemption would have to re-add nodes `project()` had removed — so no consumer may implement one, and D4's `visibleNodes` row already says these records are "thinned … exactly like any other node". What is never thinned is the gap **set**, not the mark: `coverageGaps.artifactUndocumented` is the union `R ∪ G` computed **once per load rather than per lens application** (above), keyed over neither `visibleNodes` nor `LensState`, so a thinned gap node keeps its id in the list the lens, the coverage panel and the ledger comparison all read. AC-15's equality therefore binds unconditionally on the set and is *drawn* in full at the Coverage preset's own `density: 1`; above it a gap **mark** thins with every other node while the list is unchanged. **GV07** asserts both halves |
...
4. **Create the store.** `createStore(graphModel, INITIAL_LENS, preferences)` computes the first
   `ViewModel` and holds `{graphModel, lensState, viewModel, preferences}` — the pair **detected here, by
   the shell**, defaulting to both `false`, and never projected (D3).
...
6. **Mount the renderings.** `graph-table.js` (feature-009) and `graph-canvas.js` (feature-008) each
   ...
   feature-002's Stage-1 probe lands negative. `mountCanvas` returns D8's **viewport handle** or none.
...
createStore(graphModel, initialLens, preferences)   // -> Store  (D3's pair; omitted => both false)
// Store:
//   getViewModel()        -> ViewModel   (current projection)
//   getLens()             -> LensState   (frozen copy; mutating it does nothing)
//   getPreferences() -> {reducedMotion, forcedColours}; setPreferences(patch); subscribePreferences(fn) (D3)
...
| Reduced motion | The reused `@media (prefers-reduced-motion: reduce)` block; the shell **detects the preference and exposes it on the store** — `getPreferences()` / `subscribePreferences()`, the one route (D3, GV27) — and the settled-layout obligation is feature-008's (NFR-4, AC-9) |
| Forced colours | The reused `@media (forced-colors: active)` block for the page; the preference reaches the canvas by that same store route (D3), which drops colour entirely and relies on shape, line style and filtering (D5d) |
...
| **GV07** | a zero-row `kb_gaps` entry produces a complete `Node` record carrying the entry's `name`, `kind === 'source-artifact'`, `degree === 0`, `coverageOrigin === 'ledger-only'`, **no** mismatch alarm, presence at `density: 1`, **absence from `visibleNodes` and from `nodeEmphasis` at `density: 5` with its id still present in `coverageGaps.artifactUndocumented` and in `coverageOrigin`** — D10's density row in both halves, so an implementation exempting it from thinning fails and one dropping it from the gap set fails too — membership of the `no relationships` group under the two edge-derived grouping dimensions, its **own single-node group** under `grouping: 'document'` with `foldable === 0` and absence from `foldedInto`, and a `nodeLabels` entry ending `— no recorded relationships` | D10 |
...
| **GV17** | manifest↔DOM bijection over `data-control` attributes in a generated `graph.html`, every matched element focusable; then each is driven by keyboard input only and its `LensState` effect asserted — for the seven viewport entries, with a **viewport handle** registered (D8), presence and focusability being asserted with and without one and `zoom` unchanged where there is none; **and every `data-group-toggle` element is focusable and keyboard-operable** (its completeness is GV22's). Asserted at both gate widths, so the mobile `<details>` collapse of the control panel cannot drop either kind | **AC-21**, D8, NFR-6 |
...
| **GV27** | the two preferences are reachable and are **not** projected: `getPreferences()` returns both keys, each value following a `prefers-reduced-motion` and a `forced-colors` emulation set **true and false in turn** — so a constant answer fails one way or the other — and a `subscribePreferences` listener receives the new pair on a mid-session flip of each with no reload, while that flip leaves `revision` unchanged and notifies **no** `subscribe` listener; the same `LensState` projects **non-empty and identical** `visibleNodes`, `nodeEmphasis`, `nodeEncoding` and `counts` under both values of both preferences, so neither reaches a `ViewModel` field and an implementation folding one into the projection fails; and `createStore` at its default pair with no DOM still projects, which is what keeps every lens assertion above headless | D3, D5d, **NFR-4** |
| **GV28** | the emphasis composition is D4's precedence, over a fixture carrying a `document` in `kb-unbacked`, a `source-artifact` in `artifact-undocumented`, a dimmed remainder and rows of a chain and a non-chain category: with `emphasis: 'coverage'` and no selection each gap id carries its **own** gap class; selecting one of them gives **that id and no other** `'focus'` while its id stays in the same `coverageGaps` list and every other gap id keeps its class; the same select under `'provenance-chain'` and under `'none'` likewise yields `'focus'`, so the precedence is asserted where a coverage class, a `'dimmed'` and a `'focus'` all apply and not only where one does; every `visibleNodes` id has exactly one entry and every entry is one of the five values. On the edge axis, with a **live `focus.nodeId`** in each projection: every drawn row is `'chain'` or `'dimmed'` under `'provenance-chain'` and `'normal'` under the other two, so no selection invents an edge class and no drawn row is unclassed | D4, D6d, D6f, D7a |
...
13. **AC-6a and NFR-4's settled render are feature-008's, and NFR-8's warning is feature-010's.** Named
    ...
    **Owners: feature-008, feature-010.** *(Extended 2026-07-30, scoped freeze exception: D10's density row,
    D3's preference route, D8's viewport handle and D4's precedence answer feature-008's Open Items 1, 2 and
    3 and feature-009's Open Item 5 in place; **both owe a re-sweep**, and neither is edited here.)*
