...
| 2026-08-05 | **FIX PASS — the work is AT THE B- FLOOR: zero blocking rows across all 22 ledgers (was 3)** | feature-012 + feature-013 SPECs | Owner directed advancing the work, so I inventoried the ledgers rather than assuming: **60 open rows across 7 ledgers, but only 3 above the B- floor** — feature-012 row 17 `[CRITICAL]` + row 18 `[MEDIUM]`, and feature-013 row 17 `[HIGH]`. feature-008's apparent 2 MEDIUM were my own miscount (a severity grep matching Description text, not the Severity cell); it was already clean. **Row 17 (CRITICAL):** the Section Description at `:85-86` said a new skill directory moves **one** derived quantity while D4 `:392` said **more than one** — and the Description was the false one, which matters because it is the premise the whole value-keyed needle sweep rests on. Corrected to D4's wording, with **no numerals** (Q19), because the merge has since moved every figure the row's evidence quoted. Ran the row's OWN oracle rather than a fix-pass vocabulary: `moves one derived` now returns **0**, and the surviving `exactly one` hits are a different subject (an emission record at `:359`, a per-directory sidecar at `:659`) plus the dated Change Log, which is correct to leave. **Row 18 (MEDIUM):** the second `--list` snapshot was unpinned — "again after" plainly reads as after Flow step 1, which authors `canonical/` only, so the needle set collapses from four fields to one and CR08 is then satisfied vacuously. Pinned to **after the L1 roster edit lands in Flow step 9**, in both the D4 text and Flow step 9 itself, since the row's point was that no step placed it. **feature-013 row 17 (HIGH):** two KB docs carry an `aid-summarize` roster slot in exactly D1's slot shape and neither was named anywhere in the SPEC — grep for both filenames returned **0** — making AC-T1's "**every** hand-authored surface carrying a roster slot" unsatisfiable. **I validated the premise against disk first, and the merge had rewritten the quoted text:** the row quotes `aid-query-kb … aid-ask is its friendly-named alias`, and work-004's alias retirement means disk now reads a plain `aid-ask`. The slots themselves survive at `architecture.md:214-216` and `pipeline-contracts.md:94-96`, so the finding stands, but the fix had to be written against current bytes rather than the row's quote. Added both as D3 rows, and verified first-hand that both lines really do close with `CONFIRMED in docs/aid-methodology.md "Skill Inventory"` — they do (`:217`, `:98-99`) — so each row now also carries the re-verification obligation, this feature's own table-A and `:408` edits being what falsifies them. **Ledger rows flipped to Fixed; blocking count re-counted across all 22 ledgers: 0.** |
| 2026-08-05 | **The four agent-built suites, verified by me after stopping both builders at the owner's 2.5-hour mark** | -- | Owner called the elapsed time on two builders. I stopped both rather than let them continue, because the artifacts were already on disk: four suites, **4,938 lines**, all parsing clean and all already carrying the `# COVERS:` manifests and declared subject-invocation counts I had asked for mid-flight. **Verified so far, my runs not their reports: `test-graph-runtime-digest.sh` 45/0 in 229 s, `test-graph-runtime-gate.sh` 73/0 in 318 s** (extraction and runtime still measuring). **A wrong inference I made and corrected within the same turn:** a log line reading "15 subject invocations: 2 baseline, 9 matrix arms, 3 negatives, 1 closing revert" led me to say the mutation matrix was running in the default path, violating S3. It is not — `MODE="assert"` is the default, gated at `:274`/`:467`, the default run reported **0** mutants, and `:54` records the matrix was deliberately moved from inline to the flag. Both builders complied with S1-S5 and T1-T6; the 4-5 s per assertion is genuine work against per-check broken fixtures, declared and justified as S4 requires ("the flag set is per-script, so they cannot be shared"). **My own instrumentation was the thing at fault twice here:** the pass/fail counter missed `Tests passed:  45` because their summary uses two spaces, and an earlier severity count read Description text as a Severity cell. Both were my greps, not their output. |
| 2026-08-05 | **T1-T6 — the write-first / run-at-milestone test protocol (owner-directed), and how it is ENFORCED rather than merely asked for** | -- | Owner's question: how do we instruct agents to write the tests first but run them at the end of each milestone instead of after every change? **The protocol, binding on every dispatch brief from here: T1** the suite is authored BEFORE or ALONGSIDE the mechanism and committed with it — never bolted on afterwards. **T2** between edits, verification is **spawn-free**: `bash -n` / `node --check` for parse, plus a static grep for the invariant just changed. A suite run is NOT a per-edit check. **T3** ONE focused run per milestone (a feature, or one coherent mechanism): `select-suites.sh --run`, which runs only what the change set selects. **T4** ONE full run per deliverable: `select-suites.sh --all --run`, plus `--self-mutate` on the suites that carry a mutation matrix. **T5** the brief DECLARES a ceiling on full-suite runs (default 3) and the final report states the ACTUAL count per suite. **T6** debug at the smallest granularity — re-run the FAILING ASSERTION GROUP, never the whole suite; this makes a group filter a design requirement on every suite, not a nicety. **Why T5 is real and not an honour system: the transcript is auditable and I have already audited it** — I recovered exact per-suite invocation counts from the agents' own tool calls (runtime 33 own-suite runs + 19 runs of five suites it does not own; extraction 8 + 5 justified). An agent that overruns its declared budget is therefore caught by evidence, not by self-report, which is the same principle as the reviewer being a safety net rather than the first line of defence. **The gap that limits the payoff today:** the three in-flight suites carry no `# COVERS:` header yet, so they are selected fail-safe on every change — `README.md` currently selects 3 of 9 suites purely for that reason. Both agents have been instructed to add the header; once they do, an unrelated change selects nothing. **Repo-wide follow-up, owed at the end-of-build pass:** the other ~150 `tests/canonical/` suites have no manifest either, so `--glob` is currently required to keep selection useful; promoting `# COVERS:` across the repo is what makes this pay off beyond work-005, and belongs in the KB alongside S1-S5. |
| 2026-08-05 | **THE SPAWN COST LOCATED, and my own "1,733 spawns" claim CORRECTED** | `select-suites.sh`, six suite headers | **I told the owner the scan makes "1,733 process spawns". That was WRONG and I corrected it unprompted: 1,733 was the count of TRACED COMMAND LINES, and 1,634 of those are cheap shell builtins. The real count is ~100 external spawns, and they account for 8.4 s of the 11.2 s traced run.** So the variable is not spawn count being enormous; it is **per-spawn cost being pathological**. Measured directly, 50 iterations each, on a THREE-LINE input so no measurement includes real work: `sort` **105.7 ms**, `cut` **106.7**, `wc` **114.4**, `tr` **120.1**, `awk` **137.5**, `grep` **136.5** — against a bash parameter expansion at **3.4 ms**. Any external process costs ~30× a builtin here, and ~100× what it costs on Linux. **Cause found, and it is machine-level, not code-level: `Get-MpComputerStatus` reports Defender real-time protection ENABLED with `ExclusionPath` holding exactly one EMPTY entry — no exclusions at all — while `C:\Program Files\Git\usr\bin` holds 249 executables that every pipeline stage re-launches.** That gives two independent levers: **(A)** Defender exclusions for the repo root and the Git/node toolchain — zero code change, plausibly 5-10× on EVERY suite, but the owner's to make since it needs admin; **(B)** fold the ~100 spawns toward ~30 in `scan-source.sh`: `sort` 23 spawns/3.07 s (dedup-only sorts become `awk '!seen[$0]++'`, no spawn), `awk` 14/1.54 s (consecutive stages fold into one program), `tr` 10/0.74 s, `wc` 6/0.47 s, `cut` 5/0.42 s, `grep` 5/0.36 s all fold into a neighbouring pass or a builtin; `dirname`/`realpath` become `${p%/*}`; the 4 `xargs` STAY because they are the batching mechanism, as do the 2 `mv` (atomic writes). 63 of the ~100 are foldable, worth ~4.3 s. **FULL-SET BASELINE, measured, all passing: relationship-validator 196 s/121, gap-ledger 79 s/303, source-enumeration 73 s/189, schema-loader 58 s/211, view 29 s/119 (22 SKIPs — jsdom is not a repo dependency), skill-registration 25 s/207. 1,150 assertions, ~460 s.** The validator is the outlier at **1.6 s per assertion** and is the first optimisation target. **A RACE I CAUSED AND MUST NOT REPEAT:** the view suite first reported `rc=2` with `line 456: unexpected EOF`, and I nearly filed it as a defect — it was **my** `python` rewrite of the suite headers landing while the background timing job was still reading those files. Re-run clean: `rc=0`, 119/0, 29 s. **This is the second time in this build I have written to files another process was actively using** (the first cost a wasted carriers build). The rule I keep relearning: **never write into a tree a background job is reading; wait for it, or work on a copy.** |
| 2026-08-05 | **OWNER DIRECTIVE — test strategy adjusted for run time; the cost is SPAWN COUNT, and the diagnosis separates three causes** | -- | Owner asked why suites take so long, whether we run them too often, and whether we run out-of-scope suites. **Measured, not guessed: `scan-source.sh` takes 9.7-9.9 s on a TWO-FILE repository, flat across three runs, so it is essentially all fixed overhead.** A `bash -x` trace with `PS4='@@${EPOCHREALTIME}@@'` (export required — an inline assignment silently produced no timestamps) shows **1,733 commands for that 2-file input, ~5 ms each, with no hot spot**: largest single gap 393 ms, aggregate `sort` 2.5 s / `awk` 1.0 s / `xargs` 0.69 s / `wc` 0.49 s / `mv` 0.46 s. `dirname` cost 268 ms for ONE call and `mv -f` 388 ms to rename ONE file — that is process creation, not computation. **So local runtime is proportional to PROCESS SPAWN COUNT, not input size.** IMPORTANT SCOPE CORRECTION I made to my own claim: I first measured 110 ms/fork in the interactive tool shell and computed "1,733 × 110 ms = 378 s", which **contradicts the measured 9.37 s** — that figure belongs to the tool shell, not the script environment, and I discarded it. On Linux CI these spawns cost well under 1 ms, so this is **mostly a Windows-developer problem, not a shipped-product defect**. **Three distinct causes, ordered by weight: (1)** the ~10 s fixed subject cost; **(2)** suites re-invoking the subject per assertion group instead of scanning once and asserting many times against cached output; **(3)** agent iteration habit — evidence from the transcripts: the runtime agent ran `test-graph-runtime.sh` **33×** and the digest **13×** across 149 shell calls, plus **19 runs of five suites it does not own** (skill-registration 7, gap-ledger 4, view 4, schema-loader 3, validator 1); the extraction agent was clean by comparison (8 own + 5 schema-loader, the latter justified because it changed `relationship-schema.sh`, exactly that suite's subject). **THE STANDARD, issued to both in-flight agents and binding on the committed suites too: S1** invoke each subject ONCE per distinct input, never per assertion group; declare the invocation count in the suite header. **S2** load subject output into memory in one pass and assert with bash builtins — no command substitution or grep/awk/cut per assertion (measured 300 command substitutions **20.7 s** vs 300 builtin calls **0.16 s**). **S3** mutation matrix behind an explicit `--self-mutate` flag so CI pays one pass. **S4** never weaken coverage to hit a time target — if an assertion truly needs its own invocation, keep it and say so; report before/after wall time AND invocation count. **S5** to verify a one-line fix run the single failing assertion group, and run a sibling suite you do not own at most ONCE, only if you changed a file it covers. **Owed at the end-of-build pass: promote S1-S5 into the KB as a canonical testing convention** (it applies beyond work-005's suites). Also noted: my own profiling helper used `$(grep -c … \|\| echo 0)`, which emits `0\n0` — the very false-count shape catalogued earlier in this build, committed by me in a throwaway command. |
