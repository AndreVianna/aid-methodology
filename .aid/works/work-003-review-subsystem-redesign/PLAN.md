...
| 6 | A recall figure first taken late in the work cannot be attributed to any one delivery | M | delivery-024 lands the corpus early and free, and from its close **Lane A breaks the build**: a script rule that stops catching its seeded defect fails CI at that close rather than surfacing at 027. The agent-lane figure cannot start earlier than the script that computes it -- `feature-009`'s SPEC § 7 states what each close gives, and this row does not restate it |
