...
Carries `FR-E2` and `AC-17`. Feature owner `feature-009` -- see `PLAN.md § Open at Plan`.
...
- [ ] `state-fix.md` states the sweep as a step with an output, not as advice
- [ ] **The fixture fails without the sweep.** A corrected claim restated at two other sites must
      leave the fix rejected until the sweep output naming both sites is on the record. Verified in
      both directions -- a run with the sweep passes, a run without it fails
...
`REQUIREMENTS.md § 5` — which is the same discipline this delivery mechanises.
