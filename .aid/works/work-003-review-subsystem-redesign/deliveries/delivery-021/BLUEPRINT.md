...
finding. The real one was that four decisions read as settled in prose while disk disagreed, and
they fail in **three different ways**: **two** sit inside a delivery that closed `Done` over work it
did not deliver; **one** (`Q5` decision 7) is **benign** -- the work shipped and the decision was
consciously reversed, so only the record's text is stale; and **one was never scheduled into any
