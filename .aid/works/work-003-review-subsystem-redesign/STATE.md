...
| definition/spec-report-shape-vs-downstream-gate-criterion | criteria | Answered | 1 | 0 | specify-feature-009 | No definition-family rule reaches a feature SPEC that DEFERS a report element a downstream delivery gate criterion requires and cites this SPEC for. DEF-08 runs BLUEPRINT-to-task and no task set exists yet; DEF-04 needs the artifact this one derives from (REQUIREMENTS, not a BLUEPRINT); DEF-09 needs a false claim, not a deferral. Live instance: deliveries/delivery-027/BLUEPRINT.md:52 gate criterion 5 requires the report to name the coverage worklist item that should have caught a miss, while SPEC:189 defers -- Attribution needs FR-D10 -- and the report shape at SPEC section 3 step 4 carries no attribution element. The intent ladder puts SPEC above BLUEPRINT, so the SPEC may be right to defer and the BLUEPRINT the artifact out of step, which is why no rule fires. Raised in cycles 7, 8, 9 and 10 and registered on the fourth sighting. | Answered 2026-08-11 as an ORCHESTRATOR disposition, NOT a human ruling -- flagged to the owner as overturnable. The gap said no rule reaches a SPEC that defers a report element a downstream gate criterion requires, and that it could not fire because no task set existed. The task set now exists, so DEF-08 fired for real at detail-024-025-027 cycle 1 [HIGH]: delivery-027 gate criterion 5 (miss attribution) was asserted by the TEST task and built by no IMPLEMENT task. Resolved by ADDING THE TASK rather than by amending either document: delivery-027 task-004 (IMPLEMENT, Miss attribution) now carries it, depends on task-002, and states explicitly that it is executable only after delivery-026 lands because attribution needs FR-D10. The former TEST task renumbered 004 -> 005 and every citation of it was swept. Why this closes the gap without a new rule: the disagreement was never SPEC-versus-BLUEPRINT, it was a BLUEPRINT criterion with no task behind it, which DEF-08 already expresses -- the option (c) rule would have duplicated DEF-08 one artifact-level up. The SPEC keeps its deferral, correctly: it says attribution needs FR-D10, and the task is where that dependency becomes concrete. Of the three options the reviewer put, this is nearest to (b) -- confirm the owner and cite it -- with the owner being delivery-027 itself rather than feature-005. Overturn this if the owner wants the attribution element written into the SPEC report shape instead, which would move the work from delivery-027 to a feature-009 SPEC amendment. |
| definition/task-depends-on-minimality | criteria | Declined | 1 | 0 | detail-024-025-027 | No rule in definition.md or the KB requires a task Depends-on edge to be minimal -- only that dependencies are acyclic and reference tasks that exist. Raised because two edges looked over-serialized: delivery-024 task-004 (needs only the seeded corpus) chained after task-003 (the Lane A suite), and delivery-025 task-004 (renders task-002 output) chained after task-003 (the AC-17 suite). | Declined 2026-08-11 as an ORCHESTRATOR disposition, NOT a human ruling: no rule is added, because the premise does not need one. Both edges were simply WRONG and are corrected -- each now depends on task-002, the task whose output it actually consumes. That is DEF-03 territory already (a downstream section traces to what it needs), not a new minimality criterion. A rule saying "make edges minimal" would also be near-undecidable: minimal against what, and who arbitrates when a reviewer thinks a chain is deliberate sequencing? The correction is visible in the graph rather than in prose: both deliveries went from 4 waves to 3, with task-003 and task-004 now parallel, which is exactly the effect the gap predicted and is what makes the fix checkable. Straight-chain-by-default is NOT confirmed as a convention -- it was the authoring default and it produced two wrong edges out of twelve, so the lesson is to derive each edge from what the task consumes. Reopen for a human decision if a reviewer and an author disagree about whether a chain is deliberate. |
| definition/parallel-siblings-sharing-a-target-file | criteria | Pending | 1 | 0 | detail-024-025-027 | No rule reaches two wave-parallel sibling tasks with no Depends-on edge between them that modify the SAME file. definition.md DEF-01..DEF-09 do not cover it and the aid-detail reviewer brief asks only that dependencies be acyclic and reference tasks that exist. It is a real hazard, not a theoretical one: aid-execute/references/state-execute.md PD-1 computes the ready set from satisfied Depends-on edges alone and prints Wave-infinity (pool), so there is NO wave-boundary barrier at runtime -- two edge-less siblings can be dispatched to concurrent agents editing one file, and a third task depending on only one of them can verify before the other lands. Found at detail-024-025-027 cycle 2 in delivery-027, where task-003 and task-004 both modify canonical/aid/scripts/review/recall-measure.sh. Reviewer proposal: extend the DEF-08 family to check same-target-artifact overlap between edge-less siblings. | PENDING A HUMAN DECISION -- the two live instances are already fixed, so nothing is blocked; what is open is whether a rule should exist. Fixed instances, swept as a class rather than only where the reviewer pointed: (1) delivery-027 task-004 gains a Depends-on edge to task-003, labelled in its Scope as a SHARED-FILE edge rather than a data edge, with a do-not-remove-as-redundant note -- delivery-027 goes back to a 5-wave straight chain; (2) delivery-024 task-004 named NO target file at all, which is why its overlap with task-003 was invisible -- it now names tests/canonical/test-recall-grade-neutrality.sh, its own suite, and the two stay wave-parallel legitimately. Checked and clean: delivery-025 task-003 (fixtures + test-class-sweep.sh) and task-004 (the five-profile render of state-fix.md) share no target. NOTE the tension a rule would have to resolve, because it is why this is a human call and not an orchestrator one: cycle 1 of this same review declined a dependency-MINIMALITY gap on the grounds that an unnecessary edge is a defect, and this gap asks for an edge that carries no data. Both are right, and a rule has to say so: an edge is justified by a data dependency OR by a shared write target, and by nothing else. Anything looser re-authorises the over-serialisation cycle 1 removed. |
## Cross-phase Q&A

<!-- DERIVED -- union of per-delivery Q&A plus WORK-OWNER-AUTHORED entries below
     (work owner is the single writer for those). -->

### Q35 -- an edge is justified by data OR by a shared write target, and by nothing else (2026-08-11)

- **Raised by:** the two `/aid-detail` review cycles over deliveries 024, 025 and 027
- **Impact:** Medium -- it decides whether a rule is added; the live instances are already fixed
- **Status:** **Open for a human decision.** Register key
  `definition/parallel-siblings-sharing-a-target-file`

**How it arose, and why it is one question rather than two.** Two review cycles produced two gaps that
look like opposites:

| Cycle | Gap | What it wanted |
|---|---|---|
| 1 | `definition/task-depends-on-minimality` | **fewer** edges -- two tasks were chained behind a task whose output they did not consume |
| 2 | `definition/parallel-siblings-sharing-a-target-file` | **more** edges -- two edge-less parallel tasks both wrote `recall-measure.sh` |

Both are right, and a rule has to say why. **An edge is justified by a data dependency, or by a shared
write target, and by nothing else.** Cycle 1's two edges had neither, so they were deleted. Cycle 2's
missing edge had the second, so it was added and labelled a *shared-file edge* in the task's Scope with
a do-not-remove-as-redundant note -- because a later reader applying cycle 1's lesson would otherwise
delete it as carrying no data.

**Why the hazard is real rather than theoretical.** `aid-execute/references/state-execute.md` PD-1
computes the ready set from satisfied `Depends on` edges **alone** and prints `Wave ∞ (pool)`. There is
**no wave-boundary barrier at runtime.** So two edge-less siblings can be dispatched to concurrent
agents editing one file, and a task depending on only one of them can verify before the other lands.
The `Wave` column in a BLUEPRINT is a derived display, not a scheduler.

**The class sweep found a second instance the reviewer did not point at**, and it was the more
interesting one: `delivery-024`'s `task-004` named **no target file at all**, which is exactly why its
overlap with `task-003` was invisible. It now names its own suite, and the two stay parallel
legitimately. The lesson generalises past this work: **an unnamed target cannot be checked for
collision**, so "name the file you write" is the precondition for any rule about shared writes.

**What the owner is being asked.** Whether to add the rule -- the reviewer proposed extending the
`DEF-08` family to check same-target overlap between edge-less siblings -- or to leave it to authoring
judgment. Nothing is blocked either way.
