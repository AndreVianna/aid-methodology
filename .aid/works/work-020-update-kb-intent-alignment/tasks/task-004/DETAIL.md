...
**Scope:**
- Add structural/consistency tests (in the repo's existing skill-test harness) that assert the redesign's invariants (AC-1..AC-10) are encoded in `canonical/skills/aid-update-kb/`:
  - **AC-10/FR-11:** SKILL.md has a Pre-flight ISOLATE step that creates + enters the skill's own worktree on an `aid/update-kb-<ts>` branch off `master` (plain `git worktree add -b` + the `worktree-lifecycle.md § Step 2` enter step; NOT the work-NNN-keyed `worktree-lifecycle.sh`) before any state, fail-closed.
  - **DONE branch:** `state-done.md` commits on the Pre-flight worktree's `aid/update-kb-<ts>` branch — no residual `git checkout -b`/"Ensure branch" step — and never pushes `master`; SKILL.md's "DONE (commit convention)" note agrees.
  - **AC-9/HL-8:** SKILL.md wires ANALYZE/SCOPE as clean-context dispatches receiving only the verbatim instruction + KB/code (no session transcript); `state-scope.md`'s `Traces-to` may cite the instruction or a KB/code location, never "the session".
...
- **Source-doc sweep:** confirm no lingering `Change Plan` references remain in ANY source doc (specifically `state-apply.md` Step 1 read, `state-review.md` fallback, and `state-done.md` commit template), not only tests/fixtures.
...
**Acceptance Criteria:**
- [ ] Tests assert AC-1..AC-10 are encoded in the skill's reference docs — including AC-4's disk-derived scope-diff guard, AC-5's re-scope revert, AC-9's clean-context dispatch, and AC-10's Pre-flight ISOLATE worktree step.
