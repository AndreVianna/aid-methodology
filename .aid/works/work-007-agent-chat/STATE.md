...
| 2026-09-01 | **`W7-2` fixed upstream; conversion still held, now on `W7-3`** | -- | The repository defect that blocked this work's state conversion is fixed on its own branch and merged in: `features[]` is a declared AUTHORED key, and four further silent defects in the same converter were found and closed once the guard stopped hiding them (tables merged across a section's range; a fixed-field read leaking the `\x1f` separator and making the document unparseable; Q&A state read under the wrong label, emptying 28 of 28 entries; an inline annotation riding inside the enum). A fifth was created by fixing the first and is closed too — `review_history[]`, because un-merging the Review History table would otherwise have deleted it. Every fix is mutation-tested. **The conversion is still not done, and the reason is a new and larger finding rather than the old one:** this file converts to 64,138 bytes from 110,852, and the missing 42% includes the **D1–D14 decision registry**, the `### Cross-Reference` narrative, and the scope-reset blockquote — none of which the YAML key set can hold. Filed as **`W7-3`** (High/P1). The conversion was run, measured and **deliberately not committed**. **Decision needed before `/aid-specify` can run: declare keys for the remaining blocks, relocate that content to where it now belongs (`decisions.md`, `REQUIREMENTS.md`), or make the converter refuse what it cannot place.** |
| 2026-09-01 | **Re-baseline chosen over migration — `CARRY-FORWARD.md` drafted** | -- | Stakeholder decision: wind this work down and restart the pipeline in a fresh work folder rather than migrate this one's state. **The reason is the plumbing, not the requirements** — three weeks produced no product code while v3.0.0 retired the artifact set underneath the work and `W7-3` blocked its state conversion; a successor created by current tooling writes `STATE.yml` natively and meets neither. **The requirements are the asset and are carried, not rewritten:** `CARRY-FORWARD.md` in this folder is the handoff, and its central instruction is to seed the successor from this work's folded `REQUIREMENTS.md` rather than from a prompt written from memory, because a prompt from recall silently drops decisions and re-litigates them later. It records what moves (D1–D14 to `decisions.md` first, §1–§10 with a per-section disposition, the store contract verified by execution on Node 22/24/26, the `22.13.0` effective floor, the discovery-mechanism survey, `feature-001`'s A+ spike protocol whole, and four open questions), what is dropped as a decision rather than an omission (the process state; `feature-003`'s spec on four dead premises — **but carrying its two HIGH findings forward as constraints**, since exit `5` is taken and `id INTEGER PRIMARY KEY` without `AUTOINCREMENT` lets SQLite reuse a reaped member's id; 006's and 009's stale descriptions; all withdrawn scope), and what is already durable in the KB and survives pruning regardless. **Two limits of the restart are stated rather than glossed:** it removes existing debt but does not protect against future mid-flight churn, and it does not escape `SY-6` — `/aid-describe`'s own routing prose still reads the retired `STATE.md` shape and would misroute a `STATE.yml` work to `FIRST-RUN`, which the target state happens to self-correct. **Next: open `work-024-agent-chat`, then cancel this work.** |
| 2026-09-01 | **Successor seeded — `work-001-agent-chat`** | -- | The successor is created and seeded, at the stakeholder's chosen number (`work-001`, not the `work-024` this file previously proposed; nothing on disk references `work-001` and the `W1-*` debt-id pattern is undocumented convention, so the objection raised against it did not hold). It has a native full-layout `STATE.yml` -- so `W7-3` never touches it -- and a `REQUIREMENTS.md` seeded from this work's folded document rather than re-interviewed. **Carried:** §1-§3, §6, §10 verbatim; §5 with FR-0-FR-7 and FR-8 withdrawn-not-renumbered; `ID-1`-`ID-14`; the store contract and the `22.13.0` floor evidence; Feature 001's A+ spike protocol whole; eight features' Descriptions and user stories. **Changed on the way, all mechanically verified:** §9 renumbered to a gapless **AC-1-AC-22** (the five gaps left by criteria deleted at interview or withdrawn with FR-8 are closed, since a fresh work has no citation history to protect), and the §9 provenance footnote rewritten to keep the honesty record -- which six criteria carry pipeline wording rather than the stakeholder's -- and drop the withdrawal archaeology. **Dropped:** Feature 003's pre-reset Technical Specification, ~1,100 lines resting on four dead premises, **but its two HIGH findings are restated as constraints inside the section that must satisfy them** (exit `5` is taken; `id INTEGER PRIMARY KEY` without `AUTOINCREMENT` reuses a reaped member's id). **Verified after every edit:** 22 criteria owned exactly once, §10 and §11 agreeing stage for stage, 40 requirements mapped, and no prose reference pointing at a feature that does not exist. Feature ids keep their gap at 002 deliberately -- closing it meant re-pointing ~100 citations with no mechanical oracle, traded against one contiguous number, and the reasoning is recorded in §11. **This work is not yet Canceled:** that is the stakeholder's call once the seed has been reviewed. |
| 2026-09-01 | **Decision registry relocated — carry-forward step 1, done differently than written** | -- | The fourteen carried decisions moved out of this file into `REQUIREMENTS.md` § 7 as **`ID-1`–`ID-14`**, not into `.aid/knowledge/decisions.md` as `CARRY-FORWARD.md` first proposed. **That proposal was wrong on three independent counts, each sufficient:** `decisions.md`'s authoring contract requires every decision to be grounded in evidence and each existing entry cites a file that exists, whereas this product does not exist and the only available evidence is this prunable work folder, which the KB may not cite; these decisions are explicitly re-openable ("none has passed this work's approval gate") while `decisions.md` entries are `Accepted` with an enforcement mechanism; and `decisions.md` already uses `D1`–`D29`. Promotion is **ship-time** work as `D30`+, which §9's AC-25 withdrawal note had already said. **The relocation still achieves what the step was for** — `REQUIREMENTS.md` is never touched by the `STATE.yml` converter, so the registry and its fourteen *rejected alternatives* are out of `W7-3`'s reach, and they now travel to the successor with the requirements rather than needing a separate move. Two defects fixed on the way: each override in `ID-9`/`ID-12`/`ID-13` is now written out **inside its own row** instead of pointing at a Q entry in this file, which would have dangled when the folder was pruned; and the ids are `ID-n` rather than `D-n`, because Feature 003's Scope Ledger already uses `D-1`–`D-8` in the same document for its spec-level decisions — one character apart, in a table a reader could take for the same series. Coverage re-verified after the edit: 22 criteria owned once each, 40 requirements mapped. |
...
**Moved 2026-09-01 to `REQUIREMENTS.md` § 7 → "Decisions taken, and the alternatives rejected",
renumbered `ID-1`–`ID-14`** (`ID` for *interview decision*; the mapping is exactly `Dn` → `ID-n`,
chosen because `D1`–`D29` is `decisions.md`'s namespace and `D-1`–`D-8` is Feature 003's Scope
Ledger's). The table is not deleted and nothing in it is lost; only its home
changed, and for two reasons that both point the same way.

**It is requirements rationale, not process state.** This file records where the work is in the
workflow; why a design was chosen and what was rejected is part of the design, so it belongs with
the requirements.

**And it would not have survived this file's conversion.** `STATE.yml`'s key set has no home for
an unnamed table, so a `STATE.md` → `STATE.yml` conversion drops it silently (tech-debt `W7-3`).
`REQUIREMENTS.md` is never touched by that converter, so the move puts the fourteen decisions —
and, more to the point, the fourteen *rejected alternatives*, which cannot be reconstructed from
the design — out of reach of the defect entirely.

Three rows (ID-9, ID-12, ID-13) had an element overturned later. Each override is now stated **inside
its own row** rather than as a pointer to a Q entry in this file, because those pointers would have
dangled the moment this folder was pruned.

**Promotion to `.aid/knowledge/decisions.md` is ship-time work, not now** — that document grounds
every entry in evidence on disk, this product does not exist yet, and its `D1`–`D29` namespace
already collides. See the note above the table in §7.
...
- **`W7-2` is fixed and the conversion still has not happened — one blocker replaced by a different, larger one (2026-09-01).** With the guard corrected, this file converts cleanly: 12 features, 10 interview sections, 13 review-history rows, 8 lifecycle rows and all 28 Q&A entries, with the four genuinely-pending questions reading `Pending` rather than blank. **But it converts 110,852 bytes into 64,138** — and the missing 42% is not filler. Three blocks have no key in the YAML schema: the **D1–D14 decision registry** carried from the originating interview (14 architectural decisions, each with the alternative it rejected — cited by requirement and criterion reasoning throughout this document), the `### Cross-Reference` narrative (grade, cycle count, trajectory, and the dated coverage note that marks which parts of cycle 14's proof no longer hold), and the ~40-line scope-reset blockquote recording the runtime decision. The conversion prints `OK` and there is no reverse converter. **It was therefore run, measured, and deliberately not committed**, and the defect is filed as `W7-3` — a schema decision (declare keys / relocate the content / make the converter refuse), not something to settle inside this work. This work stays on markdown `STATE.md` until it is answered.
- **Not done in the fold, deliberately:** the `STATE.md` → `STATE.yml` conversion. It is blocked by `W7-2`, and unblocking it is a repository schema decision (does Features State get a real key, or is per-feature state derived from whether §11's Technical Specification is filled?) touching the three state templates, `bin/aid`, both dashboard readers and their tests. Exempting the section from the converter's DERIVED guard would make conversion **silently drop** the Feature 003 row — grade D, twelve findings — so the cheap fix is the wrong one. Folding first was also the correct order independently: the fold changes what Features State says, so converting first would have meant rewriting the YAML afterwards.
