...
One `###` subsection per feature — a decomposition of §5 into an independently implementable unit, not a second place to state requirements. Every §5 functional requirement maps to at least one feature, and **every live §9 criterion is owned by exactly one feature**, so both are checkable. Twenty-two criteria are live (AC-8 and AC-14 were deleted during the interview; AC-20, AC-24 and AC-25 were withdrawn with FR-8), and the ownership below accounts for all twenty-two, once each, matching §10 stage for stage.
...
- **Criteria:** AC-21  ← ids from §9; never restated here
...
- **Criteria:** AC-7, AC-23  ← ids from §9; never restated here

> **Re-specification pending, and the Technical Specification below is the reason.** It was
> written before the 2026-08-10 runtime decision and rests on four premises that decision
> contradicts: the store is stdlib `sqlite3` rather than `node:sqlite`; the node is a separate
> `chat-node/` distributable carrying its own `pyproject.toml` rather than shipping inside the
> `aid` payload (FR-7.6, now inverted); a **Python** runtime is the prerequisite rather than Node
> (FR-7.7); and `aid chat deploy` is specified as an idempotent verb, though FR-7.2 deletes it
> outright. It is carried here rather than reset to a placeholder because parts of it survive
> intact — the store schema was verified to run verbatim on `node:sqlite` — and because twelve
> open review findings cite it. Read every passage naming Python, `sqlite3`, `pyproject.toml`,
> `chat-node/` as a distributable, or `deploy` as **superseded, not current**.
>
> **Review state:** In Review at grade **D** (cycle 3), twelve findings open, two of them HIGH —
...
> silently inherits its messages. Feature 005 is gated on the second.
>
> **FR-7.4 ownership note — three clauses, three features, and this is where they are settled.**
> FR-7.4 has three separable clauses and the retired `SPEC.md` files disagreed about who owned
> which, because Feature 008 was rewritten from an MCP façade into the chat skill after the
> carve-outs were written. As folded: **this feature** owns *the node publishes no client library
> or SDK* (the clause cross-reference cycle 12 retargeted it onto); **Feature 008** owns *the
> chat skill's instructions are CLI invocations*; **Feature 006** owns *the subscriber is a CLI
> invocation*. Together they keep the HTTP transport internal to the node, which is what FR-7.4
> asks for.
...
so Feature 002 (withdrawn) gates nothing here and nothing here gates Feature 002 (withdrawn) — which is what the
...
- **Criteria:** AC-2  ← ids from §9; never restated here
...
- **Criteria:** AC-5, AC-5b, AC-6, AC-9, AC-12, AC-22  ← ids from §9; never restated here
...
- **Criteria:** AC-1b, AC-3, AC-4, AC-16, AC-19  ← ids from §9; never restated here
...
> zero-configuration discovery layered above it as best-effort, and **AC-3 says outright that it
...
> rather than restated, so AC-3 governs. What remains is the Description's own wording and the
...
- **Criteria:** AC-10  ← ids from §9; never restated here
...
- **Criteria:** AC-13  ← ids from §9; never restated here
