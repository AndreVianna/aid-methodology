...
| FR-B6 | SHOULD | ~~One grading backend for letter grade: retire or demote parallel points/`grade-summary.sh` paths so `grade.sh` is the sole letter producer.~~ **DECLINED by owner decision, 2026-08-17 (Q5)** — recorded, not skipped, which a `SHOULD` permits. Two measured blockers: the V1 human visual gate can force `F`, and `grade.sh` cannot emit `F` (`grep -c 'GRADE="F"' canonical/aid/scripts/grade.sh` → `0`), so unifying the backends first needs a decision about how that verdict is expressed; and `grade-summary.sh` has ten dependents, including `tests/coverage-baseline.tsv`. Both are design changes outside this work's scope. The two-backend state stands. |
