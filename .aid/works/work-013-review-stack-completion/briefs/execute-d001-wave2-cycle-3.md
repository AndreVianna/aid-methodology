ARTIFACTS UNDER REVIEW:
  ...
  HUNT (scoped -- look for NEW findings only here):
    - task-003's per-row table Severity column and its correction section
    - task-005's outcome tally block

CYCLE 3 -- WHAT THE FIX CHANGED (verify rows 1, 4 and 5; rows 2 and 3 are already Fixed):
  Row 1 [MEDIUM] -- you were right and the previous fix was wrong. `Step 2` was in the SEVERITY
    column of the PER-ROW table (cell 6), not the evidence column; the earlier fix changed only
    the summary table. FIX: cells 6 of the two per-row entries now read HIGH and MEDIUM, matching
    the summary. Parse the row rather than reading it -- that is what caught this.
  Row 4 [LOW] -- the correction section asserted those cells were "in the evidence column", which
    was false and is what caused the incomplete fix. FIX: the claim is replaced with a correction
    that states what actually happened and why a wide row makes the two columns look identical.
  Row 5 [MINOR] -- the tally carried TWO admit rows (a stale bolded `1` for DEF-06 and the
    corrected `5`), so the Count cells summed to 30 against a 29-row corpus. FIX: the stale row is
    removed; the tally is 5 + 0 + 3 + 21 = 29.

  A class sweep was run over all three FINDINGS files: every severity cell in every per-row table
  now names a real band, with only table separators and a column header left as non-band cells.
  Re-test that claim rather than accepting it.

  Read the existing ledger at .aid/.temp/review-pending/execute-d001-wave2.md FIRST. Update each
  row's Status in place -- do not delete rows, do not rewrite Severity or Description. Append any
