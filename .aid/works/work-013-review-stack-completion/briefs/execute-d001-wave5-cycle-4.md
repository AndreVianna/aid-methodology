ARTIFACTS UNDER REVIEW:
  ...
  HUNT (scoped -- NEW findings only here):
    - the corrected sed excerpt in the kb_baseline paragraph

CYCLE 4 -- ONE ROW (do not re-litigate rows 1-5, all Fixed):
  Row 7 [MINOR] -- the excerpt was headed `sed -n '377,383p'` but showed only lines 378-382.
    FIX: the header now reads 378,382 and the body was produced by RUNNING that command and
    inserting its output, not by retyping. Re-run it and compare byte for byte.

    I also swept the class rather than the instance: every `$ command` block in
    artifact-schemas.md, RECORD.md and task-010/CORRECTION.md was re-run and compared. 11
    reproduce; 3 were flagged by my sweep and are its artifacts, not defects -- a recorded base
    commit that is deliberately not current HEAD, a two-line awk with a backslash continuation
    that the sweep ran only the first line of, and a bare `echo $?` that has no meaning alone.
    Spot-check that judgement: if any of those three IS a real defect, say so.

  Read the existing ledger at .aid/.temp/review-pending/execute-d001-wave5.md FIRST. Update row 7
