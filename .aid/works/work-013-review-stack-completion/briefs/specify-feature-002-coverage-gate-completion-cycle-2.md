ARTIFACTS UNDER REVIEW:
  ...
  HUNT (scoped -- look for NEW findings only here):
    - the same file, limited to the seven regions the cycle-1 FIX touched:
      § Artifacts & Surfaces (the flow.json row), § FR-B4 (the ledger-scope enumeration),
      § FR-B4 (the per-section characterisation), § FR-B5 (the depth counts),
      § FR-B6 (the AUTO_POOL cite, the letter-count command, and the new severity table).

  Derivation: the single ledger row set names this one file, so VERIFY is that file in full.
  `git diff --stat` over the fix shows changes only in the regions listed above, and nothing
  outside this work folder references them.

CYCLE 2 -- WHAT THE FIX CHANGED (verify these seven rows; do not re-litigate them):
  Row 1 [LOW]   letter-grade count: the command now uses `grep -oE ... | sort -u | wc -l` -> 11,
                and names the plain `grep -cE` -> 27 as a different quantity. Re-run both.
  Row 2 [LOW]   AUTO_POOL: now `3` with the file made explicit and the three line numbers given.
  Row 3 [LOW]   flow.json: now `111` with the producing command inline.
  Row 4 [MINOR] depth count: the drifting constant is replaced by the ratio plus a pinned
                commit, because the number grows as the work runs.
  Row 5 [MINOR] per-section: now 4 summarize hits + 1 aid-specify heading, named exactly.
  Row 6 [MINOR] ledger scopes: now enumerated by command instead of an incomplete list of four.
  Row 7 [MINOR] FR-B6 severities: a per-check severity table is added, so no executor invents one.

  Read the existing ledger at .aid/.temp/review-pending/specify-feature-002.md FIRST. Update
  each row's Status in place -- do not delete rows, do not rewrite Severity or Description.
  Append any genuinely NEW finding in the HUNT regions as a new Pending row.
