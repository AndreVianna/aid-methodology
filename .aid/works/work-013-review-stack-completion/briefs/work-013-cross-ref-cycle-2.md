ARTIFACTS UNDER REVIEW:
  ...
  Derivation, so it can be checked rather than trusted. VERIFY is every file named in a
  cycle-1 ledger row's Doc column: REQUIREMENTS.md (row 1) and feature-002's SPEC (row 2).
  HUNT is what the cycle-1 FIX changed -- `git diff --name-only` over the fix commit
  returns exactly those two SPEC files -- plus anything that mechanically references them,
  of which there is none outside this work folder (`grep -rl feature-00[12]-` returns only
  the work's own STATE.yml, which is a state file and never an artifact).
...
CYCLE 2 -- WHAT THE FIX CHANGED (verify these two rows, do not re-litigate them):
  Row 1 [LOW] REQUIREMENTS.md:141 -- NFR-5's first component (VERIFY/HUNT remains
    mandatory) had no acceptance criterion. FIX: feature-001's SPEC gained a criterion
    requiring the cycle-2+ brief to carry the two labelled lists, and AC-2's criterion is
    now scoped to "NFR-5 parts 2-3". Decide Fixed or Recurred.
  Row 2 [MEDIUM] feature-002 SPEC:55 -- the criterion dropped FR-B4's "per-section"
    qualifier, pre-deciding open Q6. FIX: the qualifier is restored verbatim from FR-B4
    and a note states that Q6 is the owner's to answer. Decide Fixed or Recurred.

  Read the existing ledger at .aid/.temp/review-pending/interview-work-013-cross-ref.md
  FIRST. Update the Status of those two rows in place -- do not delete them, do not
  rewrite their Severity or Description. Append any NEW finding as a new Pending row.

  The cross-document contradiction pass (Guard 2) already ran on cycle 1. Do not run it
  again: it is once per phase by construction.
