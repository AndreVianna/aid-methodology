ARTIFACTS UNDER REVIEW:
  Six tasks in one wave. Cycle 1, so this is ONE unlabelled list.

  task-010 CONFIGURE — settings schema of record
    - .aid/knowledge/artifact-schemas.md § settings.yml (rewritten)
    - canonical/aid/templates/settings.yml (format_version added)
    - .aid/works/work-013-review-stack-completion/deliveries/delivery-001/tasks/task-010/CORRECTION.md (a refused scope line)

  task-013 IMPLEMENT — lint wiring
    - canonical/skills/aid-discover/references/state-generate.md § Step 5a
    - .github/workflows/test.yml (new citation-lint step)
    - .aid/knowledge/artifact-schemas.md (the two lint rows + the paragraph under them)

  task-016 IMPLEMENT — kb.html claims gate
    - canonical/aid/scripts/summarize/kb-html-claims-check.sh (new)

  task-022 IMPLEMENT — dispatch protocol history removal
    - canonical/aid/templates/reviewer-dispatch.md

  task-023 TEST — widened history sweep
    - tests/canonical/test-kb-template-authoring-standard.sh (AS09, AS10)

  task-024 IMPLEMENT — grade.sh attribution
    - canonical/skills/aid-summarize/references/state-manual-checklist.md
    - canonical/skills/aid-summarize/references/state-generate.md

  Also in the diff, NOT independently graded: the profiles/ render and the two root
  install trees (.claude/, .cursor/), which are generator output and byte-copies of it.
