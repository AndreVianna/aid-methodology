...
- `args.value` (both ops): a single-line string, length-capped by the op arg-schema; rejected server-side if it contains a newline, `|`, or a backslash `\` (the backslash guard mirrors `settings.set`'s KI-001 charset guard — added during the delivery-001 gate so a two-char `\n`/`\t` sequence can't corrupt the target). Defense-in-depth: the writers substitute the value into `awk` via an environment variable (`ENVIRON[...]`), so even a backslash that bypassed validation is written literally rather than interpreted as an escape (`write-requirement.sh` still rejects `\n`/`|` → exit 4; `writeback-state.sh mode_field` rejects `|`/newline → exit 4). Empty string is an **accepted** input meaning clear-to-fallback, but is NOT forwarded verbatim: `writeback-state.sh` dies **exit 5** on an empty `--value` (`--value is required with --task-id --field`, L403) and `write-requirement.sh` needs a non-empty bullet, so the argv-builder substitutes each writer's null sentinel first — `--` for `task.rename`, `*(pending)*` for `pipeline.rename` (§Feature Flow). Exit 4 → HTTP 422 `invalid-value` (feature-001 map).
