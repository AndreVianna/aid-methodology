...
- [ ] The `task.rename` arg-schema validates `work_id`/`task_id`/`delivery_id`/`value` (single-line, no `|`/newline/backslash), and the argv-builder substitutes the `--` null sentinel for an empty `args.value` (so an empty clear never hits `writeback-state.sh`'s exit-5 empty-value die).
