...
- Shared exit alphabet with `writeback-state.sh` / the server `DEFAULT_MAP` — **0** ok, **4** invalid-value, **5** missing/malformed required arg, **3** IO-or-unverifiable-write failure; the lock-free writers NEVER emit **2** (that code = lock contention → 409 `busy` in `DEFAULT_MAP`) — so `DEFAULT_MAP` maps both writers correctly. (Corrected during EXECUTE — the original "0/2/4, mirror read-setting.sh" premise was wrong: `read-setting.sh` is a reader whose exits the server never maps, and `writeback-state.sh` uses 5=missing-arg / 2=lock, not 2=arg/IO.)
...
- [ ] Both new writers' exit alphabet conforms to the server `DEFAULT_MAP` / `writeback-state.sh` (0 ok / 4 invalid-value / 5 missing-arg / 3 IO-or-write-fail; NEVER 2, which is lock contention) so DEFAULT_MAP maps their exits correctly.
