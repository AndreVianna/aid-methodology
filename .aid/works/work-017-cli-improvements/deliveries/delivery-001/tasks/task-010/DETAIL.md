...
**Acceptance Criteria:**
- [ ] The `task.set-notes` arg-schema + argv-builder are added identically to both twins: `task_id` accepts the prefixed or bare form and normalizes to the numeric id; `value` <= 1 KiB rejects `|`/newline/backslash (422); an empty `value` -> the `--` sentinel; env `AID_STATE_FILE`/`AID_WORK_DIR` from `resolve_work_dir` (WT-1).
