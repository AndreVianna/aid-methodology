...
- **Root cause (investigated):** on `origin/master`, Node `_buildDeliverableRef` (reader.mjs) **deliberately strips `delivery_state` at model-build time** ("tracked internally but NOT serialized here -- server.py `_ser_deliverable_ref` omits it for parity"), whereas the Python model RETAINS it internally (Python omits it only later, at server serialization). `capture_golden.py` reads the two reader MODELS directly (not through server serialization), so the Python leg sees `delivery_state` and the Node leg does not -- hence the recorded inequality. (`done_pct` `50.0` vs `50` is equal by value and never contributed.)
- **Resolution (owner: "do 1 then 2" -> option 1, fix in THIS branch):** the golden's authoring-time twin agreement was reconstructed by making the throwaway `old_pkg` Node model retain `delivery_state` in `_buildDeliverableRef` (matching the Python model, i.e. the `21bf9636` behavior). Golden regenerated: all three fixtures now record `old_python_equals_old_node: true`; the full task-011 golden suite passes (48 passed, 0 failed). NO product code changed -- only the reconstructed authoring tool + the committed golden/meta in this branch.
- **Scope note:** the master-level model-build asymmetry itself is a `master` reader concern, NOT introduced by this refactor; work-009's own shipped readers agree. Not promoted to `tech-debt.md` (a master issue, outside this work).
