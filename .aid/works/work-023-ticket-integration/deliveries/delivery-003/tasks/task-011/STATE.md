---
state: Done
review: 'self-verified: single-clause deletion confirmed (entity mapping = 0; ''tool or "none"'' + ''access method'' retained; lead question + red flag verbatim); accuracy checked against on-disk ### infrastructure.md block; canonical-only edit (rendered by task-013)'
elapsed: "--"
notes: "--"
...
`deliveries/delivery-003/tasks/task-011/STATE.md`. The `## Task State` mutable cell
...
<!-- AUTHORED -- values live in the YAML frontmatter block at the top of this file
     ...
     `writeback-state.sh --task-id 011 --field State --value VALUE`.
...
<!-- AUTHORED -- written by `writeback-state.sh --task-id 011 --findings ...` during the
