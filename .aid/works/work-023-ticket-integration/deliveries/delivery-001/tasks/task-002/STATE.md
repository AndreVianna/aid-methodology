---
state: Done
review: 'quick-check: no blockers; argument-hint normalized to bare grammar line (LOW)'
elapsed: "--"
notes: "--"
...
`deliveries/delivery-001/tasks/task-002/STATE.md`. The `## Task State` mutable cell
...
<!-- AUTHORED -- values live in the YAML frontmatter block at the top of this file
     ...
     `writeback-state.sh --task-id 002 --field State --value VALUE`.
...
<!-- AUTHORED -- written by `writeback-state.sh --task-id 002 --findings ...` during the
