...
| FR-0.4 | **The product writes no host tool's configuration EXCEPT the chat stop hook, and only under FR-0.6's guards.** It renders a skill into the host dialects it already supports, through the same pipeline as every other AID skill, and touches nothing else — no MCP registration, no settings file beyond that one hook entry. A host whose skill is absent loses discoverability, never capability: the full message plane stays reachable over the CLI per FR-0.3. **This was an absolute prohibition and is now bounded**, on explicit authorisation from the owner. What changed is the scope, not the reasoning: the two hazards behind the original rule are real and are now handled as *checks* rather than as a blanket refusal, which is why FR-0.6 exists and why the exception is one hook entry rather than a general licence |
| FR-0.5 | **The hook is generated, not hand-assembled.** `aid chat hook --tool <host>` prints the stop-hook block with the resolved absolute `node` path, the adapter path, and both timeout numbers filled in from the running system; `--check` READS an installed hook back and reports what is wrong with it -- above all the two numbers disagreeing, which otherwise fails with no error and no log. This does not weaken FR-0.4 and depends on it: generating text and reading a file are both short of writing one. It exists because the manual assembly FR-0.4 implies is where the operator actually fails |
| FR-0.6 | **`aid chat hook --install` writes the hook into the operator's USER-scope host settings, after showing the change and asking.** Five guards, each the direct answer to a hazard FR-0.4's original prohibition existed to prevent, and none of them optional: (1) **user scope only** — the target is resolved under `$HOME` and re-checked by the writer itself, never trusting its caller; (2) **a git-tracked target is refused outright**, whatever its path, because that is the one mistake the operator cannot undo alone — it reaches every contributor's checkout on the next commit; (3) **merge, never overwrite** — the file is parsed, only the hook entry the product owns is changed, and everything else is preserved; an unparseable file is left untouched rather than replaced, because 'it was malformed anyway' is not the product's judgement to make about somebody else's settings; (4) **a backup is taken** before any write; (5) **nothing is written without an answer**, the default is no, and a non-interactive caller is refused rather than defaulted — `--yes` is the only way to skip the prompt. `--uninstall` is the inverse and equally surgical, removing only the product's own entry. Both are **idempotent**: re-installing an identical hook and uninstalling an absent one are no-ops rather than errors, because a command safe to re-run is one an operator will re-run instead of checking first |
...
| FR-2.1a | **`name` is optional, and when omitted the node MINTS one as `<adjective>-<noun>`** (`green-giraffe`, `proud-thistle`). It is deliberately not derived from the working directory: a basename collides inside one machine (every checkout has a `src`), collides ACROSS machines almost by construction (two people with the same project laid out the same way both produce `api`, and FR-9.2's federated roster then shows one name for two distinct sessions), and cannot distinguish two hosts started from one folder. A minted name is **opaque** — it says nothing about what the session is working on — and that is the accepted cost of being unique and sayable, with FR-2.1c as the remedy |
| FR-2.1b | **The minted name is looked up, not recomputed**, keyed on **(cwd, tool)**. Stability across a restart is what FR-2.2's reattachment depends on, and a random name cannot be re-derived, so the node answers "what name does this directory under this tool go by" and is the single source of truth — for the CLI and for the waker adapters alike. **The tool is part of the key**, not a refinement: keyed on directory alone, starting two hosts in one folder made the second REATTACH to the first's session, giving two agents one identity. Two sessions of the **same** tool in the **same** directory remain indistinguishable and require `--name` or `AID_CHAT_SESSION`; a verb that cannot choose **refuses and lists the candidates** rather than picking, because picking is that same defect |
| FR-2.1c | **A session may be renamed while a conversation is in progress**, with `rename(name, to)`. The rename moves the session's identity and its membership — including on every peer holding it as a remote member — and **leaves the transcript alone**: the log records who spoke at the time they spoke, and rewriting it would also break the idempotency and per-speaker uniqueness keys that include the sender name. Renaming **restarts the speaker sequence** (FR-4.2), because a reader holds an expected next `sender_seq` per sender NAME and would otherwise wait out the gap grace on a message that had already arrived |
...
| FR-5.7a | **A woken turn is instructed to tell the user what arrived, before acting on it.** The wake text otherwise addresses the model alone, and from the user's seat a wake is indistinguishable from the assistant deciding to do something unprompted: an idle session starts working, and there is no channel visible anywhere to explain why. The instruction names the sender and the message and comes AHEAD of the reply command, because told afterwards the user learns what happened from a transcript instead of being kept in the loop. The same instruction appears in the agent-facing skill, since a session in a host with no adapter reads the skill and nothing else |
| FR-5.7b | **The re-entry ceiling is `adapterLoopLimit`, 5 by default, matching Cursor's own documented cap** — and it is a configurable limit rather than a constant in each adapter (ID-10), read from one registry by both, because an unattended exchange is only as deep as its shallower side and cross-tool is the case this exists for. It was 1 on one adapter and 2 on the other, which measured **three hops** before an exchange stalled. The reasoning for being stricter than the host was sound about loops and wrong about cost: a wake requires a **message**, since the adapter reads what is pending before it waits and returns empty when there is nothing, so a chain of follow-ups carries a chain of real messages rather than empty laps. The ceiling **throttles rather than ends** — a declined stop clears the count, so the next one serves again — and **defers rather than drops**, leaving the message pending for whichever stop comes next |
...
| **P1 — Skeleton** | Node lifecycle + CLI + registration by stable name + **the hub plane** (roster and the directed connect request, FR-9) + an agent opening / joining / leaving its **one** channel and that channel's automatic end (FR-3.3, FR-3.4) + the operator's **eviction** of a session from its channel (FR-7.2) + the local half of listing (FR-3.1) + durable `send`/`inbox`/`ack` with the two positions (FR-4.4) and fan-out to every member, **pull only** (FR-5.3 — the pull floor ships here, not at P2). **The hub plane lands here because it must exist by P2, not because nothing else can create a channel.** An earlier version of this row claimed the connect request was the only way two agents ever meet; that is **false and is corrected here rather than left standing** — FR-3.1's local listing plus FR-3.4's join already let one agent open a channel and another find it by name. What the connect request adds is reaching an agent that is **not looking**, and that becomes the only working rendezvous at P2, where the wake arrives and an idle agent stops polling anything. So the hub plane's criteria sit at P1 and its necessity is proved at P2 | AC-3, AC-6, AC-7, AC-8, AC-9, AC-10, AC-13, AC-22, AC-27, AC-28, AC-29, AC-30, AC-31, AC-32, AC-33 |
...
> is **not** forced by there being no other way for two agents to meet — an earlier version of this
> note said so and was wrong. FR-3.1's local listing and FR-3.4's join let one agent open a channel
> and another find it by name, with no hub plane involved. The hub plane's necessity appears one
> stage later: at P2 the wake arrives, an idle agent stops looking at channel lists, and a directed
> request becomes the only rendezvous that still works. Its criteria sit at P1 because that is where
> `§10` puts them, and because the roster and channel membership read the same tables the message
> plane already owns.
