...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _fail(self, code: int, why: str, run: str = "") -> None:
        ...
        try:
            ...
            # Same reasoning as the success path: a departed client is not a stub fault, and the
            # log is the record. This was the last write that could still reach the terminal.
            _emit(self.log_sink, "client_gone", run,
                  note=f"client={self.client_address[0]} err={type(exc).__name__} "
                       f"(while reporting {code} {why})")
            return
