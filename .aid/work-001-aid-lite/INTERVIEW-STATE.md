...
| 6 | 2026-05-23 | A (carried) | Split — work-003-traceability extracted | FR4 (progress traceability), pain-point #4 ("no progress visibility"), and `feature-007-you-are-here-heartbeat` (renumbered to `feature-001` there) moved to a dedicated work `work-003-traceability`. Rationale: traceability is orthogonal to AID-Lite's speed concern; separating them keeps each work's scope tight. Work-001 now ships 4 features (FR1+FR2+FR3+FR6) with grade A carried forward on the reduced scope. See `work-003-traceability/INTERVIEW-STATE.md` for the receiving end. |
