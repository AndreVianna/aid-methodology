...
**Scope:**
- `canonical/skills/aid-monitor/SKILL.md` uses **Step-keyed** as its per-state body convention.
- Extract each `## Step N: TITLE` body block into `references/step-{N}-{slug}.md` (or fold into single `references/procedure.md`).
...
- [ ] Per-state (or per-section / per-step) body extracted to `canonical/skills/aid-monitor/references/*.md` files loaded on demand per the convention above.
...
- [ ] When state completes, router prints `Next: [State: {NEXT}] — run /aid-aid-monitor again` and exits (no auto-advance).
- [ ] Worker column points at sub-agent name where state warrants offload, else `inline`.
- [ ] Behavioural parity: running `/aid-aid-monitor` before and after on the same workspace produces the same state transitions and outputs.
