...
**Scope:**
- `canonical/skills/aid-interview/SKILL.md` uses **State-keyed** as its per-state body convention.
- Extract each `## State N: NAME` body block into `references/state-{name-slug}.md`. Leaves clean insertion point for delivery-002's State TRIAGE between State 1 and State 2.
...
- [ ] Per-state (or per-section / per-step) body extracted to `canonical/skills/aid-interview/references/*.md` files loaded on demand per the convention above.
...
- [ ] When state completes, router prints `Next: [State: {NEXT}] — run /aid-aid-interview again` and exits (no auto-advance).
- [ ] Worker column points at sub-agent name where state warrants offload, else `inline`.
- [ ] Behavioural parity: running `/aid-aid-interview` before and after on the same workspace produces the same state transitions and outputs.
