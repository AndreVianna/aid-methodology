...
- [ ] When state completes, router prints `Next: [State: {NEXT}] — run /aid-deploy again` and exits (no auto-advance).
- [ ] Worker column points at sub-agent name where state warrants offload, else `inline`.
- [ ] Behavioural parity: running `/aid-deploy` before and after on the same workspace produces the same state transitions and outputs.
