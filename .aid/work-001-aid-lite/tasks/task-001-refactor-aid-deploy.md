...
**Scope:**
- Extract each `## Mode: NAME` H2 block in `canonical/skills/aid-deploy/SKILL.md` into a separate `references/state-{name}.md` file (lowercase, hyphenated).
...
- Note: smallest skill — first cutover validates the per-skill recipe before applying to larger skills.
