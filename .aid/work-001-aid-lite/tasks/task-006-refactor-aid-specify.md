...
- Extract each `## State N: NAME` body block into `references/state-{name-slug}.md`. Multi-state machine: INITIALIZE / CONTINUE / SPIKE / BLOCKED / REVIEW / DONE.
...
- [ ] When state completes, router prints `Next: [State: {NEXT}] — run /aid-aid-specify again` and exits (no auto-advance).
- [ ] Worker column points at sub-agent name where state warrants offload, else `inline`.
- [ ] Behavioural parity: running `/aid-aid-specify` before and after on the same workspace produces the same state transitions and outputs.
