...
**Scope:**
- Extract each `## Mode: NAME` H2 block in `canonical/skills/aid-discover/SKILL.md` into a separate `references/state-{name}.md` file (lowercase, hyphenated).
...
- Note: largest skill (~596 lines); last cutover per SPEC. Sub-agent dispatch pattern in Steps 2-5 preserved as-is in the dispatch table (Worker column points at the 4 discovery agents).
