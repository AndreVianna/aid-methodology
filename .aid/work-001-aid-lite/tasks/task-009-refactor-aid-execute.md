...
**Scope:**
- Extract each `## Mode: NAME` H2 block in `canonical/skills/aid-execute/SKILL.md` into a separate `references/state-{name}.md` file (lowercase, hyphenated).
...
- Note: most-touched skill in subsequent deliveries (003 two-tier, 005 pool). Refactor leaves clean insertion points for the per-task quick-check + per-delivery gate + pool dispatch.
