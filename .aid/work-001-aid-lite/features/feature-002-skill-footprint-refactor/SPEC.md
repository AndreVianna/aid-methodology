...
| `Advance` | **One of three forms.** (1) **Unconditional:** `→ {NEXT-STATE-NAME}` — the literal name of the next state in the machine. (2) **Halt:** `→ halt` — terminal / human-gated (the user re-invokes the skill to proceed). (3) **Conditional:** `→ {STATE-A|halt} ({condition}) / → {STATE-B|halt} ({otherwise})` — allowed *only* when the branch depends on a **computed criterion** (a grade, a count, a status field) that is deterministic and inspectable from STATE.md without further dialog. Either leg may be a named state OR the literal `halt` (terminal — user re-invokes). Exactly one conditional split per row, with a clear else. No multi-step ladders, no mode logic, no user-input branches (those are state-detection logic). **Canonical examples:** `aid-execute` REVIEW row — `→ FIX (grade < min) / → DONE (grade ≥ min)` (two named states); `aid-execute` DELIVERY-GATE row — `→ halt (grade ≥ min) / → FIX (grade < min)` (halt-as-leg authorized). **Retro-applied to:** `aid-summarize` VALIDATE row (`→ MANUAL-CHECKLIST (grade ≥ min) / → FIX (grade < min)`), `aid-interview` LITE-REVIEW row (`→ LITE-DONE (grade ≥ min) / → CONDENSED-INTAKE (grade < min, loopback)`). User-driven escalate paths in aid-interview's lite states are NOT dispatch-row branches per the no-user-input-branches constraint — they are state-detection-driven re-entries via `lite-to-full-escalation.md`. |
