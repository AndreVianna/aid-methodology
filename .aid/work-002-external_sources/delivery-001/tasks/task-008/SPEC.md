...
- A `mcp` (tool-managed) connector records that it is available via the host tool's own MCP/plugin and instructs the agent to request it from the tool; ELICIT wires no hosts and stores no secret for it (Q10 — AID never wires host configs, and tool-managed connectors carry `auth_method: none`). Renders to all 5 profiles.
