...
- **feature-001 coherence touch-ups (keystone re-touch, then re-verify):** (CF4) fix the stray "user-profile location" wording — the store is repo-relative `.aid/connectors/.secrets/` per feature-001's own layout diagram; (CF5) note that non-Claude-Code hosts' MCP configs land OUTSIDE the repo tree (user-home), beyond the `.aid/connectors/` P7 carve-out — deferred (only `claude-code` installed today → repo-root `.mcp.json`, in-repo) [**MOOT under Q10** — AID writes no host config at all, so this edge no longer exists]; (CF-INDEX) connectors index builder is deterministic (no timestamp). Also: Q6(f) "context files flow through the render" was CORRECTED during drafting — `CLAUDE.md`/`AGENTS.md` are hand-maintained (installer in-place managed-region updater + FR12 AGENTS.md byte-identity), only `principles.md` is a rendered canonical asset.
- **Amended by Q10** (2026-07-09): the feature-004 wiring language in items 1, 3, and 5 is RETRACTED — feature-004 wires nothing (it is now a no-code catalog/consumption contract). Item 1 ("feature-004 UPDATES wiring-related fields when wiring an `mcp` connector") and item 3's clause ("feature-004 records the reference … into the host MCP config") no longer apply: a `mcp` connector is tool-managed — the host tool owns its config + auth, so AID records nothing into any host config and stores no secret for it. Item 5's INDEX trigger is feature-002 (on author) + feature-006 (on reconcile) only. Item 3's use-time secret RESOLUTION by feature-005 / the consuming agent still stands for aid-managed connectors. The rest of the ownership matrix stands.
