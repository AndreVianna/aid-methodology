...
- **Amended by Q10** (2026-07-09): item 1's "feature-004 UPDATES wiring-related fields when wiring an `mcp` connector" and item 5's "feature-004 (on wire) [triggers the INDEX builder]" are RETRACTED — feature-004 wires nothing (it is now a no-code catalog/consumption contract). A `mcp` connector is tool-managed (no wiring, no AID secret); the INDEX is triggered by feature-002 (on author) and feature-006 (on reconcile) only. The rest of the ownership matrix stands.
