...
(purge-before-delete so an interrupted removal re-converges on the next run). Removing an `mcp`
tool also unwires it from installed host configs, composing delivery-002's wiring path.
...
- **feature-006-idempotent-reconcile** — the reconcile diff (add/update/remove keyed on the
  ...
  INDEX regeneration via feature-005's deterministic builder; unwire-on-remove of `mcp`
  connectors via feature-004; the auth-downgrade orphan disposal is owned by feature-003 (not
  reconcile) and is out of this delivery's write path.

**Out of scope:** initial registration / elicitation (delivery-001); first-time wiring
(delivery-002); the secret twin and INDEX builder themselves (owned by 003/005 in delivery-001)
— this delivery only orchestrates them.
...
- [ ] Removing an `mcp` connector also unwires it from installed host MCP configs (composes delivery-002).
...
feature-002's declared set + ELICIT re-entry, feature-003's `purge` op, feature-005's
deterministic INDEX builder, and feature-004's unwire path. Sequenced last so its REMOVE path
can compose real wiring (delivery-002) rather than stub it.
