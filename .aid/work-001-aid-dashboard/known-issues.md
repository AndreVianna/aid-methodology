...
## KI-010: the per-repo dashboard page `.aid/dashboard/home.html` is not provisioned to user repos (deprioritized)

- **Status:** ⏸️ DEFERRED (user decision 2026-06-13: NOT a blocker — current adoption is negligible). Logged as a future enhancement; does not gate the version or delivery-010 (A+/concluded).
- **Type:** Distribution / install-wiring gap (not a code defect, not a breaking change)
- **Severity:** Medium (blocks the per-repo dashboard UI for *fresh* user repos; no effect on the AID dogfood repo, which commits the file by hand)
- **Affects:** feature-006 (per-repo work view), feature-007 (5-state KB card), feature-008 (task drill-down + breadcrumb) — everything served at `/r/<id>/home.html`
- **Description:** The multi-repo server serves the per-repo SPA from `<repo>/.aid/dashboard/home.html` (`server.py:775` ∥ `server.mjs` static-leaf handler). That file is **vendored: NO** (`vendor.js`/`vendor.py` ship only the CLI-home `dashboard/index.html`), **generated: NO** (no skill writes it, unlike `kb.html` which `/aid-summarize` renders from `canonical/templates/knowledge-summary/html-skeleton.html`), **scaffolded: NO** (no install / `aid add` / `aid dashboard` step creates it). It exists only because this repo committed it by hand, so dogfooding never exposed the gap. Fresh user repo → `has_home:false` → CLI card "dashboard not generated yet" (non-clickable) → `/r/<id>/home.html` 404. The d010 **backend** (reader detail sub-parser + server `?detail=`) ships fine — those live in already-vendored files; only the front-end host is stranded.
- **Not a breaking change:** `schema_version` stays 3 (additive/omittable `details`, RC-2 no-bump); `read_repo` output + CLI commands unchanged.
- **Intended remedy (user-directed, when adoption warrants):** tie it to repo **registration** — (a) an update-triggered re-registration of pre-existing repos into `$AID_HOME/registry.yml`, and/or (b) a post-install machine-wide `.aid/` scan that auto-registers genuine AID projects. `home.html` is a static, repo-agnostic SPA shell, so the registration step should **scaffold `.aid/dashboard/home.html` in the same pass** (or the server falls back to a vendored `$AID_HOME/dashboard/home.html` when the repo copy is absent). Decide the canonical source location for home.html then (today only `.aid/dashboard/home.html` exists).
- **See also:** `packages/npm/scripts/vendor.js`, `packages/pypi/scripts/vendor.py`; `dashboard/index.html:873` (`has_home=false` note); feature-010 SPEC (SEC-2 served-path construction).
