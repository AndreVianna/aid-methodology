# task-007a-update-aid-execute-skill-base: Update aid-execute SKILL.md — base (AC1+AC2+AC3 + state-refs)
...
**Scope:**
- Edit `canonical/skills/aid-execute/SKILL.md` to add heartbeat invocations per feature-001 SPEC:
...
- **State-ref updates** (per FR2 area-STATE rule): Update all `task-NNN-STATE.md` references to point at the work `STATE.md` `## Tasks Status` table row updates (status, elapsed time, review outcome per task). Any `DEPLOYMENT-STATE.md` reference in aid-execute (rare; aid-execute typically doesn't write deploy state) goes to `## Deploy Status`. References inside `references/reviewer-guide.md`, `references/task-type-rules.md` also need updating (in scope).
...
**Acceptance Criteria:**
- [ ] `canonical/skills/aid-execute/SKILL.md` contains state-entry prints for every state (AC1).
- [ ] `canonical/skills/aid-execute/SKILL.md` brackets every long operation with `▶/✓` lines (AC2), citing rough-time-hints table from task-011.
- [ ] `canonical/skills/aid-execute/SKILL.md` renders an ASCII state-map on each state transition (AC3).
- [ ] All legacy state-file references in this SKILL body are updated to area-STATE per FR2.
- [ ] **Scope exclusion confirmed.** This task does NOT implement AC4 sub-unit drill-down for `EXECUTE-WAVE`. AC4 lives in `task-007b` (depends on this task's base). The 007a/007b split exists to provide intermediary verification of AC1+AC2+AC3 + state-refs before stacking AC4 on top.
...
- [ ] **§6.4 — Heartbeat renders correctly *(manual verification — not automatable)*.** Invoke the edited SKILL in Claude Code (or equivalent host) on the following toy scenario: execute a 3-task synthetic delivery (3 trivial tasks: 1 IMPLEMENT, 1 TEST, 1 DOCUMENT); observe state-entry prints, state-map for each state in aid-execute, bracket-pairs around developer + reviewer dispatches (per-task quick check). AC4 NOT yet verified at this task — that gate runs in task-007b. Observe in the chat output: a `[State: NAME] — {desc}` line at each state entry; an ASCII state-map block; a matching `▶ … starting (~…)` / `✓ … done in …` pair around the bracketed operation.
