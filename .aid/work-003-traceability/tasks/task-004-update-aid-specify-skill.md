# task-004-update-aid-specify-skill: Update aid-specify SKILL.md
...
**Scope:**
- Edit `canonical/skills/aid-specify/SKILL.md` to add heartbeat invocations per feature-001 SPEC:
...
- **State-ref updates** (per FR2 area-STATE rule): Update all per-feature `STATE.md` (formerly `feature-state.md`) references to the per-work `.aid/work-NNN/STATE.md` with `## Features Status` and `## Cross-phase Q&A` section pointers. The previous 'check STATE.md for Ready status' becomes 'check the Features Status row for this feature in the work STATE.md'. References inside `references/handling-outcomes.md`, `references/known-issues-scope.md` also need updating (in scope).
...
**Acceptance Criteria:**
- [ ] `canonical/skills/aid-specify/SKILL.md` contains state-entry prints for every state (AC1).
- [ ] `canonical/skills/aid-specify/SKILL.md` brackets every long operation with `▶/✓` lines (AC2), citing rough-time-hints table from task-011.
- [ ] `canonical/skills/aid-specify/SKILL.md` renders an ASCII state-map on each state transition (AC3).
...
- [ ] **§6.4 — Heartbeat renders correctly *(manual verification — not automatable)*.** Invoke the edited SKILL in Claude Code (or equivalent host) on the following toy scenario: specify a single trivial feature (e.g., 'add a hello-world greeting') with 3 sections; observe state-entry prints, state-map, bracket-pairs around the reviewer dispatch on the Write step. Observe in the chat output: a `[State: NAME] — {desc}` line at each state entry; an ASCII state-map block; a matching `▶ … starting (~…)` / `✓ … done in …` pair around the bracketed operation.
