...
- **State-ref updates** (per FR2 area-STATE rule): Monitor area STATE is deferred per FR2 OQ-3 resolution (wait until Monitor matures). Do NOT introduce actual `.aid/work-NNN/MONITOR-STATE.md` references in the SKILL body; instead, leave a body comment at the appropriate location pointing at the area-STATE rule (`canonical/templates/work-state-template.md` and `data-model.md §1A`) so when Monitor matures, the author knows where to look. No new STATE.md writes in this task.
