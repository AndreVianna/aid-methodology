# task-006-update-aid-detail-skill: Update aid-detail SKILL.md
...
**Scope:**
- Edit `canonical/skills/aid-detail/SKILL.md` to add heartbeat invocations per feature-001 SPEC:
...
- **State-ref updates** (per FR2 area-STATE rule): Update all `task-NNN-STATE.md` references to point at the work `STATE.md` `## Tasks Status` table (init: one row per task created by /aid-detail). The previous 'write task-NNN-STATE.md per task' becomes 'append one row to the Tasks Status table for each task created'.
...
**Acceptance Criteria:**
- [ ] `canonical/skills/aid-detail/SKILL.md` contains state-entry prints for every state (AC1).
- [ ] `canonical/skills/aid-detail/SKILL.md` brackets every long operation with `▶/✓` lines (AC2), citing rough-time-hints table from task-011.
- [ ] `canonical/skills/aid-detail/SKILL.md` renders an ASCII state-map on each state transition (AC3).
...
- [ ] **§6.4 — Heartbeat renders correctly *(manual verification — not automatable)*.** Invoke the edited SKILL in Claude Code (or equivalent host) on the following toy scenario: decompose 1 delivery into 3 task-NNN.md files; observe state-entry prints, state-map, bracket-pair around the per-task file writes. Observe in the chat output: a `[State: NAME] — {desc}` line at each state entry; an ASCII state-map block; a matching `▶ … starting (~…)` / `✓ … done in …` pair around the bracketed operation.
