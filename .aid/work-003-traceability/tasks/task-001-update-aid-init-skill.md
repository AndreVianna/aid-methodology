# task-001-update-aid-init-skill: Update aid-init SKILL.md
...
**Scope:**
- Edit `canonical/skills/aid-init/SKILL.md` to add heartbeat invocations per feature-001 SPEC:
...
- **State-ref updates** (per FR2 area-STATE rule): aid-init creates the Discovery area workspace. Update SKILL body references from `DISCOVERY-STATE.md` (and any `SUMMARY-STATE.md` mention) to `.aid/knowledge/STATE.md`. The scaffold step that previously created `DISCOVERY-STATE.md` now creates `STATE.md` per the discovery-state-template.md.
...
**Acceptance Criteria:**
- [ ] `canonical/skills/aid-init/SKILL.md` contains state-entry prints for every state (AC1).
- [ ] `canonical/skills/aid-init/SKILL.md` brackets every long operation with `▶/✓` lines (AC2), citing rough-time-hints table from task-011.
- [ ] `canonical/skills/aid-init/SKILL.md` renders an ASCII state-map on each state transition (AC3).
...
- [ ] **§6.4 — Heartbeat renders correctly *(manual verification — not automatable)*.** Invoke the edited SKILL in Claude Code (or equivalent host) on the following toy scenario: create an empty target dir at `/tmp/aid-test-init-001` and run aid-init to scaffold the `.aid/` workspace; observe at minimum one state-entry print, one ASCII state-map, and one bracket-pair around the KB scaffold operation. Observe in the chat output: a `[State: NAME] — {desc}` line at each state entry; an ASCII state-map block; a matching `▶ … starting (~…)` / `✓ … done in …` pair around the bracketed operation.
