# task-010-update-aid-summarize-skill: Update aid-summarize SKILL.md
...
**Scope:**
- Edit `canonical/skills/aid-summarize/SKILL.md` to add heartbeat invocations per feature-001 SPEC:
...
- **State-ref updates** (per FR2 area-STATE rule): Update all `SUMMARY-STATE.md` references in the SKILL body to point at `.aid/knowledge/STATE.md` (Discovery STATE) with section pointers `## Knowledge Summary Status` (profile, machine grade, human grade, approval, last-run) and `## Summarization History` (per-run audit trail). Writeback section update: `writeback-discovery-state.sh` script (or its equivalent) writes to the consolidated STATE.md `## Summarization History` table.
...
**Acceptance Criteria:**
- [ ] `canonical/skills/aid-summarize/SKILL.md` contains state-entry prints for every state (AC1).
- [ ] `canonical/skills/aid-summarize/SKILL.md` brackets every long operation with `▶/✓` lines (AC2), citing rough-time-hints table from task-011.
- [ ] `canonical/skills/aid-summarize/SKILL.md` renders an ASCII state-map on each state transition (AC3).
...
- [ ] **§6.4 — Heartbeat renders correctly *(manual verification — not automatable)*.** Invoke the edited SKILL in Claude Code (or equivalent host) on the following toy scenario: run aid-summarize against the existing `.aid/knowledge/` (no `--reset`; idempotent); observe state-entry prints, state-map (PROFILE → GENERATE → VALIDATE → APPROVAL), bracket-pair around the long mermaid validation step. Observe in the chat output: a `[State: NAME] — {desc}` line at each state entry; an ASCII state-map block; a matching `▶ … starting (~…)` / `✓ … done in …` pair around the bracketed operation.
