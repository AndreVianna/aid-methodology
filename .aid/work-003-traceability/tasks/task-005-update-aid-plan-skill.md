# task-005-update-aid-plan-skill: Update aid-plan SKILL.md
...
**Scope:**
- Edit `canonical/skills/aid-plan/SKILL.md` to add heartbeat invocations per feature-001 SPEC:
...
- **State-ref updates** (per FR2 area-STATE rule): Update all references to per-work state to point at `.aid/work-NNN/STATE.md` with the `## Plan / Deliveries` section pointer. (aid-plan previously had no dedicated state file — it lived in `INTERVIEW-STATE.md` indirectly; now it has its own section in the consolidated STATE.md.)
...
**Acceptance Criteria:**
- [ ] `canonical/skills/aid-plan/SKILL.md` contains state-entry prints for every state (AC1).
- [ ] `canonical/skills/aid-plan/SKILL.md` brackets every long operation with `▶/✓` lines (AC2), citing rough-time-hints table from task-011.
- [ ] `canonical/skills/aid-plan/SKILL.md` renders an ASCII state-map on each state transition (AC3).
...
- [ ] **§6.4 — Heartbeat renders correctly *(manual verification — not automatable)*.** Invoke the edited SKILL in Claude Code (or equivalent host) on the following toy scenario: plan a single-feature work into 1 delivery; observe state-entry prints, state-map, bracket-pair around the cross-reference review. Observe in the chat output: a `[State: NAME] — {desc}` line at each state entry; an ASCII state-map block; a matching `▶ … starting (~…)` / `✓ … done in …` pair around the bracketed operation.
