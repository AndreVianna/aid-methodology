# task-003-update-aid-interview-skill: Update aid-interview SKILL.md
...
**Scope:**
- Edit `canonical/skills/aid-interview/SKILL.md` to add heartbeat invocations per feature-001 SPEC:
...
- **State-ref updates** (per FR2 area-STATE rule): Update all `INTERVIEW-STATE.md` references to `.aid/work-NNN-{name}/STATE.md` with the `## Interview Status` section pointer for section/grade tables, and `## Cross-phase Q&A` for Pending Q&A. References inside `references/cross-reference.md`, `references/feature-decomposition.md`, `references/interview-strategies.md`, `references/kb-hydration.md` also need updating (in scope).
...
**Acceptance Criteria:**
- [ ] `canonical/skills/aid-interview/SKILL.md` contains state-entry prints for every state (AC1).
- [ ] `canonical/skills/aid-interview/SKILL.md` brackets every long operation with `▶/✓` lines (AC2), citing rough-time-hints table from task-011.
- [ ] `canonical/skills/aid-interview/SKILL.md` renders an ASCII state-map on each state transition (AC3).
...
- [ ] **§6.4 — Heartbeat renders correctly *(manual verification — not automatable)*.** Invoke the edited SKILL in Claude Code (or equivalent host) on the following toy scenario: answer a 3-question greenfield triage interview (status → Approved → Feature Decomposition); observe state-entry prints, state-map (with the FR1 lite/full fork collapsed after triage), and bracket-pairs around the cross-reference review dispatch. Observe in the chat output: a `[State: NAME] — {desc}` line at each state entry; an ASCII state-map block; a matching `▶ … starting (~…)` / `✓ … done in …` pair around the bracketed operation.
