...
| **W3-1** | Undocumented exit codes | Eleven pre-existing helper scripts violate `EXE-09` (`coding-standards.md § File Header Convention` — "a new script MUST carry a header block documenting … its exit codes"). MEASURED 2026-08-07 by comparing every `exit N` literal against the header's own exit block: **seven document no exit codes at all** (`assemble-3part.sh` uses 1,2; `assemble.sh` 0,1,2; `stale-check.sh` 0,1; `summarize-preflight.sh` 0,1; `build-project-index.sh` 0,2,141; `harvest-coined-terms.sh` 0,2; `kb-citation-lint.sh` 0,1,2; `recon-classify.sh` 0,2) and **three document codes the script cannot return** (`build-kb-index.sh` documents 3, `closure-check.sh` documents 1, `kb-freshness-check.sh` documents 4). Surfaced by the delivery-015 cycle-12 class sweep for row 9 (`manual-checklist.sh`, fixed) and row 6 of delivery-014 (`lint-frontmatter.sh` help/header drift). Recorded rather than folded into that gate: none of these scripts is in delivery-015's change surface, and grading a delivery against files it never touched is the scope mismatch the FIX circuit-breaker warns about. `EXE-09` moved `SHOULD` → `MUST` on 2026-08-07, so each instance now bands at `[MEDIUM]` rather than `[LOW]`. | `canonical/aid/scripts/{summarize,config,kb}/*.sh` — 11 files, named above | Low | S | P3 |
...
The ~78 prompt-driven skills are **out of scope** for these techniques (there is no
