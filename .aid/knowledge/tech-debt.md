...
| **L7** | Dangling skill reference | **`design-lifecycle.md` names `/aid-graph`, and no such skill exists.** `canonical/skills/aid-graph/` is absent, so the sentence describes a regeneration path a reader cannot invoke and cannot inspect. `review-path-audit.sh`'s L3 layer reports it on every run as `dangling(other)=1` with a `NOTE`, deliberately without failing — the audit gates the review family only, and this reference is outside it. Either the skill was renamed and the prose was not followed, or it was planned and never built; the prose does not say which, which is why this is a row rather than a one-line fix. | `canonical/aid/templates/design-lifecycle.md` (the sentence naming `/aid-graph`'s regeneration of `relationships.md`); detected by `scripts/checks/review-path-audit.sh` layer L3 | Low | S | **P3** |
