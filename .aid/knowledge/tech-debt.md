...
| **L8** | Test invariant contradicted by an editorial change | **`AC5` asserts the repo-root README's mermaid diagram is topologically identical to the site home page's, and that is now false by intent.** The README diagram was replaced rather than tweaked: the old one mirrored the site's technical pipeline, the new one is a friendly "Start Here" onboarding flow. Measured, the two share **zero** node labels; README has 18 nodes and `index.mdx` has 20; the `Shortcut engine` node `AC5` pins by name is absent from the README entirely; and `/aid-triage` is drawn with a solid edge where the test requires suggest-only. No partial edit fixes this -- satisfying one assertion means re-authoring one of the two diagrams wholesale, which is an editorial decision about what the README is FOR, not a test repair. **This currently fails CI.** Three ways out, none of them mechanical: restore the README diagram to mirror the site; rewrite the site home page to mirror the new README; or retire `AC5` and accept that the two diagrams answer different questions for different readers. A separate parser gap in the same test -- it could not read Mermaid's `--- config: ---` frontmatter directive -- is already fixed. | `README.md` (the mermaid block); `site/src/content/docs/index.mdx` (the mermaid block); `site/src/data/__tests__/ac13-version-injection.test.ts` (`AC5 — Home pipeline diagram`) | **Medium** | M | **P2** |
