...
| **W7-2** | Migration blocked by a false premise about which zone a section belongs to | **The `STATE.md` -> `STATE.yml` converter refuses every work that has been through `/aid-define`, and the remedy its own error names cannot run.** `_aid_sc_guard_derived_table` (`bin/aid`:2625-2636) treats *any* real table row in a DERIVED-zone section as a hard error, on the premise stated in the converter's runbook header (`:2151-2155`): a current-shape work "carries the DERIVED placeholder sections but **never a REAL DERIVED row**", so a real row must mean the work predates the per-unit hierarchy and needs `migrate-work-hierarchy` first. **That premise is false for `## Features State`.** Nothing derives that section — `/aid-specify` authors it row by row (`canonical/skills/aid-specify/references/state-continue.md`:33, `state-done.md`:5) — and the repository already knows this: `aid-specify/SKILL.md`:51 records that Features State "carries no key in any of the three" state templates, and `aid-define/SKILL.md`'s accuracy note calls it "a schema gap surfaced by the refactor". So a work that merely completed feature decomposition has a real Features State row, is misdiagnosed as pre-hierarchy, and is refused. **Why the remedy makes it worse rather than better:** `migrate-work-hierarchy.sh` is an *era* migration that requires `tasks/*.md` and exits 3 (`'.../tasks/' not found`) on any work that has not reached Detail — which is exactly the population the guard catches, since a work stopped between Define and Detail is the one with feature rows and no tasks. Following the instruction verbatim therefore fails, with no second path offered: no reverse converter exists by design (`:2157-2159`), and the whole subsystem is WARN-not-fail (`:2186-2190`), so `aid update` reports success while the file is silently left behind. **Reproduced** by running `aid __migrate-repo` over a work stopped at Specify: one WARN naming `## Features State`, exit 0, `STATE.md` unconverted. **Fix:** the guard is right to exist and wrong about its member list — either give Features State a real key in the three state templates (closing the schema gap the two skills already flag, which is the durable fix), or exempt it from the DERIVED guard and carry its rows through the conversion. Either way the error text needs a remedy that a post-Define, pre-Detail work can actually execute. | `bin/aid`:2625-2636 (`_aid_sc_guard_derived_table`), `:2151-2155` (the runbook premise), `:2186-2190` (WARN-not-fail); `canonical/aid/scripts/migrate/migrate-work-hierarchy.sh` (exit 3 with no `tasks/`); `canonical/skills/aid-specify/SKILL.md`:51 and `canonical/skills/aid-define/SKILL.md` (the acknowledged schema gap) | **High** (an in-flight work cannot be migrated to the current state format, and the failure is reported as success) | S (exempt the section) or M (close the schema gap) | **P1** |
