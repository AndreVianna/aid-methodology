...
| **W1-11** | Cross-work collision (residue) | The skill corpus shrank (111 → 75 directories) and skills were renamed skills, so the collision this row *predicted* has now happened and what remains is residue, not risk. The machine-derived half is **discharged**: `tests/canonical/check-skill-counts.mjs` derives every guarded count from the one derivation and exits 0 over the live corpus — today **76 skills**, stated here in a phrasing that guard reads, rather than in one it cannot see and that would drift the way this row's previous figure did. The hand-written half is **not** closed, and two survivors are known. `kb.html` still states the old corpus total in three places, and it is **stale rather than unregenerable**: `/aid-summarize`'s GENERATE state reads `.aid/knowledge/*.md` directly (`canonical/skills/aid-summarize/references/state-generate.md` § 2 *Read all resolved KB documents*) and stages `.aid/.temp/summarize/summary-src/` itself while the run is in flight (§ 5, § 8) — a gitignored scratch path (`.gitignore`:69) that any fresh run recreates, so its absence between runs is the expected end-state of the previous run, not a missing input. What the file needs is a re-run of the skill; the cost is why it has not had one. The last full GENERATE took **24m20s** and produced a 178 KB / 20-section file (`.aid/knowledge/STATE.md` § Calibration Log), and its automated visual gate does not run alongside it — `validate-visuals.mjs` is SKIPPED because Playwright is not installed in the summarize package, so the V1 visual gate is an orchestrator step rather than an automated one (`.aid/knowledge/STATE.md` § Knowledge Summary Status, **Visual-Gate Note**). This half of the entry closes when a re-run lands; hand-patching the assembled file is not the remedy, because a hand edit is neither current nor reproducible and the next assembler run overwrites it. And `W1-2`'s hand-measured per-shape populations are still prose. Neither survivor is machine-guarded: the count guard's file filter admits only markdown, shell, JS/TS, Python and YAML extensions, so it structurally cannot see an `.html` file. Re-derive the live figure (`ls -1d canonical/skills/*/`) before trusting any prose enumeration. | `.aid/knowledge/kb.html`; `.aid/knowledge/module-map.md` (see W1-2) | **Medium** | M | **P2** |
