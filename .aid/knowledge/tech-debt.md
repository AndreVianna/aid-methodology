...
| **M5** | Unverified support claim (Python floor) | **The PyPI package promises a Python floor that nothing tests, and that floor is itself past end of life.** `packages/pypi/pyproject.toml`:10 declares `requires-python = ">=3.8"`, while every CI lane pins `python-version: '3.11'` — seven pins across four workflows (`test.yml`:52,:146; `release.yml`:83,:201,:323; `installer-tests.yml`:59; `coverage-parity.yml`:68). So the four minor versions between the promise and the test (3.8, 3.9, 3.10) are exercised by nothing: a user installing `aid-installer` on any of them receives a package whose compatibility was never demonstrated, and a syntax or stdlib feature newer than 3.8 would land green through CI and fail only on their machine. Python 3.8 additionally reached end of life in October 2024, so the declared floor is both untested and unsupported upstream. Six documentation statements repeat the `>=3.8` claim to users (`README.md`:90; `docs/install.md`:51,:105,:852; `docs/faq.md`:55; `docs/aid-methodology.md`:1446), which is how a fictional floor becomes a support commitment. **Fix:** raise the declared floor to the version CI actually runs and move both together thereafter, so the promise and the evidence cannot drift apart again. Note this is a **breaking change for PyPI-channel users** and touches the installer, the profile renderer, and the dashboard reader — it deserves its own change rather than riding along with a feature. | `packages/pypi/pyproject.toml`:10; `.github/workflows/{test,release,installer-tests,coverage-parity}.yml`; the six doc statements above | **Medium** | S | **P2 -- when convenient** |
| **W5-16** | Accessibility regression (keyboard) | **The graph page now has no keyboard zoom and no keyboard pan at all.** On 2026-08-06 the owner removed the seven viewport buttons (zoom in/out/fit, pan left/right/up/down) from the controls panel, because the mouse gestures do the same job and the panel was spending a third of its height restating them. The gestures that replaced them are wheel-to-zoom and drag-to-pan -- both pointer-only -- and no keydown handler was ever added in their place (`grep -n "keydown" graph-canvas.js` returns nothing). The `shellState.viewport` / `gcViewportFor` seam that the buttons drove is intact and unreachable: nothing in the shell calls `viewportFor` any more. So a keyboard-only reader can select and reveal nodes but cannot change the viewport, and NFR-6/AC-21's keyboard-equivalence clause does not hold for this page. **What keeps this at Medium rather than High** is the conforming alternate version -- `table.html` (task-033) carries every relationship as text, needs no viewport at all, and is linked from this page in two places -- so the DATA is reachable by keyboard even though the drawing is not navigable. That is the WCAG mechanism working as designed, not an excuse -- it bounds the harm to the visual affordance. **Fix, when wanted:** a keydown handler on the surface mapping the arrow keys and +/- to the same `gcViewportFor` actions the buttons called, which restores the equivalence without restoring the buttons the owner rejected. The vocabulary is deliberately preserved for exactly this (`VIEWPORT_ACTIONS`, `graph-controls.js`:61). Note the automatic one-shot fit (`gcMaybeAutoFit`) does NOT close this -- it fires once, at first rest, and is cancelled by the reader's first gesture. | `canonical/aid/templates/knowledge-graph/graph-skeleton.html` (the removed `controls-viewport` group); `graph-controls.js`:61; `graph-canvas.js` (`gcViewportFor`, no key handler) | **Medium** | S | **P2 -- when convenient** |
| **W5-19** | Untrustworthy gate provenance | **The two coverage-parity baseline files can be committed out of step with each other, and nothing detects it -- the committed pair WAS out of step, so the gate's own staleness warning could not be trusted.** `tests/coverage-baseline.meta` recorded `commit_sha` 6c7dd7d5 (written by `git rev-parse HEAD` on the runner, `tests/coverage-parity.sh`:265), while `tests/coverage-baseline.tsv` beside it held 73 rows for `test-graph-view-shell.sh` -- a suite that does not exist at 6c7dd7d5 (`git ls-tree 6c7dd7d5 tests/canonical/ | grep graph` returns only `test-executor-graph-flat-plan.sh`). The inventory therefore came from a later tree than the sha claims, so the two files were produced by different runs. **Why it matters more than a wrong comment:** the `diff` subcommand compares the recorded sha to HEAD and prints "the diff may be comparing against a stale baseline" (:372-373), and that WARN is what a reader uses to decide whether a reported reduction is real. With the sha unreliable the warning is noise in both directions -- it fires when the baseline is fine and stays quiet when the pair is mismatched. It is also how 12 reductions on PR #178 came to look like lost coverage when every one was a collection artifact. **Fix:** make the pair inseparable rather than asking a human to keep two files in step -- have `collect` stamp the same run identifier into both, and have `diff` refuse to run when the two disagree. A cheap partial that needs no format change -- `diff` fails when a suite named in the baseline is absent both from disk and from the accept list, which catches a split pair whose tsv is ahead of its tree. | `tests/coverage-parity.sh`:264-265 (the meta writer), :372-373 (the WARN that reads it); `tests/coverage-baseline.meta` | **Medium** | S | **P2 -- when convenient** |
...
| **W5-10** | A specified, twice-assigned deliverable that no task owns -- the FIFTH instance of this class in one work | **feature-007 requires AC-6's runtime prerequisites in TWO carriers and only one was ever built.** SPEC `:1604`-`:1608`: "The generator emits, into the page footer *and* the run's console summary: whether a network is required, which companion files must travel with the entry point, whether a build output is involved, and ... that a **working WebGL context** is required for the live graph, with the statement that the table view remains fully usable without one." `:1817` (GV23) repeats it: the footer **and** the console summary **each** state them. On disk, `render-graph-view.sh` emits **nothing at all on a successful run** -- its only `echo`s are the unknown-argument (`:49`-`:50`) and node-missing (`:57`) paths, both stderr, both failures. A sweep of all 30 task DETAILs (task-031 did not yet exist) finds **zero** occurrences of `GV23`; "runtime prerequisite" appears only in `task-011` (type DOCUMENT, emits nothing) and `task-017` (which explicitly EXCLUDES prerequisite messaging). feature-007's IMPLEMENT task, `task-013`, is already Done. `task-023` was the other candidate because `:1604` sits in feature-007's "Packaging and the entry point" section, but `task-023` is sourced from feature-012 and its DETAIL contains no occurrence of console/prerequisite/summary/footer/WebGL/network. **Why this matters beyond the missing text:** the sentence that goes missing is the one telling a reader without WebGL that they have lost nothing -- and on AID's own KB the table IS the working rendering (canvas fails NFR-7 by ~2.8x at median on a 1,609-node bench; `graph-controls.js:883` mounts the table first and unconditionally). **The pattern is the real debt:** all five ownerless obligations in this work were found by an executor DOING the work, never by a reviewer READING it -- Detail-phase review does not detect an obligation carried by no task, because there is no artifact to read. The durable fix is a Detail-phase gate that diffs each feature's acceptance-criteria ids against the union of ids claimed by its tasks. **Implementation half assigned to task-031** (created 2026-08-06 during wave-3 execution, in the owner's absence, on the task-030 precedent); its oracle already exists and already fails honestly (`GV23b`). | `canonical/aid/scripts/graph/render-graph-view.sh`:49-50,:57 (only echoes, both failure paths); feature-007 SPEC `:1604`-`:1608`,`:1817`; `tests/canonical/test-graph-view-shell.sh` GV23b (failing, correctly) | **Medium** | S (the text) + M (the Detail-phase id-diff gate) | **P2** |
| **W5-12** | A documented exit code that the code does not produce, on the error path most likely to be hit | `contrast-check.mjs:27` documents "`2  Invocation error (missing file, bad/missing --profile value).`" but a **missing file** does not exit 2 -- it exits **1**, via an uncaught `ENOENT` promise rejection rather than a handled error path. The bad/missing `--profile` half of the same sentence is correct and does exit 2; only the missing-file half is wrong. **Pre-existing and unchanged by task-019** -- confirmed identical in `HEAD` and in the working tree by running both versions against a nonexistent path. It surfaced only because task-019's acceptance criteria bound the script's documentation to its actual behaviour, and the executor correctly declined to fix it unilaterally inside a FIX cycle scoped to four other rows. **Why it is worth fixing rather than ignoring:** a caller that branches on exit 2 to mean "you invoked me wrongly" will read a missing artifact as a genuine contrast FAILURE, and an uncaught rejection also prints a stack trace where a one-line diagnostic belongs. Either handle the read and exit 2, or correct the header to say 1 -- but the two must agree. | `canonical/aid/scripts/summarize/contrast-check.mjs`:27 (the claim); verified by execution against a nonexistent path on both `git show HEAD:` and the working tree -- exit 1 with an uncaught ENOENT in both | **Low** | S | **P3** |
| **W5-13** | A truthful statement wired to the wrong disk check -- true today, false the moment the next task lands | `build-graph-src.mjs` states AC-6's companion-files prerequisite by testing whether **`graph-assets/`** is a non-empty directory beside `relationships.md`, and emits "No companion files travel with this page; everything it needs lives inside it" when it is not. **That is true today and will become false without anyone editing the sentence.** The decision record's Part 13 fact 3 requires "the companion vendor assets physically present beside `graph.html`" unconditionally, and Part 15 states the shape plainly -- "Five vendored classic-script (UMD/IIFE) builds ship as local companion files **beside `graph.html`**". Beside, not inside `graph-assets/`: a repo-wide grep finds **nothing** tying the vendored bundles to that directory, and `task-023`'s DETAIL never mentions it. So when task-023 vendors d3-force (+ d3-dispatch, d3-quadtree, d3-timer) and PixiJS as siblings of `graph.html`, `graphAssetsPresent` stays **false** and the page will assert that no companion files travel with it while five bundles sit next to it. **Why it was nearly missed:** task-031 passed its review at A+ with zero findings, and correctly so -- the sentence IS true at present, because no vendor directory exists yet (Part 10 says so outright) and `relationships.md` is base64-inlined rather than a runtime companion. The reviewer judged it would "self-correct the day the vendor directory is populated"; that holds only if the vendored files land INSIDE `graph-assets/`, which nothing establishes. **This is a constraint on task-023, not a defect in task-031:** either vendor into `graph-assets/`, or widen the check to the actual companion set. Whichever is chosen, `GV23`'s companion-files clause needs an assertion over the vendored state, which no test covers today -- the populated branch was dead code until task-031 and is still only exercised by a synthetic fixture. | `canonical/aid/scripts/graph/build-graph-src.mjs`:218-219 (the `graph-assets` check), :236-238 (the two sentences); `rendering-decision-record.md`:434 (Part 13 fact 3), :480 ("beside `graph.html`"); `task-023/DETAIL.md` contains no occurrence of `graph-assets`; repo-wide grep ties vendoring to that directory nowhere | **Medium** | S (align the check or the destination) + S (the missing assertion) | **P2** |
| **W5-14** | A CSS-block selector matched as an UNANCHORED substring, so one theme's map can be filled from another block entirely | `contrast-check.mjs`'s `blockRegex` (:102-105) builds `new RegExp(escaped + '\s*\{([^}]*)\}', 'gm')` -- **no left anchor**. So the selector `':root'` (:136, the light fallback) also matches **`html:root {`**, and `'html[data-theme="dark"]'` (:137, the CHROME dark map) also matches **`html[data-theme="dark"]:root {`**. Those two longer selectors are exactly the graph palette's own blocks (`graph-css.css`:57 and :101). Worse, `extractVars` (:117-125) **skips any matched block that declares no `--var` and keeps walking** -- and this file's own comment at :32 records that the chrome carries a `color-scheme`-only dark block with no vars. So in a document holding both families, the walk can step past the empty chrome dark block and populate `darkVars` from the **graph** dark block; `dark = {...light, ...darkVars}` (:140) then propagates that into every chrome dark pair. The consequence is not confined to the graph tokens being measured against a wrong background -- **the chrome dark map itself can be filled with graph values.** **Latent, not live:** it requires both block families in ONE document, i.e. the assembled `graph.html`, which does not exist yet (feature-010 assembles it). `kb.html` has no `html:root` graph block, which is precisely why the task-019/020 byte-identity work over `kb.html` could not see it, and why its D3 dark-extraction correction -- real and verified for the chrome -- is nonetheless incomplete for the graph profile. Found by task-014's executor while building a REAL assembled fixture for `GV13`, and confirmed here by reading :102-105 against `graph-css.css`:57,:101; it was invisible to every synthetic fixture used up to that point. **The fix is to anchor the selector match** (require a start-of-line or a `}`/`,`/whitespace boundary before it) so a selector can never match inside a longer one, plus a PV assertion over a document carrying both families -- `test-validator-profiles.sh` is the designated home. **Route to whichever task first runs a reused validator over a real `graph.html`** (task-021/task-022): a green contrast verdict there would be measuring the wrong values. | `canonical/aid/scripts/summarize/contrast-check.mjs`:102-105 (unanchored regex), :117-125 (skip-empty-and-continue), :136-137 (the two colliding selectors), :140 (the spread that propagates it), :32 (the chrome's var-less dark block); `canonical/aid/templates/knowledge-graph/graph-css.css`:57 (`html:root {`), :101 (`html[data-theme="dark"]:root {`) | **High** | S (anchor the match) + S (the both-families assertion) | **P1** |
...
| **W7-1** | Untested-by-construction CI lane (a guarantee with a hole in the lane developers feel) | `test.yml`'s **`canonical-tests`** job — the lane that runs `tests/run-all.sh` on **every pull request** — carries a `setup-node` step but **no `setup-python` step at all**, so it executes every Python suite it drives against whatever interpreter the `ubuntu-24.04` runner image happens to ship. The two `setup-python` steps in that file (`:50` in `render-drift`, `:144` in `generator-selftests`) belong to other jobs and do not reach it. **Two consequences.** (1) The dashboard reader and server suites — 794 Python tests plus the parity suites — are proven on a *declared* interpreter only at tag time via `release.yml`'s `gate`, never on a pull request. (2) **Any check that verifies pins is structurally blind to this**, because a check reading `python-version:` scalars cannot see a job that declares none; a future anti-drift guard would therefore ship green and partial on day one. Fix is one SHA-pinned `setup-python` step in `canonical-tests`, matching the exact form the other two jobs already use (`.github/dependabot.yml` states the SHA-pinning policy). **Note the interaction with M5:** that entry records the declared floor being untested, and this entry is *why* the PR lane cannot demonstrate any floor at all; resolving M5 by dropping the PyPI channel would remove M5's subject but leave this lane still unpinned, since the dashboard reader and server remain Python for as long as they remain Python. | `.github/workflows/test.yml`:65 (`canonical-tests:`), `:70` (`setup-node`, the only runtime pin in the job), `:50` and `:144` (the two `setup-python` steps, both in other jobs) | **Medium** (a PR lane outside the version guarantee; masks interpreter-specific regressions until tag time) | XS (one step) | **P2** |
