...
| **W4-1** | Shipped-code portability bug (Windows) | `check-version-sync.sh` embeds an MSYS-form path (`/c/Projects/...`) **inside a code string** handed to a Windows-native interpreter — `node -p "require('/c/...')"` and the `python3 -c` equivalent. Node and Python on Windows cannot resolve that form, the read returns **empty**, and the script reports the npm **and** PyPI carriers as out of sync. Four call sites. Any Windows user running it from Git Bash gets a false failure, and it is the gate `release.yml` depends on. **This was mis-filed for two release cycles as a "Windows-local test artifact"; it is a real defect in code we ship.** Fix is mechanical: convert with `cygpath -m` (verified: converted path returns `2.3.0` at rc=0) at each site. Product code contains **zero** `cygpath` calls anywhere, so the idiom is unguarded repo-wide. | `canonical/aid/scripts/release/check-version-sync.sh`:161, 163, 189, 191 | **Medium** | S | **P2** |
| **W4-2** | Test writes to the real repo | `test-release.sh` creates git tags in the **main repository** rather than in its own clone, and leaks them when the suite is killed — which happens routinely, since it is in the timeout-red set. Eight `aid-test-clone-*` / `aid-test-e2e-*` tags were found in the working repo; **none had reached the remote** (verified against all 33 remote tags), so nothing was ever published, but a tag in the real repo is a release-shaped artifact and this is the same isolation class as the work-018 `USERPROFILE` bit. Cleanup is `git tag -d`; the fix is to confine tag creation to the suite's own clone. | `tests/canonical/test-release.sh`:59-74 | Low | S | **P2** |
| **W4-3** | Test-suite portability (Windows) | **23 of 135 canonical suites fail on Windows** (not the 34 long assumed — the enumerated set's quantifiers over-expand under a glob: 40 matches, 17 of which pass). Diagnosed 2026-08-04, per suite, with root causes rather than a blanket "environmental" label. Five defect classes: **(A)** POSIX path inside a code string passed to a native tool — 29 sites; note MSYS *does* convert paths in argv, so only in-string paths are affected; **(E)** `chmod`-to-deny is a silent no-op on this filesystem, so negative-permission fixtures never deny — 28 sites, test fixtures only, product has no chmod-deny call; **(F)** fixtures stub only the POSIX `bin/aid` while `os.name == 'nt'` correctly takes the pwsh branch, so `pwsh -File <missing>` exits 64 — ~51 assertions; **(G)** pyenv-win's `python3` shim is a shell script routed through `cmd /C`, which cannot carry an embedded newline, so any multi-line `python3 -c` script is mangled — 47 sites, **the load-bearing one being `find_free_port()` returning an empty string**, which is the first failure in 6 suites including all 31 of `test-aid-remote`'s cascade; **(H)** dependence on local `origin/master` ancestry. **76 of ~80 non-timeout defect sites collapse into two helper functions** — that is the number that makes this tractable. Separately, 9 suites emit **zero** assertion failures and simply exceed the per-suite `timeout 300`; that is a real performance redesign, not a timeout bump (`test-aid-cli.sh` does not finish in 562 s — it forks `basename` once per file across the whole rendered profile tree before its first `echo`), and fixing it **raises** the visible failure count by exposing 19 latent class-E sites. Genuinely CI-only: **2 assertions**, not 2 suites. Sized at **12 distinct pieces: 6 mechanical, 2 design, 1 perf redesign, 3 open investigations** — the largest unknown being ~50 assertions across `test-release` / `test-release-install-e2e` sharing a `release.sh --dry-run` exit-1 root that did not reproduce outside the suite's own `make_clone`. Environment-agnostic is achievable for essentially all 23; it is not achievable cheaply, which is why it is a work of its own and not a delivery bolted onto work-004. | `tests/lib/net.sh`:27 and `tests/canonical/**` (per-class site lists to be re-derived at pickup — the 2026-08-04 sweep lived in a scratchpad, deliberately not cited here) | **Medium** | L (phased) | **P2** |
| **W4-4** | Shipped-code data corruption (locale-dependent) | `Write-AidManifest` **destroys the ISO-8601 format of every `installed_at` stamp it claims to "preserve".** `ConvertFrom-Json` does not hand back the JSON *string*: it silently coerces an ISO-8601 timestamp into a `[System.DateTime]`, and the five preserve-existing reads pass that object straight into a string-interpolated JSON write, which renders it in the **current culture's** short format (`07/26/2026 17:19:24` on en-US). Proven by experiment, not inferred: feeding `{"installed_at":"2026-07-26T17:19:24Z"}` through `ConvertFrom-Json` yields `System.DateTime` (`Kind=Utc`) that interpolates to exactly the corrupted value found on disk, while an already-corrupted `06/06/2026 20:01:03` stays a `System.String` and passes through untouched. That asymmetry makes it a **one-way ratchet** — the first PowerShell install/update degrades a stamp, and every run after that faithfully preserves the degraded form. Confirmed in AID's own dogfood manifest: `2026-06-06T20:01:03Z` was intact until an `aid update` run on 2026-06-19 rewrote it as `06/06/2026 20:01:03`, where it then sat for two months. The bash writers are unaffected (`bin/aid` emits `date -u +%Y-%m-%dT%H:%M:%SZ`; `lib/aid-install-core.sh` moves the value as text through `json.load`/awk and never re-renders it), so a bash-installed and a pwsh-installed repo now disagree in a tracked file. **Unguarded by construction:** the one suite that compares the two writers' manifests, `test-aid-cli-parity.sh`, normalises by *deleting* every `"installed_at"` line before diffing, so it is blind to precisely this field, and no test asserts the stamp's shape anywhere. ⚠️ `ConvertFrom-Json -AsHashtable` does **not** avoid the coercion (checked — still `System.DateTime`); the fix is to re-format at each preserve site, e.g. `if ($v -is [datetime]) { $v.ToString('yyyy-MM-ddTHH:mm:ssZ') } else { [string]$v }`, which round-trips the value byte-exactly. | `lib/AidInstallCore.psm1`:1021, 1049, 1066, 1162, 1175 (emitted at :920, :930); oracle gap at `tests/canonical/test-aid-cli-parity.sh`:195 | **Medium** | S | **P2** |
...
| 2.7 | 2026-08-04 | work-004 Windows-failure diagnosis | **W4-1..W4-3** opened. The diagnosis re-based two figures the project had been carrying: the Windows red set is **23 of 135, not 34**, and `test-grade-summary.sh` — logged as an unexplained red at two consecutive delivery gates — **passes** (48/0, rc=0). One genuine shipped-code defect surfaced (**W4-1**), previously mis-filed as a Windows-local test artifact. The remaining suite work (**W4-3**) is deliberately *not* folded into work-004: it is 12 distinct pieces across 5 defect classes, and its cheap majority (76 of ~80 sites in two helpers) sits behind a real performance redesign that would dominate the schedule. |
