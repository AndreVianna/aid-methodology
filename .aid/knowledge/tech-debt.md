...
| | | *ID rule for every `W<n>-<m>` row in this document, in both tables: a work number is REUSED once a work ships and its folder is pruned, so `W<n>-` does not identify one work and two unrelated efforts can share a prefix. IDs are therefore unique only DOCUMENT-WIDE. A new row must continue from the highest `W<n>-<m>` present anywhere in this document -- not the highest in the nearest table, which is how the last collision happened.* | | | | |
...
| | | *IDs in this table share the `W<n>-<m>` namespace with the main Debt Inventory above, and that namespace is flat: see the ID rule there before adding a row. The highest id in THIS table is not the highest in the document.* | | | | |
...
| **W1-21** | Coupled identity | Per-speaker FIFO ordering keys on the sender's **display name** rather than a stable session id, so a rename changes a speaker's ordering identity mid-stream. `rename` compensates by restarting the sequence from whatever that name has already said in the channel, which is correct for every case reachable today — including renaming back to a name used earlier, where it resumes above those messages instead of colliding on the uniqueness key. The residue is that ordering and identity are coupled through a mutable field, so any future feature that changes a name mid-stream has to remember the same compensation. | `chat-node/server/store.mjs` (`message.sender_name` and the two UNIQUE keys over it), `chat-node/server/core.mjs` (`reorderPerSpeaker`, `rename`) | Low — the compensation is in place and tested; the risk is to future change, not to current behaviour | Medium — key ordering on the session's stable id: a schema change across the message table, the reorder path and replication | Low |
| **W1-22** | Accepted trade | Three distinct connect-refusal causes -- an unknown target, a stale one, and one already in a channel -- collapse to the single `target_unavailable` reason token, with the distinction carried in `detail` rather than in the token. Accepted so a caller cannot use the reason code as an enumeration oracle for which sessions exist. The counter-argument is that the roster already discloses the same thing to any registered session, so the protection is thin; it is kept because a machine-readable token is the wrong place to widen disclosure, and `detail` gives an operator what they need. | `chat-node/server/core.mjs` (`connect`), `chat-node/server/hub.mjs` (`joinLocal`) | Low -- a debugging inconvenience, not a failure | Low -- split the token if the oracle argument is ever dropped | Low |
| **W1-23** | Accepted trade | `test-kb-no-work-ids.sh` permits a work id inside a code span, on the argument that command syntax showing `work-NNN` is illustrating a shape rather than citing a work folder. The rule it guards exists because a work folder is pruned when its work ships, making a cite a dangling pointer by design -- and that reasoning does apply to a code span that names a REAL work. The exemption is therefore broader than the argument supports. Kept because narrowing it needs a way to tell an illustrative id from a real one, which the guard cannot currently do. | `tests/canonical/test-kb-no-work-ids.sh` | Low -- the KB is currently clean either way | Medium -- requires distinguishing illustrative from real ids | Low |
| **W1-24** | Flaky test | `dashboard/reader/tests/test_task044_freshness_parity.py` errors non-deterministically under `tests/canonical/test-dashboard-reader.sh`: three consecutive runs of the same tree gave 4, 2 and 0 failures, while running the module directly passed 14/14. The 14 tests take ~76s, so the likely cause is a timeout or resource contention rather than logic. It matters more than a normal flake because it presents as a REGRESSION: a suite count that moves between runs was read as damage from an unrelated change, and cost real time to rule out. | `dashboard/reader/tests/test_task044_freshness_parity.py`, `tests/canonical/test-dashboard-reader.sh` | Medium -- it makes the corpus unable to answer 'did my change break anything' reliably | Low -- find the timeout and either raise it or make the tests cheaper | Medium |
...
`W1-19` is absent by resolution: it recorded `test-aid-migrate-trigger.sh`'s ISOL-01 comparing
`packages/npm/`'s state against CLEAN, so any uncommitted work there failed it -- legitimate work in
progress, indistinguishable from the scratch the check is about, which is a test that punishes the
developer for editing the thing it tests. It now snapshots that directory's state before the runs it
guards and compares after against before, so only NEW scratch fails. Verified both ways: with a
work-in-progress edit present, the old check fails and the new one passes, and new scratch still fails.

`W1-18` is absent by resolution: it recorded `writeback-state.sh` clearing a multi-line folded scalar
by rewriting only its key line, orphaning the body -- and where an orphaned line contained a colon the
file stopped parsing, which halted this repository's own pipeline three times. The writer now
recognises a block-scalar header (`>`, `>-`, `|`, `|-`, and the chomping and indentation variants) and
consumes the indented body with it, treating a blank line inside a block as part of it. Applied to the
canonical source, rendered to all seven renders, and applied NARROWLY to `dashboard/scripts/`'s
deliberate fork -- a targeted fix rather than a resync, so that fork's `Deploy` phase value survives,
verified reference by reference. Six regression cases in `tests/canonical/test-writeback-state.sh` cover
folded, literal, chomped, blank-line-inside, last-key-in-file and nested shapes, with an ordinary
single-line scalar as the control; five of the six fail against the unfixed writer and the control does
not, which is what says the fix consumes only what it should.

`W1-20` is absent by resolution: it recorded a surface-boundary criterion checked against the CLI's
verb list because the artifact it was really about -- the rendered agent skill -- did not exist yet.
The skill exists now, and the check reads all five rendered copies directly: each one's verb list
against the permitted set, each one against the forbidden set, and each one's body against the
canonical source rather than against the other renders. That last part closed a real gap, since
comparing the renders only to each other would pass a generator that mangled all five the same way.
Verified by falsification in both shapes. `tests/canonical/test-chat-node-wake.sh` WK21-WK23.
