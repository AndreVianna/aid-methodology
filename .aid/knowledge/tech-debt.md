...
| **W5-8** | Specified, owned, and implemented by nobody -- `read-setting.sh --probe` | **A deliverable that feature-004's SPEC assigns to feature-004 does not exist, and no task in work-005's 29 implements it.** The SPEC is unambiguous: `:1585` "`read-setting.sh` gains `--probe` | **feature-004** | sole caller; additive; **AC-S7/AC-20 depend on it**", `:2064` "additive change, **owned and implemented by this feature**", and its gate round 3 records "`--probe` ownership decided: this feature owns and implements it". On disk `read-setting.sh` contains **zero** occurrences of `probe`. **Consequence, which is user-visible:** `scan-source.sh:145` calls `sig_probe_ignore_list`, which shells the resolver with `--probe`, gets a usage error, and sets `SIG_PROBE=unsupported`; `:173` then prints a notice and proceeds with only the unconditional exclusions. So the `graph.ignore` setting is **inert** -- a user who uncomments it gets a notice and no filtering. The `--path` route reads the same setting correctly, which is what makes the gap easy to miss. **Why no oracle caught it, and this is the reusable lesson:** `test-graph-source-enumeration.sh` exercises all three of AC-S7's ignore-list states (`declared`/`undeclared`/`unsupported`) against a **STUB** resolver built by the suite (`:851` `R="$STUB/aid/scripts/config/read-setting.sh"`), while `REAL_RESOLVER` is defined at `:113` and never used for the probe. The suite therefore proves the rule library honours the probe *contract*, and asserts nothing about whether the shipped resolver *provides* it -- so AC-S7 reads closed while two of its three states are unreachable in production. This is the vacuous-hook class this work already catalogued, in a new shape: not an assertion that passes on absent behaviour, but an assertion that passes against a stand-in for absent behaviour. **Routing:** the implementation gap is work-005's own (feature-004 owns it and Detail failed to assign it -- an authoring defect, recorded as such); what belongs here is the missing oracle, i.e. at least one assertion that drives the probe through the REAL resolver so its absence fails. | `canonical/aid/scripts/config/read-setting.sh` (no `--probe`); `canonical/aid/scripts/graph/scan-source.sh`:145,:173; `tests/canonical/test-graph-source-enumeration.sh`:113,:851; feature-004 SPEC `:1585`,`:2064` | **High** | S (flag) + S (real-resolver assertion) | **P1** |
