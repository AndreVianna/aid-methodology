...
| **W5-7** | Config resolver returns a COMMENT as the value | `canonical/aid/scripts/config/read-setting.sh` silently resolves a comment into a config value. For a **bare `key:` line whose list continues on the following lines**, a trailing inline comment on that key line is returned *as the value*. Reproduced independently with two side-by-side fixtures: `outer:\n  ignore:   # trailing note on the bare key line\n    - examples/**` makes `--path outer.ignore` print `# trailing note on the bare key line`; move the identical comment down onto the `- examples/**` line and it correctly prints `examples/**`. Cause: in `lookup`/`lookup_list` the whitespace-stripping substitution runs first and greedily consumes the space that the comment-stripping substitution needs as its anchor, so the comment survives and the real value is dropped. **Why this is worse than a parse failure:** it does not error, it returns a plausible-looking string, so a caller cannot distinguish it from a real setting -- and this resolver is the methodology-wide settings reader. No settings key currently shipped in `canonical/aid/templates/settings.yml` uses the triggering layout, so the bug is latent rather than live; any future seed that puts a trailing comment on a bare list key would trip it. No suite covers it: `test-read-setting.sh` passes 19/19. | `canonical/aid/scripts/config/read-setting.sh` (`lookup`, `lookup_list`) | **Medium** | S | **P2** |
...
| **W5-14** | A CSS-block selector matched as an UNANCHORED substring | `contrast-check.mjs`'s `blockRegex` (:102-105) builds `new RegExp(escaped + '\s*\{([^}]*)\}', 'gm')` -- **no left anchor**. So the selector `':root'` (:136, the light fallback) also matches **`html:root {`**, and `'html[data-theme="dark"]'` (:137, the CHROME dark map) also matches **`html[data-theme="dark"]:root {`**. Worse, `extractVars` (:117-125) **skips any matched block that declares no `--var` and keeps walking** -- so in a stylesheet carrying both a var-less dark block and a var-bearing block with a longer selector, the walk can step past the first and populate `darkVars` from the second; `dark = {...light, ...darkVars}` (:140) then propagates that into every chrome dark pair. **Latent:** triggers only when the input document contains a CSS block whose selector string contains a named chrome selector as a substring. `kb.html` does not contain such a block today. **The fix is to anchor the selector match** (require a start-of-line or a `}`/`,`/whitespace boundary before it) so a selector can never match inside a longer one. | `canonical/aid/scripts/summarize/contrast-check.mjs`:102-105 (unanchored regex), :117-125 (skip-empty-and-continue), :136-137 (the two colliding selectors), :140 (the spread that propagates it) | **Medium** | S (anchor the match) | **P2** |
...
**Description:** AID has ~136 canonical suites but **no signal for whether they are
