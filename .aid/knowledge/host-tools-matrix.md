...
|------|--------|-------------------------|------------------------|-------------------------------------|
| **Anthropic Claude Code** | ✅ Live | `claude-code/.claude/` (22 agents, 10 skills, templates) + `claude-code/CLAUDE.md` | `<project>/.claude/` + `<project>/CLAUDE.md` | Row 1 — https://docs.claude.com/en/docs/claude-code/overview |
...
| **OpenAI Codex CLI** | ✅ Live | `codex/.codex/agents/` (22 TOMLs) + `codex/.agents/{skills,templates}/` + `codex/AGENTS.md` | `<project>/.codex/` + `<project>/.agents/` + `<project>/AGENTS.md` | Rows 3–4 — https://github.com/openai/codex, https://developers.openai.com/codex/ |
| **Cursor** | ✅ Live | `cursor/.cursor/{agents,rules,skills,templates}/` (22 agents, 10 skills, 2 rules) + `cursor/AGENTS.md` | `<project>/.cursor/` + `<project>/AGENTS.md` | Rows 5–6 — https://docs.cursor.com/context/rules-for-ai, https://docs.cursor.com/context/model-context-protocol |
...
|----------------|-------------|-----------|--------|------------------|----------------------|
| `aid-init` skill | ✅ `claude-code/.claude/skills/aid-init/SKILL.md` | ✅ `codex/.agents/skills/aid-init/SKILL.md` | ✅ `cursor/.cursor/skills/aid-init/SKILL.md` | ❌ | ❌ |
...
| Knowledge-summary HTML viewer assets | ✅ `claude-code/.claude/templates/knowledge-summary/` (~25 files) | ✅ `codex/.agents/templates/knowledge-summary/` (~25 files) | ✅ `cursor/.cursor/templates/knowledge-summary/` (~25 files) | ❌ | ❌ |
| `setup.sh` installer | ✅ copies `claude-code/.claude/` + `CLAUDE.md` | ❌ **CONFIRMED BUG (Q70)** — copies `codex/.codex/` + `AGENTS.md` but omits `codex/.agents/` (skills + templates). Patch trivial; tracked as `tech-debt.md H6`. | ✅ copies `cursor/.cursor/` + `AGENTS.md` | ❌ | ❌ |
...
| Cross-tool skill loading | n/a | n/a | ⭐ Reads `.claude/skills/` AND `.codex/skills/` per `cursor/README.md:142` | n/a | n/a |
...
| Agent shell-tool name | `Bash` | (Bash invoked via `developer_instructions` prose) | `Terminal` (Cursor canonical name) — but `cursor/.cursor/agents/` is **internally inconsistent** (some agents use `Bash`, e.g., `discovery-reviewer.md`). Per DISCOVERY-STATE Q52: audit + unify on `Terminal`. Tracked as `tech-debt.md M6`. |
...
All 22 agents are tier-consistent across all 3 install trees (verified by quality agent; the May 2026 tier-rename migration documented at `codex/README.md:35` was applied cleanly).
...
**Verified tier counts** (recounted from `claude-code/.claude/agents/*.md` `model:` frontmatter, 2026-05-21 post-cycle-3 review): **10 Opus + 9 Sonnet + 3 Haiku = 22 agents** per install tree. May 2026 migration (`codex/README.md:35`) confirmed consistent across all 3 trees per `tech-debt.md` L6.
...
|---|---------|-------|----------|--------|-----|
| 1 | Codex | ❌ `setup.sh` / `setup.ps1` Codex branches copy `codex/.codex/` + `AGENTS.md` but **omit** `codex/.agents/` — Codex users get agent TOMLs without skill bodies. CONFIRMED via reviewer static-analysis spot-check. | HIGH | **CONFIRMED — patch tracked in `tech-debt.md H6`** | **Q70** |
...
| 6 | Codex | Cursor `AGENTS.md` (45 lines, has KB + Permissions + Skills sections) vs `codex/AGENTS.md` (28 lines, minimal) vs `claude-code/CLAUDE.md` (30 lines, minimal) — three-way template-shape asymmetry | LOW | Pending alignment | **Q82** |
| 7 | Codex | `codex/.codex/agents/developer.toml:11-12` hardcodes a Maven build command + `ProjectRoot/pom.xml` path — likely template-fragment leak | MEDIUM | Pending cleanup | (in `security-model.md` 1.3; not separately Q'd) |
| 8 | Codex | Split `.codex/` (agents) + `.agents/` (skills) layout — documented in `codex/README.md:12-15` but rationale not stated | MEDIUM | Pending documentation | **Q9** |
...
**Positive parity finding:** the May 2026 tier-rename migration (`codex/README.md:35`) successfully unified the Opus / Sonnet / Haiku ↔ `gpt-5.5` / `gpt-5.4` / `gpt-5.4-mini` mapping across all 22 agents in all 3 trees. No drift detected.
