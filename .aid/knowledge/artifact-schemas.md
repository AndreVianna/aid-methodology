...
under a key, one key per former markdown section -- `interview`, `features`, `review_history`,
`lifecycle_history`, `deploy`, `delivery_lifecycle` [incl. its `tasks_lifecycle` child],
`delivery_gate`, `qa`, `quick_check`, `dispatch_log`), and **DERIVED** (read-only, assembled at
read time -- Plan/Deliveries, Tasks State, Delivery Gates, Calibration Log, Dispatches -- **NOT
represented on disk at all**, no key of any kind; a reader seeing no key derives the same union
it derives today).
...
| Features State | AUTHORED (created by `aid-define` at feature decomposition, updated by `aid-specify`) | `features[]` -- present on **both** layouts (a feature is a work-level concept, so this key is never omitted for a full multi-delivery work) | one entry per feature, in id order: `id` (quoted, `'001'`..), `name` (short slug; the `REQUIREMENTS.md § 11` section title is authoritative), `state: Pending \| In Discussion \| Spike Needed \| Blocked \| Ready`, `grade`, `qa_count`, `started`, `updated`, `sections[]` (per-technical-section progress, legitimately empty until `aid-specify` runs), `notes`, and `withdrawn: true` on a withdrawn feature (omitted otherwise). A feature is a **section, not a folder**: `id` identifies `REQUIREMENTS.md § 11 ### Feature NNN`, and ids are never reused or renumbered, so a withdrawn feature keeps its entry and every citation of it still resolves. |
| Review History | AUTHORED (appended by `aid-describe` / `aid-define`) | `review_history[]` | append-only sequence (`date`, `event`, `outcome`, `notes`), newest last. Records what happened **inside the interview** -- a question round, a stakeholder comment pass, a quality check, a cross-reference cycle -- and is therefore distinct from `lifecycle_history`, which records phase transitions and gates. It previously lived under a `### Review History` heading inside `## Interview State` with no key of its own, which made it invisible to the schema and mergeable into `interview.sections` by anything reading tables from that range. |
