...
| Section | Key fields |
|---------|-----------|
| Frontmatter (run-state scalars) | `kb_status`: `Initial \| In Progress \| Approved`; `kb_grade`; `last_kb_review`; `summary_approved`: `yes \| no`; `last_summary`. These five were relocated out of the header blockquote, where they read `Status` / `Current Grade` / `User Approved` / `Last KB Review` / `Last Summary`; a body line and a table row for the same value were being parsed inconsistently. |
| Header | blockquote: `Source`, and `User Approved` for the KB itself (distinct from the summary's `summary_approved` scalar). |
...
| Knowledge Summary Status | Table rows: Profile, Profile Source/Confidence, Theme, Output, Mermaid version/cache. Body lines, written by the state that produced them: `Grade`, `Grade Source`, `Checklist`. **One** grade, from `grade.sh` over the review ledger; `Checklist` records whether the mandatory human checklist has been completed. The section declares no `User Approved` field: the summary's approval is the `summary_approved` frontmatter scalar. A `**User Approved:**` bold line inside this section is a **legacy form** that `summarize/stale-check.sh` still reads as a fallback for a `STATE.md` written before the relocation — read, never written. The former `Machine Grade` / `Human Grade` pair belonged to a second grading model that has been retired; an existing `STATE.md` may still carry those rows as history, and they are deliberately not back-converted, because re-deriving a letter for findings that were never itemised would be fabrication. |
