...
> **User Approved:** yes (2026-06-06, cycle-12 — user approved the work-002 installer KB-delta refresh at A+; on branch worktree-work-002-auto-installer. Prior: 2026-06-03, cycle-11 lite-path-restructure refresh on aid/housekeep-2026-06-03)
> **Last KB Review:** 2026-06-06 (cycle-12 — work-002 installer KB-delta refresh; reviewer A+ after re-pointing the v3.2 methodology-spec anchors. Prior: 2026-06-03 cycle-11 via /aid-housekeep KB-DELTA — A+)
...
- **Impact:** Medium
- **Status:** Answered
- **Context:** The mirror-replication design implies that every helper-script change must be made in `canonical/scripts/` and propagated by `python .claude/skills/aid-generate/scripts/run_generator.py`. There is no enforcement mechanism documented in the project index (no pre-commit hook, no CI gate per `CLAUDE.md:44`). A contributor who edits one of the 4 mirror copies directly will be silently overwritten on the next render. `CLAUDE.md:75-76` warns "Never edit `profiles/{claude-code,codex,cursor}/` directly" — but `.claude/` (the dogfood tree) is also a mirror per the same logic and is not in the warning list.
