...
- **Answer:** Confirmed by user 2026-06-03 (via /aid-housekeep scope confirmation — core + optional, all 9 docs). Targeted re-discovery dispatched to the owning sub-agents: discovery-architect (architecture.md, repo-presentation.md), discovery-analyst (module-map.md), discovery-scout (project-structure.md), discovery-integrator (integration-map.md, pipeline-contracts.md, domain-glossary.md), discovery-quality (test-landscape.md); orchestrator authored feature-inventory.md and regenerated README.md + INDEX.md. Then REVIEW → APPROVAL.
