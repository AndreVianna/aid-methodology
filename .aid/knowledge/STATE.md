...
**User Approved:** yes (2026-06-22 -- /aid-housekeep SUMMARY-DELTA: summary-src 06/08/09/10 refreshed to the work-005 architecture; Overall A+; V1 visual gate approved by user)
...
| 10 | 2026-06-22 | A+ | agentic-pipeline | 11.15.0 | kb.html (3427573) | Housekeep SUMMARY-DELTA: summary-src 06/08/09/10 refreshed to work-005 architecture (2 agent formats, codex unified .codex/, render.py copy core, suites 56); 0 live stale refs; Overall A+; user-approved V1 visual. |
...
- **Impact:** Required
- **Status:** Answered
- **Context:** /aid-housekeep reconciled the repo against the KB after **work-005-profile-generator-simplify** (merged #100/#101/#102) and found drift in: **module-map.md** (the §3 renderer table + the "13/12 renderer Python files" count still describe the deleted per-renderer scripts `render_agents`/`render_skills`/`render_templates`/`render_recipes`/`render_canonical_scripts` + 2 emitter self-tests -- the generator was collapsed to a **7-file render.py copy core**: `render.py` + `render_lib.py` + `aid_profile.py` + `run_generator.py` + `verify_deterministic.py` + `verify_advisory.py` + `test_manifest_safety.py`); **test-landscape.md** ("49 test suites" -- now **56** on disk; missing `test-multitool-isolation.sh` + `test-ps51-compat.sh` + the new WinPS-5.1 CI lane); **technology-stack.md / infrastructure.md / schemas.md** (residual deleted-renderer / .py-count / agent-format references where they describe CURRENT state, not history). New infra: a real WinPS-5.1 CI lane (`installer-tests.yml`) + the `test-ps51-compat` AST lint.
- **Suggested:** Targeted re-discovery of the owning sub-agents -- module-map.md: replace the §3 per-renderer table + "13/12 renderer" count with the 7-file render.py copy-core architecture; test-landscape.md: suite count 49->56 + add `test-multitool-isolation`, `test-ps51-compat`, the WinPS-5.1 lane; technology-stack/infrastructure/schemas: refresh any CURRENT deleted-renderer/count/agent-format refs (leave dated history/changelog entries intact). Then REVIEW -> APPROVAL.
- **Answer:** Resolved 2026-06-22 via /aid-housekeep KB-DELTA. Verified ground truth: 7 generator scripts (`ls .claude/skills/generate-profile/scripts/*.py`), 56 canonical suites (`ls tests/canonical/test-*.sh | wc -l`). Refreshed 5 docs: **module-map.md** -- §3 rewritten to the single copy-core model (render.py = sole copy generator, `copy_tree` over canonical/{agents,skills,aid}: agents/skills translated, aid verbatim; dormant Codex-TOML branch); the 5 per-type renderer rows + 2 emitter-self-test rows replaced with the 7-file table; intent/contract/Module-classes counts 13/12->7. **test-landscape.md** -- suite count 49->56 (intent, contract, Suites header); added test-multitool-isolation.sh (structural multi-tool isolation) + test-ps51-compat.sh (AST WinPS-5.1 lint) subsections + the WinPS-5.1 CI lane; generator-self-test refs re-pointed (deleted emitter tests -> render.py --self-test + test_manifest_safety.py); kept qualitative (§9a). **infrastructure.md** -- build-pipeline component table + flow diagram rewritten to copy-core (line counts refreshed); CI generator-selftests + suite-count 35->56; added the WinPS-5.1 lane + ps51-compat lint. **technology-stack.md** -- Python row de-referenced the deleted emitter tests; replaced the invalid per-tree `render_skills.py --output-root` build example with `render.py --self-test`. **schemas.md** -- §9 Profile TOML rewritten to the shrunk schema ([layout]/[agent]/[skill]/[filename_map]/[extras] + LayoutConfig/FrontmatterConfig/AgentConfig/SkillConfig/RuleEntry/ExtrasConfig DROPPED; survivors name/root_dir/root_file/agent_format + [tool_names]/[model_tiers]/[capabilities]); §6/§7 renderer citations re-pointed to render.py; §8 location-rule re-pointed LayoutConfig.common_parent()->Profile.common_parent(). All dated history/changelog entries left intact; one dated changelog line added per refreshed doc. No code or other docs touched; left uncommitted for the orchestrator. REVIEW -> APPROVAL next.
