...
| 7 | 2026-07-30 | Pending | hybrid:methodology-tooling+software-cli | 6 of 21 docs | none (not regenerated) | work-001 delivery-006 gate. Six primary docs revised OUTSIDE a discover cycle: architecture, module-map, decisions, pipeline-contracts, test-landscape, tech-debt. Corrected live stale skill counts (55 found repo-wide across 15 files); reversed module-map's `site/ X canonical/` "key non-dependency" to the enforced build-time dependency work-001 created; added the five Skill Explorer module rows; migrated work-001's 12 open known-issues into tech-debt as W1-1..W1-12 so they survive the work folder being pruned. Guarded going forward by `tests/canonical/check-skill-counts.mjs`. `kb_grade` reset to Pending: these edits have had NO KB review panel. |
