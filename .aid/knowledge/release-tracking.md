...
- [CHANGE] `aid projects` is now number-driven — `aid projects list` prefixes each row with a 1-based index (a `#` column, in the existing registry-union order; the `*` current-directory marker is preserved), and `aid projects remove` accepts that number: an all-digits argument is a 1-based list index (`aid projects remove 2` removes the 2nd listed project; `0` / negative / out-of-range → clear stderr error, exit 2), while any argument containing a non-digit is treated as a path that must resolve to a registered project (an unregistered path now errors instead of the former silent no-op; a folder literally named `1` is targeted via `./1`). Both CLI twins (`bin/aid` + `bin/aid.ps1`) kept behavior-identical; the `projects` usage/help + synopsis document the new forms and drop the stale "Idempotent" wording. (work-018, PR #147)
