...
| 1.7 | 2026-07-23 | Renamed `## Unreleased` -> `## v2.3.0 - 2026-07-23` (first stable since v2.2.2, promoting the `2.2.3` beta line). Reconciled against `git log v2.2.3-beta.3..HEAD` before renaming: Unreleased held only the work-022 scan-exclusion item, so backfilled the two work-023 items (ticket skills [NEW] + PM-TOOL retirement [CHANGE], PR #168 — which merged without a ledger entry) and the transient-work-folders governance [CHANGE] (PR #167). Opened a fresh empty Unreleased. Cut by the `/release-aid minor` skill. |
