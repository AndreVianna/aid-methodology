...
- **What:** Add a layer of verb-first, direct-entry "shortcut" skills (`/aid-fix`,
  `/aid-create-api`, `/aid-document-runbook`, …), generated by `build-shortcut-skills.py` from a
  single catalog (`canonical/aid/templates/shortcut-catalog.yml`). D20 originally shipped **67**
  shortcuts from a **69-row** catalog (45 canonical names + 24 aliases; two `repurpose` rows emit
  no directory, so 69 rows -> 67 dirs); the v2.1.0 follow-on grew this to the current **76**
  shortcuts across 14 families (5 new: remove, deprecate, migrate, review, research) generated from
  an **80-row** catalog (51 canonical names + 29 aliases; four `repurpose` rows, so 80 rows -> 76
  dirs). Every shortcut is a thin doorway that binds a `{verb, artifact}` pair and delegates to one
...
- **Why:** Proportionality should be entered by *naming the change*, not weighed inside an
  interview. A shared engine keeps all 76 doorways behaviorally identical with no per-skill logic,
