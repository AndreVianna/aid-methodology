...
- **Why:** The KB is small -- a couple of dozen short markdown docs; the bottleneck is *knowing what exists*,
...
- **What:** Add a layer of verb-first, direct-entry "shortcut" skills (`/aid-fix`,
  ...
  single catalog (`canonical/aid/templates/shortcut-catalog.yml`).
  D20 originally shipped a smaller catalog that carried alias rows alongside canonical names, and
  in which some rows emitted no directory -- so the shortcut count and the row count differed. The
  alias model was later retired, which is why they now agree. The superseded figures are in git;
  reading them from the current catalog is the only way to get them right.
