...
- **What:** Add a layer of verb-first, direct-entry "shortcut" skills (`/aid-fix`,
  ...
  D20 originally shipped **67** shortcuts from a **69-row** catalog (45 canonical names + 24 <!-- count-history -->
  aliases; two `repurpose` rows emit no directory, so 69 rows -> 67 dirs), and the v2.1.0 <!-- count-history -->
  follow-on grew that to **76** from an **80-row** catalog (51 canonical names + 29 aliases). <!-- count-history -->
