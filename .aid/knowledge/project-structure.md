...
| `site/` | An Astro website (marketing + docs). Separate build from the CLI/toolkit, but its pages are generated from `canonical/` + `docs/`. | Yes |
...
6. **Website** — `site/` builds via Astro on its own toolchain (content generated from
   `canonical/` + `docs/`).
...
3. **The website (`site/`) is a separate build, not a separate source.** It has its own
   ...
   but its `prebuild` step generates four reference pages from `canonical/` and syncs four
   more from `docs/`, so a `canonical/` change can break `npm run build`. CONFIRMED
   (`site/package.json` `prebuild`; `site/scripts/gen-reference.mjs`, `sync-docs.mjs`).
