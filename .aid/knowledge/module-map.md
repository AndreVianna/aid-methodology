...
| `chat-node/server/` | Runtime | The agent chat hub: an HTTP server over `node:sqlite` holding sessions, channels and messages, plus the inter-hub TCP link, peer registry and outbox that make a channel span machines. | `node:sqlite`, `node:http`, `node:net`, `node:dgram` — no third-party dependency | medium | `tests/canonical/test-chat-node-{core,store,hub,federation,retention,integration,lifecycle,endtoend}}.sh` | Per-speaker FIFO delivery; channels replicated across every hub holding a member and authoritative nowhere. |
| `chat-node/adapters/` | Runtime | The waker adapters, one per host tool, invoked as that host's stop hook. Each blocks inside the host's own hook timeout and emits that host's documented wake shape. | `chat-node/server/` over loopback HTTP | small | `tests/canonical/test-chat-node-wake.sh` | Host-specific by necessity: re-entry signal, payload shape and BOM handling all differ. The node itself is host-blind. |
| `chat-node/scripts/hook-install.py` | Tooling | Installs or removes the chat stop hook in a host tool's user-scope settings, merging rather than overwriting and refusing any git-tracked target. | `python3` stdlib | small | `tests/canonical/test-chat-node-wake.sh` (WK42—WK45) | The only writer of host configuration in the product; both CLI twins delegate to it so one parser serves both. |
