...
**Definition-as-used-here:** One of **76** verb-first, direct-entry skills (`aid-fix`,
`aid-create-api`, `aid-change-ui`, `aid-document-runbook`, `aid-review`, `aid-remove`,
`aid-migrate`, …) — the way you enter the Lite path by *naming* your change. Each is a thin doorway
that binds a `{verb, artifact}` pair and delegates to the shared **Shortcut Engine**; the doorways
are generated by `build-shortcut-skills.py` from a single **80-row** catalog
(`canonical/aid/templates/shortcut-catalog.yml`) = **51 canonical names + 29 aliases**
(`aid-add-*` mirrors `aid-create-*`; `aid-update-*` mirrors `aid-change-*`; `aid-delete` mirrors
`aid-remove`; `aid-audit` mirrors `aid-review`; `aid-investigate`/`aid-spike` mirror `aid-research`),
where four `repurpose` rows point at existing skills and emit no directory, so 80 rows produce 76
shortcut directories. Naming a change with a shortcut is one of AID's three entry points (alongside
`/aid-triage` and `/aid-describe`). The 14 shortcut families are create, change, fix, refactor,
remove, deprecate, migrate, test + experiment, prototype, document, report, show-dashboard,
review, and research.
...
**sources:**
- `canonical/aid/templates/shortcut-catalog.yml` (the 80-row single-source catalog + field contract)
- `docs/glossary.md` ("Shortcut")
- `docs/aid-methodology.md` ("The 76 direct-entry shortcuts, by family")
