...
| **Format ⊥ behavior (behavioral metadata)** | A skill/agent's *behavior* = instruction content + **behavioral metadata**, executed within each tool's capability ceiling; *format* (markdown / TOML / `.agent.md` / rule-md) is an orthogonal container. Prose re-encodes between formats freely without changing behavior; behavioral metadata — **activation** (`alwaysApply`/glob/`trigger`/a skill's `description`), **execution** (`model`, `reasoning_effort`), **capability** (`allowed-tools`/permissions), **dispatchability** (invocable agent vs background reading) — is behavior, not formatting, and MUST be preserved/translated across any re-encoding, never dropped. | work-005 `REQUIREMENTS.md` `FR4-distinction` (agreed 2026-06-20) |
