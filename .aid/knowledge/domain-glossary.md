...
change. The single **94-row** catalog (`canonical/aid/templates/shortcut-catalog.yml`) =
**58 canonical names + 36 aliases** (`aid-add-*` mirrors `aid-create-*`; `aid-update-*` mirrors
`aid-change-*`; `aid-delete` mirrors `aid-remove`; `aid-audit` mirrors `aid-review`;
`aid-investigate`/`aid-spike` mirror `aid-research`). **64** of the rows are verb-first thin
doorways (`aid-fix`, `aid-create-api`, `aid-change-ui`, `aid-document-runbook`, `aid-remove`,
`aid-migrate`, …) — each binding a `{verb, artifact}` pair and delegating to the shared **Shortcut
Engine**, generated by `build-shortcut-skills.py`; the other **30** rows are `repurpose: true`
...
`/aid-describe`). The shortcut families mirror the verb catalog: create, change, fix, refactor,
remove, deprecate, migrate, test + experiment, prototype + design, document, report + dashboard,
review, research, deploy + monitor, and query.
...
- `docs/aid-methodology.md` ("The 64 direct-entry shortcuts, by family")
