...
a catalog row name, and every row name is a canonical one. It writes nothing — no work
...
**Definition-as-used-here:** A catalog row that lets you enter the Lite path by *naming* your
change. The single **94-row** catalog (`canonical/aid/templates/shortcut-catalog.yml`) holds one
row per invocable name, every row a canonical name: a name that is not a row is not invocable,
and there is no synonym form and no runtime redirect. **34** of the rows are verb-first thin
doorways (`aid-fix`, `aid-create-api`, `aid-update-ui`, `aid-remove`,
`aid-migrate`, …) — each binding a `{verb, artifact}` pair and delegating to the shared **Shortcut
Engine**, generated by `build-shortcut-skills.py`; the other **24** rows are `repurpose: true`
hand-authored skills (`aid-review`, `aid-research`, `aid-report`, `aid-design`, the document
family, …). Every one of the 94 rows owns its own `canonical/skills/<name>/` directory. Naming a
change with a shortcut is one of AID's three entry points (alongside `/aid-triage` and
`/aid-describe`). The shortcut families mirror the verb catalog: create, update, fix, refactor,
remove, deprecate, migrate, test + experiment, prototype + design, document, report,
review, research, deploy + monitor, and query. `create` and `update` are the two canonical grid
verbs and account for 28 of the 34 engine-generated doorways — 14 each, a bare verb plus 13
artifact suffixes (`api`, `cli`, `config`, `dashboard`, `data-model`, `data-pipeline`, `infra`,
`integration`, `job`, `messaging`, `test`, `theme`, `ui`).
...
**sources:**
- `canonical/aid/templates/shortcut-catalog.yml` (the 94-row single-source catalog + field contract)
- `docs/glossary.md` ("Shortcut")
- `docs/aid-methodology.md` ("Using the Direct-Entry Shortcuts")
...
| seed (design seed) | A `.aid/design/<artifact>.md` file recording a settled direction *before* it is built — problem, options considered, current direction, constraints, open questions, destination. Written only by a `design` skill; consumed and deleted by `create`. It is an input to the build, never a substitute for it | `canonical/aid/templates/design-lifecycle.md`; `canonical/aid/templates/design-seed.md` |
