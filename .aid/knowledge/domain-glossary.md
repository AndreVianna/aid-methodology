...
change. The single **58-row** catalog (`canonical/aid/templates/shortcut-catalog.yml`) holds one
...
family, …). Every one of the 58 rows owns its own `canonical/skills/<name>/` directory. Naming a
...
**sources:**
- `canonical/aid/templates/shortcut-catalog.yml` (the 58-row single-source catalog + field contract)
