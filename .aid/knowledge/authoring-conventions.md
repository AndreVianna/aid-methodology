...
| G-08 | `*` | validate | No superseded frontmatter field survives in a file's OWN frontmatter -- `intent:` is replaced by `objective:` plus `summary:`, and `changelog:` is not a valid field at all. A `template-payload` file is out of reach: its frontmatter belongs to the artifact it emits, not to itself, and `tests/canonical/test-kb-template-authoring-standard.sh` AS05 requires the emitted shape to keep `intent:` while the coexistence window is open | LOW | the rule to delete them existed with no id, so a reviewer that found one could not cite it and had to report outside the ledger; a rule nobody can cite is not enforced |
