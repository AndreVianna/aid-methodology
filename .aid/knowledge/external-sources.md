...
- `profiles/codex/.agents/skills/aid-discover/SKILL.md` — Inlined skill body (258 lines (post-work-001 thin-router refactor; byte-identical to canonical via canonical-generator; pre-canonical-generator the Codex tree inlined to 1,078 lines while Claude Code was 453)).
...
- `profiles/cursor/.cursor/skills/aid-discover/SKILL.md` — 258 lines (post-work-001 thin-router refactor; byte-identical via canonical-generator; pre-canonical-generator the Cursor tree was 1,090 lines).
...
**Future hardening (deferred to v3.x per Q80 resolution):** add a per-URL "expected content patterns" allow-list to `canonical/templates/scripts/fetch-vendor-docs.sh` (if such a script is introduced) that rejects fetched content not matching expected markers (e.g., expected H1 headings, expected code-fence languages). Until then, the trust assumption is stated explicitly and adopters can audit.
