...
| 2026-07-09 | **Brought PR #134 up to date with master (through PR #135)** | -- | Merged origin/master to clear the `BEHIND` state. Master's delta is entirely within `.aid/` (housekeep KB-DELTA refresh for the connectors subsystem + release drift across 15 KB docs + kb.html, `settings.yml`, and the prune of the concluded `work-001-add-deliveries-folder`); no `canonical/` or `profiles/` change → no re-render or dogfood resync. Clean auto-merge; only `pipeline-contracts.md` touched by both sides with disjoint hunks, both kept (ours: L9/L10 Monitor→/aid-fix // /aid-triage; master: ELICIT E1/E2 External-Documentation + `.aid/connectors/` registry rows + 1.3 change-log). Merge commit; PR now MERGEABLE, awaiting CI green |
| 2026-07-09 | **Documentation + KB + kb.html sync — folded into PR #134 (owner-directed)** | -- | Brought ALL hand-maintained docs current with work-001 before merge. **docs/**: aid-methodology.md (5 diagrams redrawn: 3-door entry, shortcut engine, L9/L10 re-point, SDD, workspace/build trees) + faq/glossary/repository-structure/install/diagram-content-reference. **site/**: hand-authored pages (index/lite-path/first-work/pipeline/maintainer/artifacts/overview) + regenerated concept/reference pages; generator build-fix (`gen-reference.mjs` 14→82 drift guard, data-driven shortcut families). **KB (full review, owner-requested):** 12 `.aid/knowledge` docs + README revision history + regenerated INDEX — 82-skill taxonomy, recipe removal, /aid-triage + shortcut engine, BLUEPRINT/DETAIL + flattened-Lite (no per-task STATE.md), Monitor re-point, test-count 105→118, global gate A→A+, 6 ADRs D20–D25 (+D14 superseded), tech-debt L7/M2/L1 closed + L8/L9 added. **kb.html:** 24 text edits (21/21 validated). Process: 5 doc authors + 6 aid-researcher KB reviews + 2 adversarial aid-reviewer gates; all findings applied (incl. a fact-sheet Lite-tree error caught by review + fixed across tree blocks + prose). Commit a9bc767d; Docs-CI MDX brace fix (pipeline.mdx) 133d17fd. SURFACED for a separate pass: 5 canonical-code gaps (aid-monitor/deploy task SPEC.md; 11 dangling `§ Task SPEC.md` cites; 2 stale templates; generate-profile 14-skill VALIDATE) + a pre-existing ~359-occurrence `canonical/(templates|scripts)` (missing `/aid/`) path drift |
| 2026-07-09 | **Merged master (PR #136) + dashboard reader: represent flattened/shortcut works** | -- | Merged origin/master (PR #136 dashboard fixes: render lane-less tasks directly / drop empty Unsequenced group; reap Windows server process tree) — disjoint from the doc set, conflict-free (merge d8cc44df). Dashboard reader twins (`reader.py` + `reader.mjs`, lockstep): **FIX 1** — default `work_path` to `lite` for flat works (a shortcut work authors no `## Triage → Path`), fixing home.html's `[Identifying path]` mislabel + phantom `Delivery #1` wrapper + missing Lite badge (NO home.html edit). **FIX 2** — detail-view gate parser falls back to the flat singular `## Delivery Gate` (additive; plural parsing provably unchanged). Tests: `work_path=lite` parity-enforced + 3 gate cases + node parity → 129 pass (1 pre-existing Windows 8.3 `TestLocator` flake). `packages/**` + `home.html` untouched (release-time vendored). Overall coherence check (cross-layer / render-dogfood / master-merge) in flight |
| 2026-07-09 | **Overall coherence check — PASS (3 parallel audits)** | -- | Cross-layer: docs/KB/site/kb.html/root-README tell ONE coherent work-001 story (C1–C7 consistent). Render/dogfood: profiles byte-identical to canonical (`verify_deterministic` PASS, 310 files/profile), dogfood `.claude/` byte-identical (`test-dogfood-byte-identity` 625/0), 82 skills in all 5 profiles, recipes absent, zero orphans, generators clean, INDEX fresh. Master-merge: work-002 connectors + PR #132 deliveries + PR #135 KB-delta + PR #136 dashboard/bin all preserved + reconciled; work-001 artifacts consistent (41/41 tasks, 4/4 A+, Completed); merge history sane; PR MERGEABLE. Residual = canonical-source rename drift only (+ a material profile-README miss) |
| 2026-07-09 | **Canonical rename-propagation cleanup + re-render (owner: fix-all-now)** | -- | Fixed the residual work-001 rename drift in `canonical/`: aid-monitor/aid-deploy SKILL.md per-task `SPEC.md`→`DETAIL.md`; 11 `§ Task SPEC.md`→`§ Task DETAIL.md` cites (shortcut-engine ×3 + 6 scaffolding files); work-state-template per-task `SPEC.md`→`DETAIL.md`; task-state-template lite per-task STATE.md reworded (flat layout has none); requirements-template location →work root; writeback-state.sh comment (code already correct); generate-profile VALIDATE 14→82 (dogfood-only). Feature + work-root `SPEC.md` deliberately left intact. Re-rendered all 5 profiles (`run_generator.py`, 310 files each, deterministic gate PASS); surgically resynced 13 dogfood `.claude/` files; **byte-identity 625/0**, `verify_deterministic` PASS, `git grep 'Task SPEC.md'` = 0 (render self-test T7 exec-bit = pre-existing Windows `core.filemode=false` artifact). Plus 5 profile READMEs → 82-skill taxonomy + recipe-bullet removal + stale diagram-ref TODO (commit c90cbb47). STILL TRACKED (separate pass, NOT this): pre-existing ~359-occurrence `canonical/(templates|scripts)` missing-`/aid/` path drift |
| 2026-07-09 | **Path-drift cleanup (`canonical/{templates,scripts}` → `canonical/aid/`) + CI-red fix — v2.1.0 quality pass** | -- | Owner directive: fix the pre-existing missing-`/aid/` path drift too (quality build). Fixed ~1088 occurrences across 88 canonical + 13 tests + 4 dashboard (docstrings; real `Path()` code already correct) + examples/bin → re-rendered (35 profile files) + resynced 7 dogfood `.claude/`. Preserved the BY-DESIGN flat form: `render.py` re-flattens the manifest `src` field for stability; `render_lib.rewrite_install_paths` treats flat==nested + skips comments; so `EMISSION-MANIFEST.md` examples, `generated-files.txt` data lines, `render_lib.py` test fixtures, and the 2 reorg-history test comments were left flat. Found + fixed a real narrow functional bug: `cleanup-classify.sh:193` read `canonical/templates/generated-files.txt` (wrong) when the raw canonical script runs against the monorepo root; + no-trailing-slash misses that had broken a housekeep unit. **ALSO fixed the canonical suite that had been RED since the KB refresh (a9bc767d):** `test-deploy-monitor-repurpose` DMR05a/DMR05c asserted the old `Fix (bug)`/`Triage (CR)` labels, but the KB now binds L9/L10 to `/aid-fix` // `/aid-triage` → test updated to match. Verified local: byte-identity 625/0, verify_deterministic PASS, targeted suites green (compute-block-radius/read-setting/grade/housekeep×3); full suite → CI. (Windows-local `test-version-sync` 12 fails + `housekeep-workfolder` Unit 10 = pre-existing node/`/tmp` + gh env artifacts, stash-confirmed, not regressions.) |
| 2026-07-09 | **Security hardening — v2.1.0 "harden everything now" (5 subsystems)** | -- | Owner-directed full security pass. 3 adversarial reviews (secrets/deps · dashboard-server/installers · scripts/reader) → secrets SAFE, product dep-exposure ZERO, no RCE/traversal/injection; found 1 HIGH + MEDIUMs + LOWs, all fixed: **(reader twins)** HIGH `git` option-injection via `kb_baseline.branch` + LOW via `approved_at_commit` → `--end-of-options` (was: arbitrary file create/truncate from merely viewing a malicious repo's dashboard); 5 MB bounded-read across all 44 read sites (anti-DoS); TTL freshness cache (doc-freshness cache correctly skipped w/ reasoning). **(server twins)** `Host`-header allowlist → 403 (kills DNS-rebinding exfil) + `nosniff`/CSP on every response; `escHtml` `'`→`&#39;` (+ dogfood mirror); confirmed CLI/installer-tests use loopback `Host` (no CI break). **(installers)** fail-closed checksum (missing `SHA256SUMS` → abort, was warn-and-proceed) + endpoint override gated behind `AID_ALLOW_ENDPOINT_OVERRIDE`. **(canonical)** writeback octal `10#` (tech-debt L8) + `kb-freshness-check.sh` git guard → re-render + resync. **(site)** astro 6.4.6 / dompurify 3.4.11 / esbuild 0.28.1 → `npm audit` 0 vulns (all 5 Dependabot alerts, docs-site-only). Verified: byte-identity 625/0, `verify_deterministic` PASS, reader 554 pass + Python↔Node parity, server SEC-6 tests, installer valid/missing/tampered scenarios; the ~12+9 local failures are pre-existing Windows node/`/tmp`/registry env flakes (all stash/pop A-B baseline-confirmed). Recovered a shared-worktree `git stash` scare (a subagent stashed repo-wide) with **zero loss** — all 39 files + byte-identity intact. FOLLOW-UP (flagged, needs infra, NOT this): cryptographic release SIGNING over SHA256SUMS (`release.sh --sign` is a deliberate stub) + the LOW git-poll doc-freshness cache. Full CI pending. |
| 2026-07-09 | **Coverage-gap closing — 5 new shortcut families + `/aid-ask` + triage Q&A route (v2.1.0)** | A+ | Owner directive "close all gaps now" + "recover /aid-ask as an alias to /aid-query-kb". Verified the intent (shortcuts cut red tape / reserve the full path for complex work; specialization shortens spec→execution; `/aid-triage` guides the unsure; broad common-task coverage without losing quality structure) was **achieved and documented** (README + methodology narrative), then closed the surfaced coverage gaps. **Catalog** 69→**80 rows** (+6 canonical: `aid-review`, `aid-remove`, `aid-deprecate`, `aid-migrate`, `aid-research`, `aid-query-kb`; +5 alias: `aid-audit`, `aid-delete`, `aid-investigate`, `aid-spike`, `aid-ask`); repurpose 2→**4** (`aid-deploy`/`aid-monitor`/`aid-query-kb`/`aid-ask`). **Dirs** 82→**92** (+10 new doorways; `aid-query-kb` already had its classic dir). Framing everywhere: **92 = 14 classic + `/aid-triage` + `/aid-ask` + 76 shortcuts across 14 families; 80-row = 51 canonical + 29 alias, 4 repurpose.** Wired verbs into `shortcut-engine.md` (Default-Type Mapping A-6 +5; verb→family) + `shortcut-scaffolding/{analyze-report,change-refactor}.md`; `research/investigate/spike` correctly placed in **G11 RESEARCH** (not G3 DESIGN). `/aid-ask` hand-authored as a thin `repurpose:true` alias delegating fully to `aid-query-kb` (generator skips it). Triage gained a **QUESTION-intent branch** (Step 0 → SUGGEST Case D `/aid-ask`) with the canonical-names-only rule scoped to Step-3 catalog match + explicit `/aid-ask` exception. Re-rendered all 5 profiles + resynced dogfood; `gen-reference.mjs` reports "14 classic + aid-ask"; regenerated site reference docs. Full doc/KB/kb.html/profile-README **count-sync to zero current-truth stragglers** (dated changelog/revision entries + `.aid/work-001` build artifacts correctly frozen); INDEX regenerated. A+ gate closed 8 findings (stale gen-reference test 82/67, triage Case-D alias-vs-canonical rule, stale VALIDATE count, missing tests, stale comments/KB counts). Verified local: catalog↔dir parity reconciles (80/51/29/4, 92 dirs), **byte-identity clean** (only maintainer-only `generate-profile` extra), `gen-reference` vitest **36/36**, full site `.mjs` suite **87/87** (`.ts` suites deferred to CI — local `site/node_modules` absent). Full CI pending. |
| 2026-07-10 | **Tech-debt full re-validation + safe cleanups (v2.1.0 quality)** | -- | Owner asked to validate ALL debt items against disk/CI evidence, not relay the KB. Found the inventory carried ~4 already-done items as "open": **M1** (npm+PyPI publish jobs SUCCEED on the v2.0.6 release run — channels live, never "blocked/GitHub-only"), **M4** (visual gate already asserts no-overflow at 732px+390px — the proposed T4 remediation is implemented), **L8** (all `printf '%03d'` sinks in `writeback-state.sh` are `$((10#$id))`-normalized), **L9** (generate-profile VALIDATE now catalog-derives "92"). Reconciled `tech-debt.md`: removed M1/M4/L6/L8/L9; reframed **L2** as accepted/won't-do (npm `--provenance` + PyPI PEP 740 sigstore attestations already cover the channels users install from + `SHA256SUMS` on tarballs; a detached GPG signature is key-management burden for marginal gain — revisit only if threat model changes); narrowed **M3** to the remaining no-CI-count-guard gap; updated **L3** (deprecation window closed → excise as its own installer-CI-gated change); removed the obsolete zero-padded-id gotcha; added Change Log rev 1.9. **Remaining open: H1, M3, L3, L4.** Applied the safe fixes same pass: **M3** EMISSION-MANIFEST profile table 3→5; **L6** DBI orphan-scan skips gitignored `node_modules/`/`.git/`; **L5(a)** em-dash→`--` in summarize/writeback-state.sh (re-rendered 5 profiles + resynced dogfood — byte-identity clean); generate-profile framing "15 classic"→"14 classic + aid-triage + aid-ask". Deferred with reasons: H1 (M-effort refactor, test-guarded), L3 (installer change needs own CI gate), M4 (already done), L5(b) (by-design banner-flagged reference), L4 (accepted gap). Superseded dangling stash left in place (drop blocked as irreversible-destructive; benign, owner can drop). Full CI pending. |
