...
- **State:** Done
- **Review:** self-verified: bash tests/canonical/test-change-refactor-family-scaffold.sh -- 89/89 passed
- **Elapsed:** delivery-002 test cluster (Dev E: 016+018+020)
- **Notes:** tests/canonical/test-change-refactor-family-scaffold.sh created: change-refactor.md contract assertions (change inherits create.md's artifact matrix by reference, refactor-kind closed 3-enum, the 3 refactor-kind task templates, Ownership-boundary behavior-changing-vs-not split), alias-equivalence + bare-verb contract against the real catalog+dirs (aid-update-api == aid-change-api {verb,artifact}; aid-refactor stays bare -- no alias, no suffix), aid-change-data-model (MIGRATE/IMPLEMENT/TEST) and aid-refactor performance-mode (REFACTOR+TEST)/rename-mode (single REFACTOR) fixture-shape halt-proofs, and a byte-identical work-shape diff proof for aid-update-api vs aid-change-api (AC-1).
