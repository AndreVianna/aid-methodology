# task-012: Real SUMMARY-DELTA body — `state-summary-delta.md` (replaces the stub)

**Type:** IMPLEMENT

**Source:** feature-003-summary-delta-refresh → delivery-002

**Depends on:** task-001, task-002, task-004, task-005, task-009

**Scope:**
- **Replace** the delivery-001 stub no-op at
  `canonical/skills/aid-housekeep/references/state-summary-delta.md` (authored by task-005) with
...
- [ ] The full sequence now terminates `KB-DELTA → SUMMARY-DELTA → CLEANUP → DONE` with
  SUMMARY-DELTA exercising real logic (the swap is asserted by task-010's integration suite — see
  that task's scope; no new dedicated suite per feature-003 SPEC § Testing).
