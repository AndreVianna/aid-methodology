# task-012: Real CLEANUP body — `state-cleanup.md` (replaces the stub)

**Type:** IMPLEMENT

**Source:** feature-004-aid-cleanup → delivery-003

**Depends on:** task-001, task-002, task-003, task-010, task-011
...
- **Replace** the delivery-001 stub no-op at
  ...
  **real CLEANUP body** — step-numbered prose in the style of
  ...
  (Mode=cleanup-only via task-011).
- **Step 1 — scan + classify:** invoke `canonical/scripts/housekeep/cleanup-classify.sh`
  (task-010) to get the candidate list; the body does NOT reimplement scan/tier/matrix/split
...
- **Step 3 — per-item confirm (NFR1):** the user toggles items and confirms the final selection;
  **no item is removed without appearing checked at confirm time.** Each `(i)✓/(ii)✗` work folder
  (gate=explicit-confirm from task-010) is NOT in the togglable list — it gets its own
...
- **Step 4 — apply deletions, tracked/untracked split** (feature-004 SPEC § "Deletion
  ...
  per-path classification from task-010; `rm -rf` each `to_rm` path and `git rm -r --quiet` each
...
- [ ] `state-cleanup.md` no longer reads as a stub no-op: it invokes `cleanup-classify.sh`
  (task-010) for candidates, presents the tiered `AskUserQuestion` checklist (Tier-0 pre-checked,
...
- [ ] The deterministic logic this body wires is covered by task-010's classification suites and
  the cross-stage transitions by the integration TEST (task-013); the LLM-authored prose body
