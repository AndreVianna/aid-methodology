# task-007: `--cleanup-only` enablement in SKILL.md `## Arguments` + State Detection routing to CLEANUP
...
- Wire the State Detection / dispatch routing (feature-001 § Feature Flow / Dispatch + § Resume,
  ...
  body (task-008) directly. Do NOT re-implement CLEANUP's body here — only the entry routing in
  `SKILL.md` prose.
...
- **Type rationale (IMPLEMENT, not REFACTOR):** accepting a previously-rejected flag and routing
  ...
  refactor). The row-2 routing now lives in `SKILL.md` prose (no parse-args unit suite); it is
  verified by dogfooding + render-drift CI — there is no bespoke integration test (AID has no
  E2E tier).
...
- [ ] The row-2 routing precondition (Mode=cleanup-only ⇒ CLEANUP entry, KB/Summary fields
  untouched) is verified by dogfooding + render-drift CI — no bespoke integration test (AID has
  no E2E tier); the delivery-001 state/resume + branch-commit suites remain green.
