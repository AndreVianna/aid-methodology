# task-007: `state-kb-delta.md` — KB-DELTA body (detection → scope → delegation → writeback)
...
- [ ] Unit coverage of the new deterministic logic lands with its scripts (task-004/005); the
  ...
  (task-008), consistent with the no-E2E-tier policy (test-landscape.md) — there is no runtime
