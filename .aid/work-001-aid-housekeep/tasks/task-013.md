# task-013: Cross-stage integration TEST — full `KB-DELTA → SUMMARY-DELTA → CLEANUP → DONE` + `--cleanup-only` entry

**Type:** TEST

**Source:** feature-004-aid-cleanup → delivery-003

**Depends on:** task-001, task-002, task-008, task-010, task-011, task-012

**Scope:**
- **Extend** the delivery-001 integration suite `tests/canonical/test-housekeep-flow.sh`
  (task-008, auto-discovered by the `tests/canonical/test-*.sh` glob, sourcing
  ...
  task-011), NOT the LLM prose bodies.
- **Stub→real-body swap (CLEANUP):** assert the full sequence now terminates
  ...
  delivery-001 stub `**Cleanup Stage:** skipped` is replaced by task-012's `passed`). With the
  ...
  `**State:** DONE`. This is the delivery-003 analogue of task-008's delivery-002 forward-compat
...
- **`--cleanup-only` entry (AC10, feature-001 row 2):** assert that the `SKILL.md` `## Arguments`
  + State Detection routing (task-011) surfaces `**Mode:** cleanup-only` and routes PREFLIGHT →
...
- **Coverage boundary (no double-ownership):** this suite owns only the **cross-stage state
  ...
  `cleanup-classify.sh` in task-010 (`test-housekeep-classify.sh`,
...
- [ ] Asserts `--cleanup-only` surfaces `**Mode:** cleanup-only` via the `SKILL.md` State
  Detection routing (task-011), routes PREFLIGHT → CLEANUP directly (KB/Summary fields untouched,
...
- [ ] Asserts a ≥1-item cleanup makes **exactly one** commit on `aid/housekeep-*` (tracked via
  ...
  (AC8).
- [ ] Does NOT duplicate task-010's classification/matrix/split unit assertions (those remain the
