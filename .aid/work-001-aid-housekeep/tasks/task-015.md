# task-015: Real CLEANUP body — `state-cleanup.md` (replaces the stub)

**Type:** IMPLEMENT

**Source:** feature-004-aid-cleanup → delivery-003

**Depends on:** task-001, task-002, task-004, task-005, task-013, task-014

**Scope:**
- **Replace** the delivery-001 stub no-op at
  `canonical/skills/aid-housekeep/references/state-cleanup.md` (authored by task-005) with the
  **real CLEANUP body** — step-numbered prose in the style of
  ...
  (Mode=cleanup-only via task-014).
- **Step 1 — scan + classify:** invoke `canonical/scripts/housekeep/cleanup-classify.sh`
  (task-013) to get the candidate list; the body does NOT reimplement scan/tier/matrix/split
...
- **Step 3 — per-item confirm (NFR1):** the user toggles items and confirms the final selection;
  **no item is removed without appearing checked at confirm time.** Each `(i)✓/(ii)✗` work folder
  (gate=explicit-confirm from task-013) is NOT in the togglable list — it gets its own
...
- **Step 4 — apply deletions, tracked/untracked split** (feature-004 SPEC § "Deletion
  ...
  per-path classification from task-013; `rm -rf` each `to_rm` path and `git rm -r --quiet` each
...
- **`--cleanup-only` inputs guard** (feature-004 SPEC § "`--cleanup-only` Entry", AC10): when
  ...
  own `STATE.md`. It does NOT read `.aid/knowledge/STATE.md **Approved-At-Commit:**` (feature-002's)
  nor any `**Summary Stage:**` field. The C1 predecessor gate is feature-001's (satisfied by the
  Mode=cleanup-only deliberate-skip path); this body does NOT re-implement the gate.
...
- [ ] `state-cleanup.md` no longer reads as a stub no-op: it invokes `cleanup-classify.sh`
  (task-013) for candidates, presents the tiered `AskUserQuestion` checklist (Tier-0 pre-checked,
...
- [ ] `--cleanup-only` (Mode=cleanup-only) path reads no KB/summary run-state (no
  `**Approved-At-Commit:**`, no `**Summary Stage:**`); its only inputs are the filesystem scan,
  git, `gh`, and each work folder's `STATE.md`.
- [ ] The body does not touch `run_generator.py` (D2 fix untouched) and sweeps only residual
  stray verify reports as S4 candidates.
- [ ] The deterministic logic this body wires is covered by task-013's classification suites and
  the cross-stage transitions by the integration TEST (task-016); the LLM-authored prose body
