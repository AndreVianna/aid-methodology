...
> **State:** Shipped — v2.0.3 released (PR #119 merged; tag v2.0.3; GitHub Release + npm + PyPI all live at 2.0.3)
> **Phase:** Execute
...
| 2026-07-05 | v2.0.3 SHIPPED | -- | PR #119 merged to master (e949bc11). Dry-run (release.yml, ref=master) green: gate + github-release + npm + pypi all success. Annotated tag v2.0.3 pushed → release.yml run 28726881862 SUCCESS: GitHub Release "AID v2.0.3" (not draft, 9 assets: 5 profile tarballs + aid-cli + AidInstallCore.psm1 + aid-install-core.sh + SHA256SUMS), release notes applied. Registries confirmed live: npm aid-installer=2.0.3, PyPI aid-installer=2.0.3. Follow-up noted (not blocking): check-version-sync.sh aborts under set -e on Windows Git Bash because Windows node.exe can't require() a /c/... MSYS path (never reaches the python3 fallback) — release gate runs on ubuntu so unaffected; local Windows use only. |
| 2026-07-04 | Copilot PR review resolved | -- | Copilot posted 2 inline comments on PR #119. (1) closure-check.sh:522 `mapfile` requires Bash 4+ / breaks Bash 3.2 — premise FALSE against disk evidence: AID canonical scripts already require Bash 4+ repo-wide (mapfile in kb-citation-lint.sh + migrate-work-hierarchy.sh; `${x,,}` in complexity-score/writeback-state/cleanup-classify; `declare -A` in 4 scripts). Single-fixing closure-check would be incoherent theater. Replied with evidence + resolved as not-applicable. (2) test-closure-batching.sh:107 — VALID: on a T01 hang (timeout RC=124) fail() doesn't exit, so T04 re-runs closure-check unguarded and would wedge CI. Fixed (commit dbd98c80): on RC=124 the suite now `test_summary; exit 1` (skips T02/T03 partial-output asserts + unguarded T04 rerun); fails fast on regression; 7/7 still pass. Replied + resolved. |
