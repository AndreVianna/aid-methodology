...
| task-010 | In Review | -- | -- | EXECUTE complete (commit a024e5f6): Phase enum → Describe/Define/Specify/Plan/Detail/Execute/Deploy both twins; Discover surfaced from KB state (not a work phase); Deploy=post-Execute pill; dead Monitor removed; back-compat read alias Interview→Describe (Monitor→Unknown); writers reject old values. home.html full stepper rebuilt + Lite rail driven by real lifecycle (not stale index) + kept DISTINCT. Template + writeback validation + 3 skill write-sites migrated; on-disk STATE + fixtures + reader tests migrated; re-render + re-vendor. Verified: pytest 625 passed / 12 baseline failures (no new); test_index_html 178/178; both twins map new+legacy; home.html byte-identical + node --check; test-writeback-state 334/334. FLAGGED task-004 regression: test-pipeline-status-walkthrough.sh + test-disjoint-merge.sh assert task-004's OLD prose format → fixing separately. Review pending. |
