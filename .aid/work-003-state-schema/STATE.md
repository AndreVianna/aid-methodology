...
| task-008 | In Review | -- | -- | EXECUTE complete (commit 4b54e551): bare top-level `--version`/`-V` in both bin/aid + bin/aid.ps1 via a single-sourced VERSION-read helper (`_aid_print_version` / `Show-AidVersion`), intercepted before subcommand dispatch. Independently verified: --version/-V/version all print 2.1.0 exit 0; subcommand `--version <v>` pin unshadowed (still requires a value). `aid -h` lists both flags distinctly. Tests: test-aid-cli.sh CLI027-M1 + test-aid-cli-ps1.sh PS028-M1 (9+9 assertions). Review pending. Ship (re-vendor+reinstall) folded into the final gate. |
