...
| task-008 | In Progress | -- | -- | STARTED — add a `--version` flag to the aid CLI (bin/aid + bin/aid.ps1), aliasing `aid version`, documented in `aid -h`; handle the `--version <v>` install-pin collision + VERSION-file path. Marked In Progress before starting work (per the task-009 mandate). |
