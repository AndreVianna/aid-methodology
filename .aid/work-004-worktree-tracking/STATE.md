...
| 11 | Reader (Py): same-work reconcile | IMPLEMENT | 5 | Pending | — | — | deps 009,010; ALSO fix wave-4 MEDIUM #3 (state_text_cache bare-work_id last-wins → proper cross-root merge) |
...
| 14 | Reader fixtures: hierarchy/legacy/multi-worktree/reconcile | TEST | 6 | Pending | — | — | deps 009-011,013; ALSO cover task-010 worktree-enum (wave-4 LOW #4: zero coverage today) |
| 15 | Cross-cutting: disjoint-merge proof, parity, render-drift, run-all | TEST | 7 | Pending | — | — | deps 004,007,008,012,013,014; MUST: final FULL regen + dogfood sync (waves 4-6 deferred theirs) + render-drift; fix test-writeback-state.sh (91 fails: --field State, ## Pipeline State, per-unit targets) + suites broken by schema change; relocate migration fixture out of canonical→tests (wave-3 LOW); sweep nits (schemas FK note, ER-diagram, partial-ASCII skill md) |
