...
| 2026-06-23 | retrofit op-sections into seed templates (delivery-009 follow-up) | A+ | Whole-work-review gap closed: f013 act-back gate requires `## Conventions`/`## Invariants`/`## Gotchas`/`## Contracts` as first-class greppable sections, scoped per concern-model.md owning-table; but the shipped seed templates (`canonical/aid/templates/knowledge-base/*.md`) carried none. Added the owning-table-mapped section to each owner doc (coding-standards=Conventions; module-map=Conventions+Invariants; pipeline-contracts=Conventions+Contracts; architecture=Invariants; domain-glossary=Invariants[conceptual]; tech-debt=Gotchas; schemas=Contracts; integration-map=Contracts) -- no over-application (glossary has no Contracts). Verified: `kb-actback-task.sh check` over the seed set reports all owner classes `present`, 0 false-absent. Regen (run_generator, byte-stable 2nd run) + .claude sync; DBI 553/0, ascii 28/0. Did NOT touch live `.aid/knowledge/` docs (next task) or the act-back script/concern-model (the spec) |
| 2026-06-23 | retrofit AID's OWN live KB (delivery-009) | A+ | Both verified whole-work-review gaps closed on AID's dogfood KB (`.aid/knowledge/`). GAP 1 (act-back): re-headed/consolidated EXISTING operational content into exact greppable `^## Class$` sections per the concern-model owning-table -- coding-standards=Conventions; module-map=Conventions+Invariants; pipeline-contracts=Conventions+Contracts; architecture=Invariants; domain-glossary=Invariants(conceptual spine); tech-debt=Gotchas; schemas=Contracts; integration-map=Contracts. `kb-actback-task.sh check` over `.aid/knowledge/`: BEFORE 10/10 expected sections ABSENT → AFTER all 10 `present`, 0 false-absent. GAP 2 (INDEX routing): added genuinely-useful `tags:`/`audience:`/`see_also:` routing frontmatter to all 16 in-scope primary docs; regenerated INDEX via canonical `build-kb-index.sh` → Tags/See-instead/Audience columns now populate all 16 rows (see_also links non-dangling). Verified: lint-frontmatter 0 findings; test-dogfood-byte-identity 553/0; KB suites (actback/build-index/frontmatter-lint/extract-list) all green. KB-content edits only -- no canonical/generator/DBI touched |
| 2026-06-24 | simplify review panel 6→4 mandates (delivery-009) | A+ | Whole-work over-complexity review simplification. Panel collapsed 6→4 mandates with NO coverage loss. DROPPED M3 Concept-closure (redundant: term-grounding is already mechanically gated by `closure-check.sh` output-(a) as the GENERATE closure-loop termination oracle + FR-32 cap-trip escalation; a reviewer restating a mechanical gate adds nothing — PROVEN: ungrounded `tokenrouter` still fires output-(a)>0 → loop cannot CLOSE, without M3). FOLDED M5 Calibration into M2 Anatomy (transcription/hollow/coverage/deferral become M2 reviewer judgments from doc text + output-(b)) and DROPPED `closure-check.sh` output-(c) (the lexical transcription-ratio "determinism theater" — a noisy proxy the reviewer judges anyway; transcription substrate fixture kept, now Anatomy-judged). RENUMBERED: teach-back M4→M3, act-back M6→M4 (keystones unchanged). Consumers updated grep-clean: state-review (6→4 full / 3-dispatch collapsed M1-M2 seq), reviewer-prompt index, reviewer-prompt-anatomy (absorbs altitude), deleted concept-closure+calibration FOCUS bodies, path-config, review-rubric, state-closure, state-generate, kb-actback-task.sh label, update-kb state-review/apply/done, housekeep state-kb-delta. closure-check.sh now ships outputs (a)+(b) only. Tests: test-closure-check (C06/C07 output-c removed → 9/0), test-calibration-fixtures (mechanical CAL-1 ratio T02/T04+CAL1_FLOOR removed; coverage T01/T03 kept; added T07 transcription-substrate-shape → 6/0), test-teachback-fixtures (--output-c dropped → 13/0), test-actback-fixtures + fixtures (M6→M4 → 14/0). Verified: regen byte-stable (idempotent 2nd run) + .claude sync; DBI 549/0 (was 553; −2 deleted FOCUS bodies dropped from manifest); ascii 28/0; run-all green except pre-existing install-parity flake (unrelated — references none of my files; passes standalone 84/0 on baseline AND with my changes; run-all-ordering/pwsh env flake) |
| 2026-06-24 | aid-detail feature-014 / delivery-010 (lite path) | -- | **Domain-driven discovery** feature added to work-001 per user direction: generalize `/aid-discover` beyond software to **any digital work**. Lite path define+specify+plan(single-delivery)+detail authored: REQUIREMENTS §5.J (FR-37–FR-44) + §10 Must entry; `features/feature-014-domain-driven-discovery/SPEC.md` (requirements + full technical spec); PLAN.md delivery-010 stanza + execution graph (8 tasks task-056..063 across 5 waves); `delivery-010/SPEC.md` + `STATE.md` (State: **Specified**; 2 open Q&A — Step 0f path-triage reconciliation, Decisions-concern promotion); 8 task SPEC/STATE pairs. Four pillars: generic-core spine (standards-grounded) · source-driven domain classify (user-on-uncertainty) · matrix-or-research doc-set + matrix-lifecycle (no auto install→canonical) · dual-audience authoring standard (single-concern/junior-clear/tables-no-diagrams/classified/frontmatter→index→content→changelog). Grounded in a 3-part standards research pass (arc42/C4/IEEE1016/ISO42010/ADR = product→KB; PMBOK/PRINCE2/Scrum = governance→pipeline). Branch `aid/work-001-delivery-010`. **NOT YET EXECUTED.** |
| 2026-06-24 | feature-014 / delivery-010 PLAN A+ gate | A+ | Adversarial clean-context `aid-reviewer` graded the lite-path plan (REQUIREMENTS FRs + feature SPEC + PLAN stanza/graph + delivery SPEC/STATE + 8 task SPECs). Clean on: DAG acyclicity, numbering, reference integrity, `synth_default_seed` byte-stability, scope discipline, gate criteria. **6 findings → D+** (1 HIGH/2 MED/2 LOW/1 MINOR), all author-side consistency: (HIGH) Decisions-vs-byte-stable-seed contradiction → reconciled (Decisions = 11th dimension realized **conditional**; 15-doc seed unchanged; consistent whether Q2 promotes/declines); (MED) T2-contract lockstep now owned by task-056; (MED) task-058 type-purity → Step 0f **Q1 RESOLVED** (path stays in discovery), task-058 now pure IMPLEMENT; (LOW) §7 reframed RESOLVED; (LOW) FR-37 stakeholders=interview-side meta; (MINOR) task-063 cite FR-2→FR-44. **All 6 Fixed → grade.sh A+ (0 open).** Ledger: `.aid/.temp/review-pending/feature-014-plan.md`. Plan is gated A+ and ready for /aid-execute (still NOT executed). Q2 (Decisions promote) left as user confirm — plan consistent either way. |
| 2026-06-24 | feature-014 / delivery-010 EXECUTED + A+ delivery gate | A+ | All 8 tasks (056–063) executed via aid-execute pool dispatch (5 waves). Built: concern-model generalized to the **11-dimension spine** (Decisions conditional, T2 10→11, standards-cited); **`domain-doc-matrix.md`** (8 domains; software row == 15-doc seed); source-driven **domain classifier** (Step 0cx) + **matrix-or-research** Step 0d rewire; **self-bootstrap STATE**; **dual-audience authoring standard** wired into kb-authoring + 15 templates + agent-prompts + Anatomy mandate; **3 new test suites** (matrix/self-bootstrap/authoring-standard, 137+ assertions incl. AS07). Cornerstones: regen 1365 files deterministic VERIFY PASS, `.claude` synced **DBI 551/0**, **75 canonical suites** + ASCII green, `synth_default_seed` byte-stable. Adversarial **Large-tier delivery gate**: 6 findings (2 HIGH / 3 MEDIUM / 1 LOW) → all **Fixed** in one cycle (label-drift normalize · 13 templates concern-tagged + orientation-exempt · P10 cross-ref · FM-severity split · AS07 test · software-* fallback) → **grade.sh A+ (0 open)**. Branch `aid/work-001-delivery-010`. |
...
| 13 | feature-013-operational-sufficiency | Drafted | — | 0 | **NEW (2026-06-23, post-detail):** the "act-back" gate — operational sibling of teach-back. Adds a 6th panel mandate (M6) to f005: clean-context agent given ONLY the KB + a representative project task must produce a correct plan AND flag every KB-insufficiency point ([HIGH] [ACTBACK] rows in the merged ledger; sibling keystone to teach-back; reuses f005 panel + grade.sh, NO new grading infra). Tightens f003 doc model (conventions/invariants/gotchas/contracts = first-class greppable structure). 1 new script (kb-actback-task.sh: representative-task selection + operational-structure presence check). Fixture built+exercised by f012 (V-E family + AC16 judgment-boundary row). FR-36 + AC16 (Must). Awaiting A+ gate |
| 14 | feature-015-summarize-domain-driven-redesign | Ready | — | 0 | **NEW (2026-06-25):** realign `/aid-summarize` to the domain-driven KB + reframe `kb.html` as a **non-technical-newcomer, visually-rich** product (the KB no-diagrams rule does NOT apply). FR-45–FR-51 (§5.K): doc-set/domain-driven sections (retire project-TYPE profiles, kill phantom `repo-presentation.md`, derive `noscript`) · concept-first content components (glossary/ADR/capability rendered, not linked) · best-format-per-fact + completeness grading (remove the C+/diagram-count cap) · newcomer tone · page-shell consistency with home.html/index.html · data-driven deterministic generation · pre-render inline-SVG visuals + DROP the 3MB Mermaid engine + a NEW §7 visual-fidelity gate (Playwright readable-text/minimal-overlap/correct-layout, replaces Mermaid's render check). Guardrails C1/C2/C3/C5/C6 + §5b. **Two deliveries:** D-011 correctness-core (Changes 1-5) → D-012 visual & engineering (Changes 6-7 + §7 gate); D-012 depends on D-011. Server gzip/cache = fast-follow, OUT. SPEC + technical-spec authored; delivery-011/012 SPEC/STATE created (State: Pending-Spec). Task breakdown handed to `/aid-detail`. Source: `.aid/design/aid-summarize-redesign.md` |
| 16 | feature-016-discover-dual-intent-self-eval | Ready | — | 0 | **NEW (2026-06-25):** make `/aid-discover` produce *useful* KBs off-software (not only AID's two dogfood domains) by lifting everything domain-specific **filename-keyed → spine-dimension-keyed** + deriving the self-eval probes from the project's own source + capabilities. FR-52–FR-56 (§5.L): spine-keyed depth contracts (closes the dangling-anchor gap `document-expectations.md` left for the ~20+ non-software matrix filenames) · spine-keyed safeguard + C9-derived task generation (makes `kb-actback-task.sh`'s owning-table + task selector fire off-software, not degrade to "add an endpoint" / empty presence check) · the **Dual-Intent KB Self-Evaluation** — Blind Work-Simulation (assertiveness gate, Intent 1, generalizes M4 act-back) + Blind Reconstruction & Source Confrontation (essence gate, Intent 2, generalizes M3 teach-back), both domain-general REVIEW keystone gates (`[ACTBACK]`/`[FIDELITY]`/`[ESSENCE-GAP]`) with **no external test corpus** · altitude-rule signature exception (work-critical contracts stay inline). Proven by per-domain GOOD/SHALLOW fixtures + AID dogfood; consumes feature-014's spine (does not grow it); no new grading infra/agent enum (reuses f005 panel + grade.sh + the 7-col ledger). **Four deliveries:** D-013 depth → D-014 safeguard → D-015 self-eval → D-016 signature exception + dogfood; D-014→D-013, D-015→(D-013,D-014), D-016→D-015; feature depends on feature-014 (built). SPEC + technical-spec authored; delivery-013..016 SPEC/STATE created (State: Pending-Spec). Task breakdown handed to `/aid-detail`. Source: `.aid/design/aid-discover-dual-intent-self-eval.md` |
