# task-038: home.html per-doc suspect marker on the KB card (minimal UI)
...
- Update the existing whole-KB "Outdated" refresh-prompt copy (line 1730) to per-doc language
  ...
  `/aid-housekeep` call-to-action (which f010/delivery-008 consumes).
- `home.html` reads `kb_state.doc_freshness` / `suspect_count` **literally** (the existing
  "never re-derive client-side" rule, line 1584) -- the readers (task-037) do all derivation; the UI
  only displays.
- Augment-and-supersede (SPIKE-1, settled in task-037): the per-doc suspect marker becomes the
...
derivation (task-037 -- it consumes `kb_state.doc_freshness`/`suspect_count` literally), does NOT
change `index.html`, and does NOT build `/aid-housekeep` itself (f010/delivery-008 -- only the existing
