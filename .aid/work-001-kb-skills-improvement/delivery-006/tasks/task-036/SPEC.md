# task-036: test-kb-freshness-check.sh -- canonical staleness suite + fixtures
...
- The suite builds a throwaway **git fixture repo** (`git init` inside the `mktemp -d` scratch, with
  ...
  runs the SHIPPED `kb-freshness-check.sh` (task-035) over it, asserting each verdict class:
...
- Assert the **default `text` format** (task-035 ships `text` as the DEFAULT `--format`): a smoke
...
**Boundary:** f007 EXERCISES the task-035 script. This task does NOT author/edit
`kb-freshness-check.sh` (task-035), the dashboard readers (task-037), or the reader-parity suite
(task-039 -- the Python===Node parity gate is a separate suite). It asserts only the script's per-doc
