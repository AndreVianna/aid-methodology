# task-036: test-path-fixtures.sh (brownfield + greenfield-detection) + f006 SPIKE-T1 floor pinning

**Type:** TEST

**Source:** work-001-kb-skills-improvement -> delivery-006

**Depends on:** task-033, task-023 (delivery-004)
...
- Author `tests/canonical/test-path-fixtures.sh` -- the AC7 path-classification regression suite
  ...
  `exit $?`).
- The suite runs f006's SHIPPED `recon-classify.sh` (delivery-004) over the task-033 `paths/`
  ...
  - V-D1 -- **greenfield DETECTION (classification only).** greenfield (~0-source) fixture -> recon
    ...
    greenfield **path** assertion (formerly carved to the deleted greenfield-path delivery, the
    original pre-act-back delivery-009 -- now a defunct identifier; not the current Governance
    delivery-009) collapses into this detection-only check: recon-classify yields a greenfield
    verdict on the ~0-source fixture.
...
**path** assertion (formerly carved to the deleted greenfield-path delivery) collapses into V-D1's
detection-only check; **the original pre-act-back greenfield-path delivery and its two greenfield
path-runs tasks were deleted in the 2026-06-23 greenfield de-scope** and this suite references none of
them. (Those defunct identifiers were the pre-act-back greenfield-path delivery-009 and its tasks;
they must NOT be confused with the CURRENT live delivery-009 Governance (task-053/054/055), which is
unrelated and re-uses the now-recycled delivery number after the act-back insert.)
...
- [ ] No greenfield **path-runs / greenfield-closure** assertion is authored -- greenfield is detect-and-signpost, so V-D1 is the only greenfield assertion (classification only). No reference to the deleted greenfield-path delivery (the defunct pre-act-back greenfield-path delivery-009 and its tasks -- distinct from the current live delivery-009 Governance, task-053/054/055, which re-uses the recycled number).
