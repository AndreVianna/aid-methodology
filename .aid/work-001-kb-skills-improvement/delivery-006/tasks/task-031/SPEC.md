# task-031: 'Relative bus' + closed/unclosed-KB fixture corpus
...
- Author the hand-built, ASCII, checked-in fixture corpus that the AC2/AC3 essence-capture
  suite (task-034) runs over, under `tests/canonical/fixtures/kb-essence/`:
...
brownfield, task-033) and the AC1 teach-back pass/fail KBs (task-037) are OUT of scope for THIS task
(they are authored by sibling delivery-006 tasks, not here).
The numeric floor VALUES are not chosen here (that is task-034 / [SPIKE-V2]); this task only plants
...
- [ ] `relative-bus/` contains at least one incidental single-channel capitalized common-word phrase (E4 class, e.g. `The System`/`This File`) so the precision/noise-floor assertion in task-034 has noise to discriminate against.
...
- [ ] No fixture file is written or harvested at authoring time -- the trees are static read-only inputs (the harvest in task-034 runs over a `mktemp -d` copy, never these committed files).
