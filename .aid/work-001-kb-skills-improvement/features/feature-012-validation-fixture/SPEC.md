...
| AC16 | **operational-structure limb:** `kb-actback-task.sh` emits the representative task deterministically + byte-reproducibly (V-E1); operational-structure presence check reports all expected classes `present` for `actback-pass-kb` (V-E2) and `absent` for `actback-fail-kb` (V-E3); task shape is well-formed ("Task shape: contract") for the fixture's doc-set profile (T05) | the clean-context act-back reviewer (M6) attempts the representative task over each KB at runtime: **plan-correctness limb** (can a correct plan be produced from `actback-pass-kb` alone, with no insufficiency flags) and **sufficiency limb** (does `actback-fail-kb` force the reviewer to assume/guess/reach-for-source — are the `[ACTBACK]` flags well-founded) — these are *irreducibly LLM judgment* (the operational analog of M4's engine-narration limb); CI does not score the plan or validate the flags; the fixed `actback-pass-kb`/`actback-fail-kb` fixture pair (task-039) is the anchoring substrate |
