# task-047: aid-update-kb thin-router SKILL.md skeleton + state-detection
...
- f008 Part 2 (FR-27, AC8) -- the **NEW** `canonical/skills/aid-update-kb/SKILL.md` thin-router
  ...
  per-state reference docs are task-048; this task builds the router that points at them.
...
- Reference the f010 boundary (prompt-driven-targeted vs `aid-housekeep` source-driven-global) but
  do NOT draw the contract here (FR-33/FR-34 boundary is delivery-009/f010).
- ASCII-only (C2; sibling consistency).

**Acceptance Criteria:**
- [ ] `canonical/skills/aid-update-kb/SKILL.md` exists as a thin-router (frontmatter +
  state-detection + Dispatch table only -- no per-state behavior bodies; those are task-048).
- [ ] Frontmatter: `name: aid-update-kb`,
