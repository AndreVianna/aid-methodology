# task-042: per-doc freshness in both dashboard readers (Python + Node, byte-parity)

**Type:** IMPLEMENT

**Source:** work-001-kb-skills-improvement -> delivery-007

**Depends on:** task-040, task-001 (delivery-001)
...
- Add a **per-doc freshness read** to BOTH dashboard readers in lockstep so the byte-parity contract is
  ...
  re-implement the SAME algorithm as `kb-freshness-check.sh` (task-040) -- same git verbs, same fold
...
the readers re-implement the scan rather than shell out -- pinned to one verdict by the task-044 parity
...
`home.html` per-doc badge/marker (task-043 -- the UI that reads `doc_freshness`), does NOT author the
parity suite (task-044), and does NOT build the consumers `aid-update-kb`/`aid-housekeep`
(f008/f010, delivery-008/008). It does NOT redefine `sources:`/`approved_at_commit:` (f001, d001).
...
- [ ] `dashboard/reader/derivation.py` gains `derive_doc_freshness(kb_dir, repo_root)` and
  ...
  the same algorithm as task-040 (same `git log -1 --format=%H -- <src>` + `merge-base --is-ancestor`
...
- [ ] Degrade matrix matches task-040: a git failure / URL / untracked source / missing baseline never
...
- [ ] All section-6 quality gates pass (existing reader unit tests still pass; the dashboard reader CI
  green). The Python===Node byte-parity is asserted by task-044 (this task makes parity achievable;
  task-044 verifies it).
