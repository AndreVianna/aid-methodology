# task-053: housekeep <-> update-kb boundary contract record

**Type:** IMPLEMENT

**Source:** work-001-kb-skills-improvement -> delivery-009

**Depends on:** task-048 (delivery-008)
...
- f010 Part 1 (FR-33, AC10) + Part 4 (durable artifact) -- record the non-overlapping
  ...
  `state-kb-delta.md` scoping/closure rewrite is task-054 and the behavioral guards are task-055.
...
- **Cross-delivery dependency (cite explicitly):** the contract documents the boundary AROUND the
  already-shipped `aid-update-kb` skill authored in **task-048 (delivery-008, f008)** -- this task
...
- **Boundary record (in `pipeline-contracts.md`):** append the 7-dimension contract table verbatim
  ...
  (new in f010, wired by task-054) and update-kb re-verifies before committing in DONE (f008,
  task-048, existing -- referenced not duplicated).
- **Skill description cross-refs (Part 4 item 2):** add a one-line boundary cross-reference to
  ...
  `aid-update-kb`'s description is f008's edit (task-048) -- NOT made here.
...
- [ ] The Closure dimension records both halves: housekeep re-verifies `closure-check.sh` before
  committing (f010) and update-kb before committing in DONE (f008/task-048, referenced).
...
- [ ] The cross-delivery dependency on task-048 (delivery-008, the shipped `aid-update-kb` whose
