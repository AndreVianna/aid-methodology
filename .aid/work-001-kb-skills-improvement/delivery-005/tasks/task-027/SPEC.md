# task-027: Doc-model tightening -- operational guidance first-class (concern-model + principles + document-expectations)
...
- **Extend f003's doc model (do NOT re-spec it)** with one structural rule: operational guidance an
  ...
  - **`canonical/aid/templates/kb-authoring/concern-model.md`** -- add an "Operational guidance is
    ...
    task-028's presence check consumes for its scoping.
...
`kb-actback-task.sh` (task-028 -- which *consumes* this owning-table), does NOT wire M6 into the panel
...
- [ ] The named section headings authored here are byte-identical to the headings task-028's presence
