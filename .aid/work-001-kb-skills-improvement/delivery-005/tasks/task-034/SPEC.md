# task-034: test-teachback-fixtures.sh (AC1, TEST-C)

**Type:** TEST

**Source:** work-001-kb-skills-improvement -> delivery-005

**Depends on:** task-033, task-012 (delivery-001)
...
- The suite runs f005's SHIPPED `kb-teachback-questions.sh` (delivery-001 task-012) + f004's SHIPPED
  `closure-check.sh` (delivery-001) over the task-033 `teachback/` fixtures (a `mktemp -d` copy of
...
`closure-check.sh` (f005/f004, delivery-001). It runs them over the task-033 fixtures and asserts the
