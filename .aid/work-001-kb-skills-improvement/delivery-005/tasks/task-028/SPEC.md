# task-028: kb-actback-task.sh -- representative-task selector + operational-structure presence check

**Type:** IMPLEMENT

**Source:** work-001-kb-skills-improvement -> delivery-005

**Depends on:** task-008 (delivery-001), task-027 (delivery-005)
...
- Author the NEW ASCII bash helper `canonical/aid/scripts/kb/kb-actback-task.sh` (the `aid/` path
  ...
  - **(2) Operational-structure presence check.** For each Full-Primary KB doc, grep the named
    ...
    `concern-model.md` four-classes->owning-concerns->docs map authored in task-027) so each doc is
...
- Consume `closure-check.sh`'s output-shape conventions (task-008, delivery-001) as the sibling table
  format to mirror, and task-027's owning-table (the `concern-model.md` four-classes->owning-concerns
...
does NOT author the doc-model rule or the owning-table (task-027 -- this script *consumes* it), does
...
- [ ] Function (2) greps the named operational sections (`## Conventions`/`## Invariants`/
  ...
  expected to own** via task-027's owning-table (no over-report of legitimately-absent classes).
