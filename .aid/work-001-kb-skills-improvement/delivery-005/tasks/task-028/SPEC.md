# task-028: Calibration fixture corpus (transcription/hollowness/coverage + control)
...
- Author the hand-built, ASCII, checked-in `calibration/` fixture corpus that the AC6 suite
  (task-031) runs over, under `tests/canonical/fixtures/kb-essence/calibration/` (f012 SPEC F2):
...
task-031 ([SPIKE-V2]); this task only plants docs whose fat/control separation is measurable. CAL-2
...
- [ ] The corpus is constructed so the planted docs separate cleanly from the control (the fat doc's overlap is materially higher than the control's; the gap doc misses a term the control covers) -- making the empirical floor in task-031 measurable.
