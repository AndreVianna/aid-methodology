...
> whoever next touches `canonical/aid/templates/knowledge-graph/vendor/`.
...
"/canonical/aid/scripts/graph"`) something to diff against the npm registry, so a version bump
...
(`canonical/aid/scripts/summarize/package.json` + its lock file), and the same reason: a
...
2. **Re-run the integrity grep** against the new bytes, before replacing anything:
   ...
   for f in <the changed files>; do
     ...
     grep -c "{reviewer_output_file}" "$f"; grep -c "{open_questions_file}" "$f"
     grep -ci "mermaid" "$f"
   done
...
6. **Run the full generator** (`run_generator.py`) so every profile picks up the new bytes, and
   ...
   `canonical/aid/templates/knowledge-graph/` already carries.
