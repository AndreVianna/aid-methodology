...
while IFS=$'\t' read -r p gitex; do
    ...
    case "$p" in
        canonical/skills/*/*)
            _rest="${p#canonical/skills/}"
            node="canonical/skills/${_rest%%/*}/"
            case "${_rest#*/}" in
                SKILL.md) printf '%s\tskill\n' "$node" >> "$W/convdir.tsv" ;;
            esac
            ;;
        canonical/agents/*/*)
            _rest="${p#canonical/agents/}"
            node="canonical/agents/${_rest%%/*}/"
            case "${_rest#*/}" in
...
SHORTCUT_CATALOG="canonical/aid/templates/shortcut-catalog.yml"
...
GENERATED_FILES="canonical/aid/templates/generated-files.txt"
...
if [ -s "$W/scanfiles.txt" ]; then
    tr '\n' '\0' < "$W/scanfiles.txt" | xargs -0 awk \
        ...
        function kindof(target, matched, line, from,   fb) {
            ...
            if (from ~ /^canonical\/skills\// && target == "canonical/aid/templates/shortcut-engine.md")
                return "convention"
        ...
        }
        {
            ...
            while (match(rest, /\{\{include:[A-Za-z0-9._-]+\}\}/)) {
                ...
                tgt = "canonical/aid/templates/" nm ".md"
