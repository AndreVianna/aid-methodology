...
| 4 | `Rule` | yes | The ID of the rule the finding violates, from the artifact's rule set in [`canonical/aid/templates/review-rubrics/INDEX.md`](review-rubrics/INDEX.md). Format `<CLASS>-<NN>` (e.g. `CODE-03`, `NAR-04`). See **Rule values** below. |
| 5 | `Doc` | yes | Affected file path (relative to repo root). Examples: `foo.md`, `canonical/aid/scripts/bar.sh`, `tests/canonical/baz.sh`. For doc-wide issues with no specific file, use `--` (the same sentinel as `Rule` and `Line`; **not** the em dash `—` this schema's older examples used). |
...
is defined once**, at [`canonical/aid/templates/grading-rubric.md#severity-scale`](canonical/aid/templates/grading-rubric.md#severity-scale) --
...
**Rows are written by `canonical/aid/scripts/review/writeback-ledger.sh`, one call per row.** Do not
re-emit the table.
...
- `canonical/aid/scripts/review/writeback-ledger.sh` — the sole writer of ledger rows
- `canonical/aid/scripts/grade.sh` — the grader that parses this ledger
- `canonical/aid/templates/review-rubrics/INDEX.md` — the rule sets the `Rule` column cites
