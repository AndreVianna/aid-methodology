...
started: "{YYYY-MM-DD}"
minimum_grade: "{resolved at runtime by `bash canonical/aid/scripts/config/read-setting.sh --skill {phase} --key minimum_grade --default A`; source is `.aid/settings.yml`}"
...
consumption contract: `canonical/aid/templates/connectors/consumption-protocol.md`. Coordinate
...
Artifact files (REQUIREMENTS.md, per-feature SPEC.md, PLAN.md, per-task DETAIL.md) carry **no**
inline `## Change Log` section. Content history -- what changed in the document -- is exactly what
git records, with author, date, and full diff, and without drift; a hand-maintained table
duplicates it at lower fidelity. This is the same reasoning the tracking-discipline rule already
applies to `.aid/knowledge/` docs, extended to work artifacts. Process state (where are we in the
workflow) is what THIS file is for, and it stays here.
...
<!-- AUTHORED -- single-delivery FLATTENED works only (see ## Delivery Lifecycle note above).
     ...
     `.aid/works/{work}/delivery-001-issues.md`; see `canonical/aid/templates/delivery-issues.md`) and runs
