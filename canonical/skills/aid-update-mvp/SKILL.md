...
name: aid-update-mvp
description: >
  Revise roadmap.md's ## MVP section only -- the first shippable slice:
  its contents, the line reasoning, what was cut, and its Status field
  (including the transition to Shipped <version>). May create the ## MVP
  section if roadmap.md exists without one. Reads and consumes an MVP seed
  when one is present in .aid/design/; never requires one. Asks every run
  which previously created outputs to update alongside it -- no stored list,
  no tracking metadata written. Everything outside ## MVP belongs to
  /aid-update-roadmap. When roadmap.md is absent, routes to
