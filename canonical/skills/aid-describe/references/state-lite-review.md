...
per `canonical/templates/reviewer-ledger-schema.md` (ONE markdown table, no narrative).
...
```bash
bash canonical/scripts/grade.sh --explain .aid/.temp/review-pending/interview-<work>-lite.md
```
...
`canonical/templates/reviewer-ledger-schema.md`. The table is the entire file content
