...
**Trigger:** user types `/aid-describe escalate`, or selects an escalate option
...
```
| {today} | Interview restarted — escalated from lite path ({Sub-path}) | /aid-describe escalation |
```
...
```
| {today} | Escalated from {current-state} to full path — {escalation-rationale} | /aid-describe escalation |
```
...
> decomposition step (/aid-define State 5: FEATURE-DECOMPOSITION) will
