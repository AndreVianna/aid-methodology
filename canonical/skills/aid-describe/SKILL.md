...
2. **Locate and enter:**
   `bash canonical/aid/scripts/works/worktree-lifecycle.sh locate <work_id>` -- resolves via
...
(`canonical/aid/templates/work-initiation-gate.md`) before doing anything else. Run its
enumeration helper (which scans `.aid/works/*` across the main tree **and every git
worktree**):

```bash
bash canonical/aid/scripts/works/enumerate-works.sh
```
...
2. **Create and enter the worktree, before authoring anything** (gate `§ 3a` step 2):
   `bash canonical/aid/scripts/works/worktree-lifecycle.sh create work-001 {name}` --
...
- **New work:** ask for a name; **resolve `<work-id>`** as `work-{NNN+1}`, where `NNN` is
  ...
  `bash canonical/aid/scripts/works/worktree-lifecycle.sh create <work-id> {name}` -- STOP
