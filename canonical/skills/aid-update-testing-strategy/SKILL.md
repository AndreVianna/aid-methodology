...
description: >
  Revise the project's quality (C6) documents -- the test landscape (levels,
  coverage expectations, CI lane mapping, known gaps) and the gate policy (what
  blocks a merge, the thresholds, who may waive one) -- plus any previously created
  outputs you name. Requires no design seed: the change you state in the run is a
  sufficient input. Reads and consumes a testing-strategy seed when one is present
  in .aid/design/. When a C6 document does not yet exist, routes to
  /aid-create-testing-strategy. To author or revise test CODE rather than the
  strategy, use /aid-create-test or /aid-update-test. Produced by the aid-architect
