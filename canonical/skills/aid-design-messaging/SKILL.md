...
description: >
  Develop a messaging design as a DESIGN SEED in .aid/design/messaging.md -- the
  message/event schema, its channel, and its emission points. Grounded in the
  Knowledge Base (.aid/knowledge/) and the project source. It WRITES NO production
  code and NO KB document -- realize the seed into the built schema and emission
  with /aid-create-messaging once it is ready. Produced by the aid-architect agent
  and independently verified by aid-reviewer (full verify). Allocates a work-NNN
  folder.
allowed-tools: Read, Glob, Grep, Bash, Write, Edit, Agent
argument-hint: "<subject> -- the message/event to design (schema, channel, emission)"
---

# Design Messaging (develop a design seed, resolve nothing)

Bind **VERB=`design`**, **ARTIFACT=`messaging`** -- the `design` *stage* in the contract's
...
restates none of its rules. The seed it writes is `.aid/design/messaging.md`; realizing it
into the built schema and emission is `/aid-create-messaging`'s job, not this one's.

- **Boundary vs `/aid-create-messaging`:** that skill *builds* the schema and its emission
  from a ready seed; this one only *develops* the seed. Neither resolves anything on its
  own -- the user decides when a seed is ready to realize.
