...
`canonical/aid/scripts/kb/kb-dual-intent-probes.sh essence`) below. The probes are seeded
...
**Severity comes from the rule the row cites, and is never chosen here.**
- A **Divergence** cites `NAR-05`, whose anchor is `Step 2` -- derive it from the instance per the
  canonical scale. A KB that misrepresents the project is normally `[HIGH]`.
- A load-bearing **Omission** cites the closest `KB-22`..`KB-26`; take that rule's anchor. Those are
  `[HIGH]` for an unstated contract, invariant, gotcha or quality bar, and `[LOW]` (escaping to
  `[MEDIUM]` beyond one doc) for an unstated convention.
- This used to read *"every load-bearing Omission = `[MEDIUM]`"*, which no row could satisfy: the
  only rules available to an Omission anchor at `[HIGH]` or `[LOW]`, so a fixed `[MEDIUM]` put the
  Severity cell in contradiction with the `Rule` cell beside it on every single row.
