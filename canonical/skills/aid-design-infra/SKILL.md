...
description: >
  Develop an infrastructure-resource design as a DESIGN SEED in
  .aid/design/infra.md -- the resource, its configuration, and its provisioning
  and teardown. Grounded in the Knowledge Base (.aid/knowledge/) and the project
  source. It WRITES NO production code and NO KB document -- realize the seed into
  the provisioned resource with /aid-create-infra once it is ready. To design the
  pipeline that ships to the resource rather than the resource itself, use
  /aid-design-cicd instead. Produced by the aid-architect agent and independently
