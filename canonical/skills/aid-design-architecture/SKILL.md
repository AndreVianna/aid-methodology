...
description: >
  ...
  component. Grounded in the Knowledge Base (.aid/knowledge/) and the project
  source. It WRITES NO KB document and NO production code -- realize the seed into
  the project's C1 document with /aid-create-architecture once it is ready. For a
  subject with no dedicated design row use bare /aid-design; to write documentation
  about an existing architecture rather than design it, use
  /aid-document-architecture. Produced by the aid-architect agent and independently
...
(build & shape, `canonical/aid/templates/kb-authoring/concern-model.md`), never a hardcoded
