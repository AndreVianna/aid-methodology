...
- Copy `canonical/templates/settings.yml` → `.aid/settings.yml` verbatim.
...
Run:
```bash
bash canonical/scripts/config/read-setting.sh --path <key> --default '(unset)'
```
...
The canonical resolution helper is `canonical/scripts/config/read-setting.sh`. Consumer skills invoke:
...
Canonical schema: `canonical/templates/settings.yml`. Top-level sections + per-skill overrides as described in the tables above.
