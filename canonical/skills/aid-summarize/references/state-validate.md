...
▶ validation suite starting (~1 min total — **2** validators actually run, `validate-html-output.sh` and `contrast-check.mjs`, at ~30 s each per `canonical/aid/templates/rough-time-hints.md`; the coverage and retired-runtime checks are inline)
Run the emitter against a **per-cycle scratch ledger**, never against the canonical one:

```bash
N=<this cycle's number, 1 on first entry>
bash canonical/aid/scripts/summarize/emit-summary-findings.sh .aid/knowledge/kb.html \
...
> `grep -c validate-visuals canonical/aid/scripts/summarize/emit-summary-findings.sh`.
...
```bash
bash canonical/aid/scripts/review/check-gaps.sh --ledger .aid/.temp/review-pending/summarize.md   # exit 1 = an open criteria gap; do NOT grade
