...
1. **`canonical/scripts/summarize/validate-visuals.mjs`** — **S7 visual-fidelity gate (FR-51):** Playwright-renders `kb.html` in a headless Chromium browser (offline, `file://` URL, no network) and asserts, for every authored visual (inline `<svg>` / `.diagram-box` / infographic container):
