...
See `canonical/aid/templates/knowledge-summary/grading-rubric.md` for the per-check definitions and pass criteria. It defines no grade and carries no ladder -- the letter comes from `grade.sh` over the ledger, per `canonical/aid/templates/grading-rubric.md`, AID's single grading rubric.
