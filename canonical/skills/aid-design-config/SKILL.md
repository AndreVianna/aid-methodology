...
description: >
  ...
  .aid/design/config.md -- the option or flag, its default and scope, and how it
  is read. Grounded in the Knowledge Base (.aid/knowledge/) and the project source.
  It WRITES NO production code and NO KB document -- realize the seed into the
  built option with /aid-create-config once it is ready. To design the technology
  stack itself rather than a configuration option within it, use /aid-design-stack
  instead. Produced by the aid-architect agent and independently verified by
