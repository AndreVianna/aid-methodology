...
- **Ledger lifecycle:** "Append new findings as rows with Status: Pending to
  ...
  first if it exists. Output per `canonical/templates/reviewer-ledger-schema.md` —
  ONE table, no narrative. After writing the ledger, run:
  `bash canonical/scripts/grade.sh .aid/.temp/review-pending/interview-<work>-cross-ref.md`
...
```bash
bash canonical/scripts/grade.sh --explain .aid/.temp/review-pending/interview-<work>-cross-ref.md
```
