...
to the `aid-reviewer` sub-agent. Follows `canonical/aid/templates/reviewer-dispatch.md`.
...
RUBRIC: canonical/aid/templates/grading-rubric.md (universal severity → grade table)
  - Grade is COMPUTED by canonical/aid/scripts/grade.sh, not judged
...
DELIVERABLES:
  ...
  - Grade: per canonical/aid/scripts/grade.sh; minimum resolved via
    `bash canonical/aid/scripts/config/read-setting.sh --skill execute --key minimum_grade --default A`
