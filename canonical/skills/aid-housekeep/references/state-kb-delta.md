...
Every extraction subagent in `canonical/skills/aid-discover/references/agent-prompts.md`
...
Read each dispatched agent's full prompt from `canonical/skills/aid-discover/references/
...
(`canonical/skills/aid-discover/SKILL.md S Targeted Discovery (Re-entry)`),
...
- `/aid-discover`'s targeted re-entry flips the synthesized entry's
  ...
  `Pending` (`bash canonical/aid/scripts/summarize/writeback-state.sh --set kb_grade Pending`
