...
SLUG=$(bash canonical/scripts/housekeep/housekeep-state.sh --state "$STATE_FILE" --read --field "Branch" | sed 's#^aid/housekeep-##')
[ -z "$SLUG" ] && SLUG="$(date +%Y%m%d)"
bash canonical/scripts/housekeep/branch-commit.sh --ensure-branch --slug "$SLUG"
```
