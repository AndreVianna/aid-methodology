...
description: >
  Develop the defined-and-prioritized item set as a DESIGN SEED in
  .aid/design/backlog.md -- item definitions, done-conditions, priorities,
  and which tech-debt.md rows the user is accepting into the plan. Grounded
  in the Knowledge Base (.aid/knowledge/, including tech-debt.md) and the
  project source. It WRITES NO KB DOCUMENT and NO production code -- realize
  the seed into backlog.md with /aid-create-backlog once it is ready. For
  direction rather than items, use /aid-design-roadmap instead. Produced by
  the aid-architect agent and independently verified by aid-reviewer (full
