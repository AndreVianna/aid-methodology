...
name: aid-design-api
description: >
  Develop an API design as a DESIGN SEED in .aid/design/api.md -- the resource
  shape, the request/response contract, and the error model. Grounded in the
  Knowledge Base (.aid/knowledge/) and the project source. It WRITES NO production
  code and NO KB document -- realize the seed into the built endpoint with
  /aid-create-api once it is ready. Produced by the aid-architect agent and
