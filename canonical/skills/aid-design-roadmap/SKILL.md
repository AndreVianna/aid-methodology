...
description: >
  Develop the project's committed direction as a DESIGN SEED in
  .aid/design/roadmap.md -- what is committed vs. merely wanted, the
  sequencing rationale, and the alternatives being rejected and why. Grounded
  in the Knowledge Base (.aid/knowledge/) and the project source. It WRITES NO
  KB DOCUMENT and NO production code -- realize the seed into roadmap.md with
  /aid-create-roadmap once it is ready. For drawing the MVP line as its own
  act, use /aid-design-mvp instead. Produced by the aid-architect agent and
  independently verified by aid-reviewer (full verify). Allocates a work-NNN
  folder.
