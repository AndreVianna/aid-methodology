...
> Appended by every dispatcher on subagent completion (L1+L2+L3 traceability; always-on, never optional). One row per dispatch. Created on first use ("create section if missing"). Source: `canonical/skills/aid-discover/SKILL.md ## Dispatch Protocol`.
