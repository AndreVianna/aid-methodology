...
### .aid/knowledge/test-landscape.md
```markdown
# Test Landscape
...
```bash
# Run all unit tests
{exact command — e.g., mvn test, npm test, dotnet test, pytest}

# Run unit tests for a specific module/package
{exact command — e.g., mvn test -pl module-name, npm test -- --testPathPattern=module}

# Run tests with coverage report
{exact command — e.g., mvn test jacoco:report, npm run test:coverage, dotnet test --collect:"XPlat Code Coverage"}
```

## CI/CD Integration
{how tests run in CI, pipeline file location}
...
> Top-level sections mirror `canonical/templates/knowledge-base/infrastructure.md`.
