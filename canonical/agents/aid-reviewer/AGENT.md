...
- **Look severity up** against the violated rule's anchor — see [`canonical/aid/templates/grading-rubric.md#severity-scale`](canonical/aid/templates/grading-rubric.md#severity-scale). You do not *assign* it; the rule and the instance determine it. **No invented severity bands.**
...
- **No criterion, no finding.** Report the gap. Do not invent the rule, and do not substitute
  ...
  `writeback-ledger.sh --append-gap` — see [`canonical/aid/templates/criteria-gap-protocol.md`](canonical/aid/templates/criteria-gap-protocol.md).
...
Your output is a single markdown table in **the scratch ledger path you were given**, per the schema at `canonical/aid/templates/reviewer-ledger-schema.md`. You are never told the durable ledger's path — see the resume contract below.
