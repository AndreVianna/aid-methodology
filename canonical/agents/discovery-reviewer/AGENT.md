...
description: >
  ...
  Cross-references claims against actual source code. Produces {reviewer_output_file}.
  Also adds new questions to {reviewer_output_file} when review findings reveal information gaps.
tier: large
...
## ⚠️ Adding Questions to {reviewer_output_file}
...
**When you find such a gap, you MUST add it to `.aid/knowledge/{reviewer_output_file}`.**

1. Read the existing {reviewer_output_file} to find the highest Q{N} ID
...
the orchestrator tells you about prior runs. If {reviewer_output_file} already exists on disk,
...
4. `{project_context_file}` (project root)
...
1. **{reviewer_output_file}** — Are all Pending questions still genuinely unanswerable from code? Did any primary doc already resolve one? A question marked Pending when the answer is in the codebase = [MEDIUM]. Are impact levels reasonable? Is the Q&A format correct (ID, category, impact, status, context, suggested)?
...
4. **{project_context_file}** — Do build commands, conventions, architecture, and project overview match what the primary docs say? Wrong or outdated commands = [HIGH]. Stale or contradictory content = [MEDIUM].
...
Your output is a single markdown file at `.aid/.temp/review-pending/discovery.md` containing **exactly one markdown table** per the schema at `canonical/templates/reviewer-ledger-schema.md`.
...
**Additionally**, write answers to new Discovery Q&A entries into `.aid/knowledge/{reviewer_output_file}` following the section format specified in the "Adding Questions" section above. The Q&A file is separate from the ledger and NOT a schema table.
...
|---|---|---|---|---|---|---|
| 1 | [HIGH] | Pending | architecture.md | 42 | module count wrong: doc claims 7, disk shows 9 | `ls canonical/skills/ | wc -l` = 9 (doc claims 7) |
...
The Q&A file (`.aid/knowledge/{reviewer_output_file}`) is different: it genuinely **accumulates**
entries, so `cat >>` (append) is correct there — you are adding new Q&A sections, not rewriting a
single table:
```bash
cat >> .aid/knowledge/{reviewer_output_file} << 'KBEOF'
<Q&A entries here>
KBEOF
```
