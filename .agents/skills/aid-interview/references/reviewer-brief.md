...
passed to the `aid-reviewer` sub-agent. Follows `.agents/templates/reviewer-dispatch.md`.
...
DELIVERABLES:
  ...
  - Grade: per .agents/scripts/grade.sh; minimum resolved via
    `bash .agents/scripts/config/read-setting.sh --skill interview --key minimum_grade --default A`
