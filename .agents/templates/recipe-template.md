---
name: {{recipe-name}}  # Fill in before saving as .agents/recipes/{name}.md. These slots are placeholders, not real recipe slots.
...
<!-- Multi-task recipes add ### task-002 etc. with `**Depends on:** task-NNN` per task; see .agents/recipes/add-crud-endpoint.md for a 3-task example. -->
...
as defined in .agents/templates/delivery-plans/task-template.md.
...
File must live directly under .agents/recipes/ (not in a subdirectory). Name the
