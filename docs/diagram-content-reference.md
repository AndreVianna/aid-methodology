...
| A **skill** rename / add / remove (changes the **111** count: 17 curated + 64 shortcuts + 30 repurpose skills) | kb.html **Four-plane module map** ("111 skills"); site **methodology skill diagrams**; README **R1**; the `gen-reference` roster test (asserts 111 on-disk dirs = the 17 curated skill names ∪ all 94 catalog rows — keep this assertion in sync if the count changes again) |
