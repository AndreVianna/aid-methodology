...
| A **skill** rename / add / remove (changes the **108** count: 14 curated + 64 shortcuts + 30 repurpose skills) | kb.html **Four-plane module map** ("108 skills"); site **methodology skill diagrams**; README **R1**; the `gen-reference` roster test (asserts 108 on-disk dirs = the 14 curated skill names ∪ all 94 catalog rows — keep this assertion in sync if the count changes again) |
...
- **Must show:** four planes — **Distribution** (install scripts, `lib/` install-core, `bin/`, `packages/`),
  **Toolkit** (`canonical/skills/` **108 skills** — 14 curated + 94 catalog skills (64 shortcuts + 30 repurpose),
...
- **Source of truth:** `.aid/knowledge/module-map.md` · skill count = `ls -1d canonical/skills/*/ | wc -l` (108) ·
  agent count = `ls -1d canonical/agents/*/ | wc -l` (9).
- **Update when:** the skill count (**108**) or agent count (**9**) changes; a plane/module is added;
