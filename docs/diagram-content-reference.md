...
| A **skill** rename / add / remove (changes the **82** count: 14 classic + `aid-triage` + 67 shortcuts) | kb.html **Four-plane module map** ("82 skills"); site **methodology skill diagrams**; README **R1**; the `gen-reference` roster test (asserts 82 on-disk dirs = 15 curated sections + 67 catalog shortcuts — keep this assertion in sync if the count changes again) |
