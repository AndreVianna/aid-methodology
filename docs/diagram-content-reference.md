...
| A **skill** rename / add / remove (changes the **111** count: 17 curated + 64 shortcuts + 30 repurpose skills) | kb.html **Four-plane module map** ("113 skills"); site **methodology skill diagrams**; README **R1**; the `gen-reference` roster test (asserts 111 on-disk dirs = the 17 curated skill names ∪ all 94 catalog rows — keep this assertion in sync if the count changes again) |
...
- **Must show:** four planes — **Distribution** (install scripts, `lib/` install-core, `bin/`, `packages/`),
  **Toolkit** (`canonical/skills/` **113 skills** — 17 curated + 94 catalog skills (64 shortcuts + 30 repurpose),
