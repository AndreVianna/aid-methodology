...
|   |-- skills/                 <- 78 skill definitions (20 curated + 58 catalog skills)
...
- `canonical/skills/` — 78 skill definitions, one directory per skill: 20 curated skills — the 7 pipeline-phase skills implementing the six numbered phases (`aid-discover`, `aid-describe`, `aid-define`, `aid-specify`, `aid-plan`, `aid-detail`, `aid-execute` — Describe/Define are Phase 2a/2b of the one Describe→Define phase), the `aid-config` bootstrap, the optional `aid-summarize` viewer and `aid-graph` knowledge-relationship graph, 7 off-pipeline on-demand skills (`aid-housekeep`, `aid-update-kb`, `aid-set-connector`, `aid-unset-connector`, `aid-read-ticket`, `aid-create-ticket`, `aid-update-ticket`), and the standalone `aid-triage` router — plus the 58-row shortcut catalog's skills: 34 verb-first Lite-Path shortcut doorways (`aid-fix`, `aid-create-api`, `aid-update-ui`, …) and 24 hand-authored `repurpose` skills (`aid-review`, `aid-research`, `aid-report`, `aid-document`, `aid-test`, `aid-prototype`, `aid-design`, plus the re-registered `aid-deploy` / `aid-monitor` / `aid-ask`)
- `canonical/agents/` — 10 agent definitions
- `canonical/aid/templates/` — KB document templates, grading rubric, task templates, `delivery-blueprint-template.md` and `task-detail-template.md` (delivery/task definitions), plus the shortcut system: `shortcut-catalog.yml` (the 58-row catalog every shortcut and `aid-triage` resolve against), `shortcut-engine.md` (the shared state machine every shortcut delegates to), and `shortcut-scaffolding/` (family-specific SPEC/PLAN/DETAIL scaffolding)
