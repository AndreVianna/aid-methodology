...
|   |-- skills/                 <- 113 skill definitions (19 curated + 94 catalog skills)
...
- `canonical/skills/` — 113 skill definitions, one directory per skill: 19 curated skills — the 7 pipeline-phase skills implementing the six numbered phases (`aid-discover`, `aid-describe`, `aid-define`, `aid-specify`, `aid-plan`, `aid-detail`, `aid-execute` — Describe/Define are Phase 2a/2b of the one Describe→Define phase), the `aid-config` bootstrap, the optional `aid-summarize` viewer, 9 off-pipeline on-demand skills (`aid-housekeep`, `aid-update-kb`, `aid-set-connector`, `aid-unset-connector`, `aid-read-ticket`, `aid-create-ticket`, `aid-update-ticket`, `aid-light-review`, `aid-deep-review`), and the standalone `aid-triage` router — plus the 94-row shortcut catalog's skills: 64 verb-first Lite-Path shortcut doorways (`aid-fix`, `aid-create-api`, `aid-change-ui`, …) and 30 hand-authored `repurpose` skills (`aid-review`, `aid-research`, `aid-report`, `aid-document`, `aid-test`, `aid-prototype`, `aid-design`, plus the re-registered `aid-deploy` / `aid-monitor` / `aid-query-kb` / `aid-ask`)
