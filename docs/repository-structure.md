...
|   |-- skills/                 <- 94 skill definitions (16 classic + aid-triage + aid-ask + 76 shortcuts)
...
- `canonical/skills/` — 94 skill definitions, one directory per skill: 16 classic pipeline / on-demand skills (11 pipeline + 5 off-pipeline on-demand: `aid-housekeep`, `aid-query-kb`, `aid-update-kb`, `aid-set-connector`, `aid-unset-connector`), the standalone `aid-triage` router, `aid-ask` (a Q&A alias of the classic `aid-query-kb`), and 76 verb-first Lite-Path shortcut skills (`aid-fix`, `aid-create-api`, `aid-change-ui`, …)
